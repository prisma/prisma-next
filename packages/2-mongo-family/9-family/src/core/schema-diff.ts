  ...
  for (const name of [...allNames].sort()) {
    ...
    if (!liveColl && expectedColl) {
      ...
      collectionChildren.push({
        ...
        kind: 'collection',
        name,
        contractPath: `storage.namespaces.${UNBOUND_NAMESPACE_ID}.tables.${name}`,
  ...
  for (const [key, idx] of expectedLookup) {
    if (liveLookup.has(key)) {
      nodes.push({
        ...
        contractPath: `storage.namespaces.${UNBOUND_NAMESPACE_ID}.tables.${collName}.indexes`,
        code: 'MATCH',
    ...
    return [
      {
        ...
        kind: 'validator',
        name: 'validator',
        contractPath: `storage.namespaces.${UNBOUND_NAMESPACE_ID}.tables.${collName}.validator`,
    ...
    return [
      {
        ...
        kind: 'options',
        name: 'options',
        contractPath: `storage.namespaces.${UNBOUND_NAMESPACE_ID}.tables.${collName}.options`,
        code: 'EXTRA_OPTIONS',
