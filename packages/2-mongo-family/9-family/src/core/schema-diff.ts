  ...
  for (const name of [...allNames].sort()) {
    ...
    if (!liveColl && expectedColl) {
      ...
      collectionChildren.push({
        ...
        kind: 'collection',
        name,
        contractPath: `storage.${UNBOUND_NAMESPACE_ID}.collections.${name}`,
  ...
  for (const [key, idx] of expectedLookup) {
    if (liveLookup.has(key)) {
      nodes.push({
        ...
        contractPath: `storage.${UNBOUND_NAMESPACE_ID}.collections.${collName}.indexes`,
        code: 'MATCH',
    ...
    return [
      {
        ...
        kind: 'validator',
        name: 'validator',
        contractPath: `storage.${UNBOUND_NAMESPACE_ID}.collections.${collName}.validator`,
    ...
    return [
      {
        ...
        kind: 'options',
        name: 'options',
        contractPath: `storage.${UNBOUND_NAMESPACE_ID}.collections.${collName}.options`,
        code: 'EXTRA_OPTIONS',
