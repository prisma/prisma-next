...
import type { PrintDocument, PrintEnumSection } from './print-document';
...
export const DEFAULT_SQL_PRINT_HEADER =
...
export function legacySqlIrToPrintDocument(
  ...
  const headerComment = header ?? DEFAULT_SQL_PRINT_HEADER;
...
function processTable(
  ...
  for (const rel of relationFields) {
    ...
    if (rel.fields && rel.references) {
      ...
      parts.push(
        `fields: [${rel.fields
          .map((fn) => resolveColumnFieldName(fieldNamesByTable, table.name, fn))
          .join(', ')}]`,
      );
      parts.push(
        `references: [${rel.references
          .map((fn) => resolveColumnFieldName(fieldNamesByTable, rel.referencedTableName ?? '', fn))
          .join(', ')}]`,
      );
      if (rel.onDelete) {
