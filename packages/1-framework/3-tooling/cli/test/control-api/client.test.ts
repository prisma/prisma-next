...
describe('ControlClient progress emission', () => {
  ...
  describe('migrationApply()', () => {
    ...
    it('threads per-step invariants into runner.execute', async () => {
      const { mockFamily, mockTarget, mockAdapter, mockDriverDescriptor, mockFamilyInstance } =
        createMockComponents();
      const executeCalls: Array<{ invariants: readonly string[] | undefined }> = [];
      ...
      const mockTargetWithMigrations = {
        ...
        migrations: {
          ...
          createRunner: () => ({
            execute: async (opts: { invariants?: readonly string[] }) => {
              executeCalls.push({ invariants: opts.invariants });
              return {
                ok: true,
      ...
      await client.migrationApply({
        originHash: 'sha256:empty',
        destinationHash: 'sha256:bbb',
        pendingMigrations: [
          {
            dirName: '001_phone',
            from: 'sha256:empty',
      ...
      expect(executeCalls[0]!.invariants).toEqual(['backfill-user-phone']);
      expect(executeCalls[1]!.invariants).toEqual([]);
    });

    it('defaults invariants to [] on the runner when MigrationApplyStep omits the field', async () => {
      const { mockFamily, mockTarget, mockAdapter, mockDriverDescriptor, mockFamilyInstance } =
        createMockComponents();
      const executeCalls: Array<{ invariants: readonly string[] | undefined }> = [];
      ...
      expect(executeCalls[0]!.invariants).toEqual([]);
    });
