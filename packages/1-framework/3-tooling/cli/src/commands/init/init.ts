...
import { errorInitEmitFailed, errorInitInstallFailed, errorInitProbeFailed } from './errors';
import {
...
import { applyInitPlan, buildInitPlan, type InitPlan } from './plan';
...
import { targetPackageName } from './templates/code-templates';
import { MIN_SERVER_VERSION } from './templates/env';
...
export async function runInit(
  ...
  try {
    inputs = await resolveInitInputs({
      ...
      options,
      canPrompt,
      autoAcceptPrompts: Boolean(flags.yes),
    });
  } catch (error) {
  ...
  let plan: InitPlan;
  try {
    plan = buildInitPlan({ baseDir, inputs, pm });
  } catch (error) {
    if (CliStructuredError.is(error)) {
      return emitError(ui, flags, error);
    }
  ...
  // From here on, failures are only possible at the install/emit stages,
  // which the spec treats as discrete subsequent phases (FR6.3): scaffold
  // files remain on disk so the user can fix and retry.
  const applied = applyInitPlan(baseDir, plan, {
    onFileWritten: (file) => {
  ...
  filesWritten.push(...applied.filesWritten);
  filesDeleted.push(...applied.filesDeleted);
