...
export interface EnrichEnvironment {
  ...
  /**
   * Best-effort reader for the project's `package.json`. Returns `null`
   * on any failure (file missing, parse error, key missing). The sender
   * binds this to `() => readFileSync(join(projectRoot, 'package.json'))`.
   */
