...
describe('detectAgent', () => {
  ...
  it('detects Codex CLI from CODEX_SANDBOX regardless of sandbox backend value', () => {
    expect(detectAgent({ CODEX_SANDBOX: 'seatbelt' })).toBe('Codex CLI');
    expect(detectAgent({ CODEX_SANDBOX: 'landlock' })).toBe('Codex CLI');
  });

  it('detects Gemini CLI from GEMINI_CLI', () => {
    expect(detectAgent({ GEMINI_CLI: '1' })).toBe('Gemini CLI');
  });

  it('ignores inactive Gemini CLI marker values', () => {
    expect(detectAgent({ GEMINI_CLI: '' })).toBeNull();
    expect(detectAgent({ GEMINI_CLI: '0' })).toBeNull();
    expect(detectAgent({ GEMINI_CLI: 'false' })).toBeNull();
  });
