...
- `completion-context.ts` — pure cursor classifier for PSL completion contexts, currently routing model field type positions and marking everything outside slice scope unsupported.
- `completion-provider.ts` — pure completion item provider for supported model field type contexts.
