 ...
 * `targetTypes`, `meta`) and registration concerns (`paramsSchema`;
 * optional `renderOutputType`). The runtime `Codec` instance returned
 * by `factory(params)(ctx)` carries only the conversion behavior.
 *
 * Codec authors `extend` the {@link CodecDescriptorImpl} abstract class
 * to declare their codec id, traits, target types, params schema, the
 * `factory(params)` that materializes a typed `Codec<...>`, and
 * (optionally) a `renderOutputType(params)` for the emit path.
 *
 ...
 *
 * Codec authors extend this class with their typed `TParams` and
 * declare `codecId`, `traits`, `targetTypes`, `paramsSchema`, the
 * curried `factory(params)`, and (optionally) `renderOutputType`.
 *
 * Implements the {@link CodecDescriptor} interface so a concrete
 * subclass instance is directly usable wherever the framework expects
 * a `CodecDescriptor<P>`.
 */
