/**
 * `column()` packager + `ColumnSpec<R, P>` shape + `ColumnHelperFor<D>`
 * variants for tying per-codec column helpers to their descriptor.
 *
 * `ColumnSpec<R, P>` extends {@link ColumnTypeDescriptor} so it remains a
 * drop-in for contract authoring sites that consume `ColumnTypeDescriptor`
 * shapes — both types live at the framework-components layer so the
 * `extends` clause is real (no structural mirror).
 *
 * `column()` is a trivial, non-polymorphic packager. Generic over `R`
 * (the codec instance type returned by the descriptor's curried factory)
 * and `P` (the typeParams record). The framework does NOT try to infer
 * `R` and `P` from a descriptor — that path is the variance trap.
 * Per-codec helpers absorb the descriptor relationship instead and tie
 * themselves to their descriptor via `satisfies ColumnHelperFor<D>` or
 * `satisfies ColumnHelperForStrict<D>`.
 */
...
/**
 * Column spec carrying the codec factory closure alongside the
 * {@link ColumnTypeDescriptor} fields. Codec authors return a
 * `ColumnSpec` from per-codec column helpers; the runtime materializes
 * the codec instance by calling `codecFactory(ctx)` once it knows the
 * column's `CodecInstanceContext`.
 *
 * Extends {@link ColumnTypeDescriptor} so `ColumnSpec` instances flow
 * directly into contract-authoring sites that consume the descriptor
 * shape — no structural mirroring required.
 */
 ...
 *
 * `nativeType` is the column's database-native type spelling — the value
 * the postgres adapter's migration planner, the SQL renderer's cast
 * policy, and the contract's `meta.db.<family>.<target>.nativeType` slot
 * read. Per-codec helpers pass the literal native-type string for their
 * codec (e.g. `'text'`, `'int4'`, `'character varying'`); for codecs
 * whose native-type spelling depends on parameters (none today; reserved
 * for future shapes), the helper computes the rendered string before
 * calling `column`. The framework does not derive the value from
 * `codecId` — that mapping is target-specific and lives at the helper.
 */
...
/**
 * Coerce a descriptor's `factory` first parameter into the typeParams
 * shape `ColumnSpec` accepts. Non-parameterized descriptors (factory
 * with no params, or `params: void`) collapse to `undefined`;
 * parameterized descriptors keep the params record shape.
 */
