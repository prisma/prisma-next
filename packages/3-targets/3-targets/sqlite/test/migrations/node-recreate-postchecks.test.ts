...
describe('buildRecreateSummaryOnDiff', () => {
  it('joins each issue message', () => {
    const summary = buildRecreateSummaryOnDiff('users', [
...
describe('buildRecreatePostchecksOnDiff — column-level', () => {
  it('emits a nullability postcheck when nullability differs', () => {
    ...
    const checks = buildRecreatePostchecksOnDiff('users', issues, spec);
...
describe('buildRecreatePostchecksOnDiff — column-default', () => {
...
describe('buildRecreatePostchecksOnDiff — constraints', () => {
  ...
  it('detects an inline autoincrement primary key as the expected PK', () => {
    ...
    const checks = buildRecreatePostchecksOnDiff('t', issues, spec);
    const pkCheck = checks.find((c) => c.description.includes('primary key'));
    expect(pkCheck).toBeDefined();
  ...
  it('emits one foreign-key postcheck per declared FK when a foreign-key issue fires', () => {
    ...
    const checks = buildRecreatePostchecksOnDiff('posts', issues, spec);
