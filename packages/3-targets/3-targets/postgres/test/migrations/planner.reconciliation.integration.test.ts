...
describe.sequential('PostgresMigrationPlanner - reconciliation integration', () => {
  ...
  it(
    'postcheck distinguishes varchar(64) from varchar(255)',
    { timeout: testTimeout },
    async () => {
      // Baseline: table with varchar(64) column
      const baselineContract = makeContract(
        {
          item: makeTable({
            id: { nativeType: 'uuid', codecId: 'pg/uuid@1', nullable: false },
        ...
        'typmod-postcheck-baseline',
      );
      await applyBaseline(driver!, baselineContract);

      // Target: same column but varchar(255)
      const updatedContract = makeContract(
        {
          item: makeTable({
            id: { nativeType: 'uuid', codecId: 'pg/uuid@1', nullable: false },
        ...
        'typmod-postcheck-updated',
      );

      // Plan but don't execute — we want the postcheck SQL
      const schema = await introspectSchema(driver!);
      const planner = postgresTargetDescriptor.createPlanner(familyInstance);
      const planResult = planner.plan({
      ...
      // Find the ALTER TYPE operation and its postcheck
      const alterTypeOp = planResult.plan.operations.find((op) => op.id.startsWith('alterType.'));
      expect(alterTypeOp).toBeDefined();
      expect(alterTypeOp!.postcheck.length).toBeGreaterThan(0);

      // Run the postcheck against the UNCHANGED varchar(64) column.
      // It should return false (column is varchar(64), not varchar(255)),
      // but regtype drops typmods so it incorrectly returns true.
      const postcheckSql = alterTypeOp!.postcheck[0]!.sql;
      const result = await driver!.query<{ exists: boolean }>(postcheckSql);
