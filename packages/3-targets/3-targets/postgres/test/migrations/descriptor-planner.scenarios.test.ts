...
function contract(
  ...
  extras?: { types?: SqlContract<SqlStorage>['storage']['types'] },
  ...
  return {
    ...
    storage: {
      tables,
      ...(extras?.types !== undefined ? { types: extras.types } : {}),
    },
    models: {},
    relations: {},
    mappings: {},
...
function plan(
  from: SqlContract<SqlStorage> | null,
  to: SqlContract<SqlStorage>,
...
describe('reconciliation — alters', () => {
  ...
  it('18b: unsafe type change (text → int4) — dataTransform only', () => {
    const from = contract({
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(descriptorKinds(result)).toEqual(['dataTransform']);
    expect(result.needsDataMigration).toBe(true);
    const dt = result.descriptors[0] as Record<string, unknown>;
    expect(dt['name']).toContain('typechange-user-score');
  });
...
describe('types', () => {
  ...
  it('25: enum values added → conflict', () => {
    const from = contract(
      { user: table({ id: uuidCol(), role: col('user_role', 'pg/enum@1') }) },
    ...
    if (result.ok) return;
    expect(result.conflicts[0]?.summary).toContain('enum');
  });

  it('26: enum values removed → conflict', () => {
    const from = contract(
      { user: table({ id: uuidCol(), role: col('user_role', 'pg/enum@1') }) },
      { types: { UserRole: enumType(['admin', 'user']) } },
    );
  ...
  it('27: unknown codec type missing → conflict', () => {
    const to = contract(
...
describe('data-safety gaps', () => {
  it('S1: computed backfill — NOT NULL column on non-empty table needs data migration', () => {
    // This is handled by the notNullBackfillMatcher (test 8)
  ...
  it('S2: type change (text → int) emits dataTransform via typeChangeMatcher', () => {
    const from = contract({
    ...
    expect(descriptorKinds(result)).toEqual(['dataTransform']);
    const dt = result.descriptors[0] as Record<string, unknown>;
    expect(dt['name']).toContain('typechange-user-foo');
  });

  it('S2b: lossy type change (float → int) emits dataTransform', () => {
    const from = contract({
  ...
  it('S3: column split (name → firstName + lastName) — new NOT NULL columns detected', () => {
    const from = contract({
      user: table({ id: uuidCol(), name: textCol() }),
    });
    const to = contract({
