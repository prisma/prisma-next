...
describe('inferPostgresPslContract', () => {
  ...
  it('produces a @default(autoincrement()) attribute for an identity column (GENERATED ... AS IDENTITY)', () => {
    ...
    // IDENTITY` report no `column_default` at all — introspection threads
    // them onto `SqlColumnIR.identity` (a plain boolean; PSL has no syntax
    // to distinguish the two variants), and infer must print the same
    // `@default(autoincrement())` either way.
    const schemaIR = ir({
      tables: {
        session: {
          name: 'session',
          columns: {
            id: { name: 'id', nativeType: 'int4', nullable: false, identity: true },
    ...
    // A plain column with neither a raw default nor identity gets no
