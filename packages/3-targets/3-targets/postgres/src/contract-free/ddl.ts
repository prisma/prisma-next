 ...
 *
 * Precondition: identifiers (`table`, `schema`, column names/types) and
 * string-literal defaults are emitted to SQL verbatim — they are not quoted or
 * escaped. Callers must pass pre-trusted values (e.g. fixed control-plane
 * identifiers). Quoting/escaping for untrusted identifiers is added when the
 * migration planner adopts this lowering path.
 */
