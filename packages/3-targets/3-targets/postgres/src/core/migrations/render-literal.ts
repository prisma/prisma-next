 ...
 * classes) to convert literal argument values — strings, numbers, booleans,
 * arrays, plain objects — into TypeScript source. Pretty-prints with a soft
 * 80-column single-line budget, falling back to a block form otherwise.
 * Unknown values fall through to `String(value)`.
 *
 * Package-private to the Postgres target. Structural mirror of the Mongo
 * target's `render-literal.ts`.
 */
