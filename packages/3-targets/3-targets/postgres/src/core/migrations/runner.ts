...
class PostgresMigrationRunner implements SqlMigrationRunner<PostgresPlanTargetDetails> {
  ...
  async executeOnConnection(
    ...
    const existingMarker = await this.readMarkerForSpace(driver, space);
  ...
  async executeAcrossSpaces(options: {
    ...
    try {
      ...
      for (const spaceOptions of perSpaceOptions) {
        const space = spaceOptions.space ?? 'app';
  ...
  private async readMarkerForSpace(
    driver: SqlMigrationRunnerExecuteOptions<PostgresPlanTargetDetails>['driver'],
    space: string,
  ): Promise<ContractMarkerRecord | null> {
    if (space === 'app') {
      // Reuses the family-wide app-marker reader to keep the existing
      // single-space callers' SQL probes / parsing path unchanged.
      return this.family.readMarker({ driver });
    }
    const all = await this.family.readAllMarkers({ driver });
    return all.get(space) ?? null;
  }
