 ...
 *
 * Each `OpFactoryCall` renders itself via `renderTypeScript()` and declares
 * its own `importRequirements()`; this file just composes the module source
 * around those contributions. Codec-emitted Calls (e.g. cipherstash's
 * `*Call` classes) flow through here unchanged — the renderer is fully
 * polymorphic over the framework `OpFactoryCall` interface (ADR 195) and
 * doesn't care which package contributed a given Call. The design mirrors
 * the Mongo target's `render-typescript.ts` deliberately — byte-for-byte
 * alignment isn't required (different factory module specifiers, different
 * base-class name) but the shape is, so future consolidation to a
 * framework-level helper is mechanical.
 */
