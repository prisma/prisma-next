/**
 * Identity value resolution for PostgreSQL column types.
 *
 * An "identity value" is the monoid neutral element for a given type — the value
 * that, when combined with any other value of the same type under the type's
 * canonical operation, yields that other value unchanged (e.g. 0 for addition,
 * '' for string concatenation, false for boolean OR).
 *
 * These are used when adding a NOT NULL column without an explicit default to a
 * non-empty table: the planner temporarily sets the identity value as a default,
 * adds the column, then drops the default.
 */
