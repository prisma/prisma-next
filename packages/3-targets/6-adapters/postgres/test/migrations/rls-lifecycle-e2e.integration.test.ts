...
describe.sequential('RLS lifecycle e2e — edit replaces, removal fails verify', () => {
  ...
  it(
    ...
    async () => {
      ...
      // The dict key in entries.rlsPolicy is the PSL prefix ('p_read'), not the
      ...
      const policies = Object.values(nsA.entries.rlsPolicy);
  ...
  it(
    ...
    async () => {
      ...
      const nameA = Object.values(nsA.entries.rlsPolicy)[0]?.name ?? '';
      const nameB = Object.values(nsB.entries.rlsPolicy)[0]?.name ?? '';
  ...
  it(
    ...
    async () => {
      ...
      const issueMessages = verifyResult.schema.extensionIssues.map(
