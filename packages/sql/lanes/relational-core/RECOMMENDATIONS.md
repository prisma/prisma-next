...
## Observations
- ✅ `src/schema.ts` now uses `QueryLaneContext` instead of `RuntimeContext`, removing the dependency on `@prisma-next/sql-runtime`. This allows relational-core to be consumed by other lanes without pulling in runtime dependencies.
