...
> {
  protected verifyNamespace(options: {
    ...
    const collectionControl = (name: string) =>
      this.collectionControlForName(options.contract, name);
    const { issues } = diffMongoSchemas(live, expected, false, collectionControl);
    return issues;
  }
