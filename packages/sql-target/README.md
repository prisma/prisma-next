...
### Hook Initialization (`emitter-hook-init.ts`)
- Registers SQL hook with emitter registry
- Side-effect registration on package load
...
The SQL hook is automatically registered when the package is imported. The emitter uses it when emitting SQL contracts:

```typescript
import '@prisma-next/sql-target'; // Registers SQL hook
import { emit } from '@prisma-next/emitter';

// Emit SQL contract - SQL hook will be used automatically
