...
describe("end-to-end query with emitted contract", { timeout: 30000 }, () => {
  ...
  const contractJsonPath = resolve(
    __dirname,
    "fixtures/generated/contract.json"
  );
        ...
        await withClient(
          ...
          async (client: import("pg").Client) => {
            await setupE2EDatabase(
              client,
              contract,
              async (c: typeof client) => {
                await c.query('drop table if exists "user"');
                await c.query(
                  'create table "user" (id serial primary key, email text not null)'
                );
                await c.query(
                  'insert into "user" (email) values ($1), ($2), ($3)',
                  ["ada@example.com", "tess@example.com", "mike@example.com"]
                );
              }
            );

            const adapter = createPostgresAdapter();
            const runtime = createTestRuntimeFromClient(
              contract,
              client,
              adapter
            );
          ...
          async (client: import("pg").Client) => {
            ...
            try {
              ...
              const plan = sql<Contract, CodecTypes>({ contract, adapter })
                .from(user)
                .innerJoin(post, (on) =>
                  on.eqCol(user.columns.id!, post.columns.userId!)
                )
                .select({
          ...
          async (client: import("pg").Client) => {
            await setupE2EDatabase(
              ...
              async (c: typeof client) => {
                ...
                await c.query(
                  'insert into "post" ("userId", title) values ($1, $2)',
                  [1, "First Post"]
                );
              }
            );
            ...
            try {
              ...
              expect(rows.length).toBe(3);
              const adaRow = rows.find(
                (r: (typeof rows)[0]) => r.email === "ada@example.com"
              );
              const tessRow = rows.find(
                (r: (typeof rows)[0]) => r.email === "tess@example.com"
              );
              const mikeRow = rows.find(
                (r: (typeof rows)[0]) => r.email === "mike@example.com"
              );
          ...
          async (client: import("pg").Client) => {
            await setupE2EDatabase(
              ...
              async (c: typeof client) => {
                ...
                await c.query('insert into "user" (email) values ($1)', [
                  "ada@example.com",
                ]);
                await c.query(
                  'insert into "post" ("userId", title) values ($1, $2), ($3, $4)',
                  [1, "First Post", 999, "Orphan Post"]
                );
              }
            );
            ...
            try {
              ...
              const firstPostRow = rows.find(
                (r: (typeof rows)[0]) => r.title === "First Post"
              );
              const orphanPostRow = rows.find(
                (r: (typeof rows)[0]) => r.title === "Orphan Post"
              );
          ...
          async (client: import("pg").Client) => {
            await setupE2EDatabase(
              ...
              async (c: typeof client) => {
                ...
                await c.query(
                  'insert into "post" ("userId", title) values ($1, $2), ($1, $3)',
                  [1, "First Post", "Second Post"]
                );
                await c.query(
            ...
            try {
              ...
              const plan = sql<Contract, CodecTypes>({ contract, adapter })
                ...
                .leftJoin(comment, (on) =>
                  on.eqCol(post.columns.id!, comment.columns.postId!)
                )
                .select({
                  userId: user.columns.id!,
                  email: user.columns.email!,
              ...
              const firstPostRow = rows.find(
                (r: (typeof rows)[0]) =>
                  r.title === "First Post" && r.commentId !== null
              );
              const secondPostRow = rows.find(
                (r: (typeof rows)[0]) => r.title === "Second Post"
              );
        ...
        await withClient(
          ...
          async (client: import("pg").Client) => {
            ...
            try {
              ...
              expect(
                typeof (rows[0] as Record<string, unknown>).post_title
              ).toBe("number");
        ...
        await withClient(
          ...
          async (client: import("pg").Client) => {
            ...
            try {
              ...
              expect(typeof (rows[0] as Record<string, unknown>).a_b_c).toBe(
                "number"
              );
        ...
        await withClient(
          ...
          async (client: import("pg").Client) => {
            ...
            try {
              ...
              expect(typeof (rows[0] as Record<string, unknown>).post_id).toBe(
                "number"
              );
        ...
        await withClient(
          ...
          async (client: import("pg").Client) => {
            ...
            try {
              ...
              expect(
                typeof (rows[0] as Record<string, unknown>).post_author_name
              ).toBe("number");
              expect(typeof rows[0]?.email).toBe("string");
  ...
  it("includeMany returns one row per parent with nested array of children", async () => {
    const contract =
      await loadContractFromDisk<ContractWithCapabilities>(contractJsonPath);
    ...
    await withDevDatabase(
      async ({ connectionString }: DevDatabase) => {
        await withClient(connectionString, async (client: Client) => {
          ...
          try {
            const tables = schema<ContractWithCapabilities, CodecTypes>(
              contract
            ).tables;
            const user = tables["user"]!;
            const post = tables["post"]!;
            ...
            const tessRow = rows.find(
              (r: Row) => r.email === "tess@example.com"
            );
            expect(tessRow).toBeDefined();
            ...
            const mikeRow = rows.find(
              (r: Row) => r.email === "mike@example.com"
            );
            expect(mikeRow).toBeDefined();
        ...
        await withClient(connectionString, async (client: Client) => {
          ...
          try {
            ...
            const plan = sql<Contract, CodecTypes>({ contract, adapter })
              .from(post)
              .leftJoin(user, (on) =>
                on.eqCol(post.columns["userId"]!, user.columns["id"]!)
              )
              .select({
                postId: post.columns["id"]!,
