...
describe('Postgres migration executor integration', () => {
  ...
  describe('empty DB → full schema', () => {
    it('applies migration plan and updates marker', async () => {
      ...
      try {
        const executor = controlAdapter.createMigrationExecutor(driver, []);
  ...
  describe('lock contention', () => {
    it('fails when lock is already held', async () => {
      ...
      try {
        ...
        const controlAdapter = postgresAdapter.create();
        try {
          const executor = controlAdapter.createMigrationExecutor(driver2, []);
          const result = await executeMigration({ plan, driver: driver2, executor });
