 ...
 * assembly reads `types.codecTypes.import` / `typeImports` to thread the
 * type-side imports into emitted `contract.d.ts`, and `types.storage`
 * declares the codec id's storage backing (`jsonb` on Postgres).
 *
 * Per Phase 3.5a of codec-registry-unification, `codecInstances` is empty:
 ...
 * codec lookup. Control-stack consumers that previously read codec metadata
 * from `codecInstances` migrate to `descriptorFor('arktype/json@1')`.
 */
...
const arktypeJsonPackMetaBase = {
  ...
  types: {
    codecTypes: {
      // Phase 3.5a T3.5.13 pattern: parameterized descriptor is the single
      // source of truth for codec-id-keyed metadata; the legacy
