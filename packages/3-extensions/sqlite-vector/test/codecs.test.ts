...
describe('sqlite-vector codecs', () => {
  it('has vector codec registered', () => {
    const vectorDef = codecDefinitions.vector;
    expect(vectorDef).toBeDefined();
    expect(vectorDef.typeId).toBe('sqlite/vector@1');
    expect(vectorDef.codec.targetTypes).toEqual(['text']);
  });

  it('encodes number array to JSON', () => {
  ...
  it('throws error when decoding invalid JSON', () => {
    const vectorCodec = codecDefinitions.vector.codec;

    expect(() => {
      vectorCodec.decode('not json');
    }).toThrow();
  });
