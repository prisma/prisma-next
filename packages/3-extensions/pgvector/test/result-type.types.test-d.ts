...
type ContractWithNullableVector = ContractWithTypeMaps<
  SqlContract<
    {
      readonly tables: {
        readonly post: {
  ...
  >,
  TypeMaps<NullableVectorCodecTypes, PgVectorOperationTypes>
>;

type NonNullableVectorCodecTypes = {
  readonly 'pg/int4@1': { readonly output: number };
  readonly 'pg/vector@1': { readonly output: number[] };
};

type ContractWithNonNullableVector = ContractWithTypeMaps<
  SqlContract<
    {
      readonly tables: {
        readonly post: {
  ...
  >,
  TypeMaps<NonNullableVectorCodecTypes, PgVectorOperationTypes>
>;
...
test('ResultType infers Vector<1536> | null for parameterized nullable vector column', () => {
  ...
  const tables = schema<ContractWithNullableVector>(context).tables;
  const postTable = tables['post'];
  if (!postTable) throw new Error('post table not found');
  const postColumns = postTable.columns;

  const _plan = sql<ContractWithNullableVector>({ context })
    .from(postTable)
    .select({
      id: postColumns['id']!,
