...
// set. We use `{ uri, dbName }` rather than `{ url }` because
// MongoMemoryReplSet returns URIs with a `?replicaSet=...` query string, which
// makes path-based dbName injection ambiguous.
describe('mongo() facade — e2e', { timeout: timeouts.spinUpMongoMemoryServer }, () => {
