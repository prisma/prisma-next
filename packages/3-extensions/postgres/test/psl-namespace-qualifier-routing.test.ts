...
/**
 * End-to-end demonstration that the namespace-qualifier wiring lines up:
 * a PSL document goes through the SQL PSL interpreter; the Postgres
 * `createNamespace` factory threads target-specific `Namespace`
 * concretions into `SqlStorage.namespaces`; and looking up a model's
 * `namespaceId` in that map yields the right qualifier behaviour for DDL
 * emission (`PostgresUnboundSchema` elides; `PostgresSchema(id)`
 * qualifies).
 */
describe('PSL → SqlStorage.namespaces qualifier routing (end-to-end)', () => {
  ...
  it('`namespace unbound { … }` lowers to PostgresUnboundSchema, whose qualifyTable elides the schema prefix', () => {
    const result = interpretPslDocumentToSqlContract({
      ...parseAndResolve(`namespace unbound {
  model Tenant {
    id Int @id
  }
}
`),
  ...
  it('`namespace auth { … }` lowers to PostgresSchema("auth"), whose qualifyTable emits `"auth"."<table>"`', () => {
    const result = interpretPslDocumentToSqlContract({
      ...parseAndResolve(`namespace auth {
  model User {
    id Int @id
  }
}
