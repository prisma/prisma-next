...
// ---------------------------------------------------------------------------
// 2. db.enums accessor keeps the literal int tuple.
// ---------------------------------------------------------------------------

type Accessors = typeof contract extends { enumAccessors: infer A } ? A : never;
type LevelValues = Accessors extends { Level: { values: infer V } } ? V : never;

test('db.enums int tuple keeps its literal declaration order', () => {
  expectTypeOf<LevelValues>().toEqualTypeOf<readonly [1, 10]>();
});

test('db.enums int tuple is not widened to readonly number[]', () => {
