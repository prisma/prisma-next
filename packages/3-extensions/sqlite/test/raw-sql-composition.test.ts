            ...
            tables: {
              users: {
                columns: {
                  ...
                  name: { codecId: 'sqlite/text@1', nullable: false },
                  score: { codecId: 'sqlite/integer@1', nullable: true },
                },
                uniques: [],
                indexes: [],
                foreignKeys: [],
              ...
              events: {
                columns: {
                  id: { codecId: 'sqlite/integer@1', nullable: false },
                  createdAt: { codecId: 'sqlite/text@1', nullable: false },
                  score: { codecId: 'sqlite/integer@1', nullable: true },
                },
                uniques: [],
                indexes: [],
...
describe('rawSql composition with the typed builder', () => {
  it('aliased single-column select emits a RawExpr in the projection list', () => {
    ...
    db.users
      .select('greeting', (_f, fns) => {
        const expr = fns.rawSql`'hello'`.returns('sqlite/text@1');
    ...
    expect(raw.returns.codecId).toBe('sqlite/text@1');
  });
  ...
  it('aliased single-column select with field interpolation produces the correct RawExpr parts', () => {
    ...
    const nameExpr = {
      ...
      returnType: { codecId: 'sqlite/text@1', nullable: false },
    };
    const expr = rawSql`'hello ' || ${nameExpr}`.returns('sqlite/text@1');
  ...
  it('bulk-object select AST contains a ColumnRef entry and a RawExpr entry', () => {
    ...
    const rawExpr = rawSql`coalesce(score, 0)`.returns('sqlite/integer@1');
  ...
  it('where with fns.gt(rawSql, literal) produces a BinaryExpr whose left operand is a RawExpr', () => {
    ...
    const epochExpr = rawSql`strftime('%s', ${createdAtExpr})`.returns('sqlite/integer@1');
    ...
    // The left operand must be the RawExpr produced by fns.rawSql`strftime(...)`.
    expect(binary.left).toBeInstanceOf(RawExpr);
    const leftRaw = binary.left as RawExpr;
    expect(leftRaw.parts[0]).toBe("strftime('%s', ");
  ...
  it('fns.count(rawSql) produces an AggregateExpr whose argument is a RawExpr', () => {
    ...
    const scoreExpr = {
      ...
      returnType: { codecId: 'sqlite/integer@1', nullable: true },
    };

    const coalesced = rawSql`coalesce(${scoreExpr}, 0)`.returns('sqlite/integer@1');
  ...
  it('nested rawSql: outer parts array contains inner RawExpr as an expression element', () => {
    ...
    const inner = rawSql`coalesce(${scoreExpr}, 0)`.returns('sqlite/integer@1');
    const outer = rawSql`json_object('val', ${inner})`.returns('sqlite/text@1');

    const outerAst = outer.buildAst();
    expect(outerAst).toBeInstanceOf(RawExpr);

    const outerRaw = outerAst as RawExpr;
    // Template literal `json_object('val', ${inner})` produces:
    // [ "json_object('val', ", innerRawExpr, ")" ]
    expect(outerRaw.parts).toHaveLength(3);
    expect(outerRaw.parts[0]).toBe("json_object('val', ");
