 ...
 *    routing key, and issues exactly one `sdk.bulkEncrypt(...)` call per
 *    group (AC-MW1). Per Project 1's standing decision (Decision 2),
 ...
 *    `setHandleCiphertext` (AC-MW3) and writes the envelope back through
 *    `params.replaceValues` so the runtime's `currentParams()` view
 *    reflects the post-mutation slot. The handle's `plaintext` slot is
 *    **retained** (AC-MW5) — `envelope.decrypt()` continues to return
 *    the plaintext synchronously without consulting the SDK.
 *
 * Cancellation: `ctx.signal` is forwarded by identity to every
 * `bulkEncrypt` call (AC-MW4); the SDK is responsible for honoring it.
 */
