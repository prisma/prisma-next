...
describe('routingKeyId — stable string identity per (table, column) pair', () => {
  ...
  it('produces distinct ids for different table or column', () => {
  ...
  it('does not collide on names that share a literal concatenation', () => {
    // `userXemail` concatenated would collide with `user` + `email` only if
    // the separator is not a sentinel. Use a pair of names where the
    // collision would happen under naive concatenation.
