...
- **Internal test substrate** (not published): `test/fixtures/supabase-reference/restore.ts` exports `restoreSupabaseReference(client)`, restoring the reference fixture (all Supabase schemas, tables, roles, and the platform's real default privileges) into a test database. Used by this package's tests and by `examples/supabase` via relative imports. User-facing RLS testing is Supabase's own tooling (`supabase start` + `supabase test db`).
