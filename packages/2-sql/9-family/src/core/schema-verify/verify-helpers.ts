 ...
 *
 * The referenced-namespace dimension (FR16b) is always consulted —
 * both sides carry a required `namespaceId` / `referencedNamespaceId`
 * coordinate (the introspector stamps the resolved schema, the
 * contract carries the resolved target coordinate), so the comparison
 * is strict equality at both ends of the FK.
 *
 * Name differences are ignored by default (names are for DDL/diagnostics, not identity).
 */
  ...
  for (const contractFK of contractFKs) {
    ...
    const matchingFK = schemaFKs.find((fk) => {
      return (
        ...
        fk.referencedNamespaceId === contractFK.target.namespaceId
      );
    });
    ...
    if (!matchingFK) {
      issues.push({
        ...
        table: tableName,
        expected: `${contractFK.source.columns.join(', ')} -> ${contractFK.target.table}(${contractFK.target.columns.join(', ')})`,
        message: `Table "${tableName}" is missing foreign key: ${contractFK.source.columns.join(', ')} -> ${contractFK.target.table}(${contractFK.target.columns.join(', ')})`,
      });
      nodes.push({
        status: 'fail',
        kind: 'foreignKey',
      ...
      if (actionMismatches.length > 0) {
        ...
        issues.push({
          ...
          message: `Table "${tableName}" foreign key ${contractFK.source.columns.join(', ')} -> ${contractFK.target.table}: ${combinedMessage}`,
        });
        nodes.push({
          status: 'fail',
          kind: 'foreignKey',
    ...
    for (const schemaFK of schemaFKs) {
      const matchingFK = contractFKs.find((fk) => {
        return (
          ...
          fk.target.namespaceId === schemaFK.referencedNamespaceId
        );
      });
