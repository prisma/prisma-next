...
- **`default`** (optional): Uses the shared `ColumnDefault` type from `@prisma-next/contract` for db-agnostic defaults (literal expression or db-generated expression); target-specific defaults live in target packages
