/**
 * SQL-family codec-descriptor hook: a per-instance native type for a parameterized codec (e.g. a
 * native enum's Postgres type name from its `typeParams`).
 *
 * `StorageColumn.nativeType` already lives in this package — it is a SQL-layer concept. The
 * framework `CodecDescriptor`/`CodecLookup` stay family-agnostic; this hook and its wiring live
 * here instead.
 */
