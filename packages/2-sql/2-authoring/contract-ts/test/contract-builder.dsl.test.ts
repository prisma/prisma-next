...
describe('contract DSL authoring surface', () => {
  it('lowers inline ids and uniques while keeping sql focused on table/index/fk concerns', () => {
    ...
    const storageTables = contract.storage.tables['__unbound__'] as Record<
      string,
      {
    ...
    expect(storageTables['blog_post']?.foreignKeys).toMatchObject([
      {
        source: { columns: ['user_id'] },
        target: { namespaceId: '__unbound__', table: 'app_user', columns: ['id'] },
        name: 'blog_post_user_id_fkey',
        onDelete: 'cascade',
        constraint: true,
        index: false,
  ...
  it('keeps field and belongsTo storage overrides local when possible', () => {
    ...
    const tables = contract.storage.tables['__unbound__'] as Record<
      string,
      {
        primaryKey?: unknown;
        uniques?: unknown;
    ...
    expect(tables['blog_post']?.foreignKeys).toMatchObject([
      {
  ...
  it('supports compound ids and uniques in .attributes(...)', () => {
    ...
    expect(
      (contract.storage.tables['__unbound__'] as Record<string, unknown>)['membership'],
    ).toMatchObject({
      primaryKey: {
