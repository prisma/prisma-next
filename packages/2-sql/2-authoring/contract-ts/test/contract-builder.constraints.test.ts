...
describe('contract definition constraint support', () => {
  it('emits unique constraints in the contract', () => {
    ...
    expect(findTableByName(contract.storage, 'user')!.uniques).toHaveLength(1);
    expect(findTableByName(contract.storage, 'user')!.uniques[0]).toEqual({ columns: ['email'] });
  });
  ...
  it('emits unique constraints with names in the contract', () => {
    ...
    expect(findTableByName(contract.storage, 'user')!.uniques).toHaveLength(1);
    expect(findTableByName(contract.storage, 'user')!.uniques[0]).toEqual({
      columns: ['email'],
  ...
  it('emits indexes in the contract', () => {
    ...
    expect(findTableByName(contract.storage, 'user')!.indexes).toHaveLength(1);
    expect(findTableByName(contract.storage, 'user')!.indexes[0]).toEqual({ columns: ['email'] });
  });
  ...
  it('emits indexes with names in the contract', () => {
    ...
    expect(findTableByName(contract.storage, 'user')!.indexes).toHaveLength(1);
    expect(findTableByName(contract.storage, 'user')!.indexes[0]).toEqual({
      columns: ['email'],
      name: 'user_email_idx',
    });
  });
  ...
  it('emits foreign keys in the contract', () => {
    ...
    expect(findTableByName(contract.storage, 'post')!.foreignKeys).toHaveLength(1);
    expect(findTableByName(contract.storage, 'post')!.foreignKeys[0]).toMatchObject({
  ...
  it('emits primary key without name when not provided', () => {
    ...
    expect(findTableByName(contract.storage, 'user')!.primaryKey).toEqual({
      columns: ['id'],
    });
    expect(findTableByName(contract.storage, 'user')!.primaryKey).not.toHaveProperty('name');
  });
  ...
  it('supports multiple constraints on the same table', () => {
    ...
    expect(findTableByName(contract.storage, 'user')!.uniques).toHaveLength(2);
    expect(findTableByName(contract.storage, 'user')!.indexes).toHaveLength(2);
  });
  ...
  it('supports compound ids and uniques through attributes', () => {
    ...
    expect(findTableByName(contract.storage, 'membership')!.primaryKey).toEqual({
      columns: ['orgId', 'userId'],
      name: 'membership_pkey',
    });
    expect(findTableByName(contract.storage, 'membership')!.uniques).toEqual([
      {
        columns: ['orgId', 'role'],
        name: 'membership_org_role_key',
      },
