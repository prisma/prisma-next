...
function expectDiagnosticForSchema(
  ...
  const document = parseAndResolve({ schema, sourceId: 'schema.prisma' });
    ...
    expect(result.failure.diagnostics).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          message: 'Named type "Broken" references unsupported base type ""',
        }),
      ]),
    );
  });
    ...
    expect(result.failure.diagnostics).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          sourceId: 'schema.prisma',
          span: expect.objectContaining({
            start: expect.objectContaining({ line: 0, column: 0 }),
...
`,
  ...
  it('emits distinct diagnostic codes for malformed versus uncomposed constructor calls', () => {
    const malformed = parseAndResolve({
      schema: `model User {
  id Int @id
  name sql.String(
}
`,
    ...
    const uncomposed = parseAndResolve({
      schema: `model User {
  id        Int @id
