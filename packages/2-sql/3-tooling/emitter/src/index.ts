...
export const sqlTargetFamilyHook = {
  ...
  generateContractTypes(
    ...
    const enumTypeAliases = this.generateEnumTypeAliases(storage.enums);
    const enumSection = enumTypeAliases ? `\n${enumTypeAliases}\n` : '';
...
`;
  ...
  generateStorageType(storage: SqlStorage): string {
    ...
    const enumsType = this.generateEnumsType(storage.enums);
    return `{ readonly tables: { ${tables.join('; ')} }; readonly enums: ${enumsType} }`;
  },

  generateEnumsType(enums: Record<string, StorageEnum> | undefined): string {
    ...
    for (const enumName of sortedEnumNames) {
      ...
      enumTypes.push(`readonly ${enumName}: { readonly values: readonly [${values}] }`);
    }

    return `{ ${enumTypes.join('; ')} }`;
  },

  generateEnumTypeAliases(enums: Record<string, StorageEnum> | undefined): string {
    if (!enums || Object.keys(enums).length === 0) {
      return '';
    }
