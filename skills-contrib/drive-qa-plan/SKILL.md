...
"Manual" describes *what is being run* (the live system end-to-end, against real artefacts) and *what kind of inference* the runner brings (judgement about diagnostic copy, file shape, journey feel — not just structural assertions). It does **not** mean "human-driven". The runner is autonomous and parallelises wherever the script's isolation tags say it can.
...
- **Manual QA's comparative advantage over CI is judgement, not throughput.** CI checks exit codes and structural shapes. The runner judges diagnostic clarity, output legibility, whether a generated file *looks right*, whether the developer journey *feels right* — and it does so against an oracle the script names explicitly. Scenarios should call out what the runner is *looking at*, not just what it's *running*.
...
7. **What you should see** — observables, including what the runner is *looking at*. Call out diagnostic copy, file shape, output legibility — anything tests can't easily assert.
8. **Failure modes** — explicit failure categories that constitute a finding. **Do not pre-classify severity** — severity is a property of *what was found* in a runtime context, and it belongs in the runner's report.
