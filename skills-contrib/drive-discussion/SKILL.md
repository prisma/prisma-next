...
> **Emit `falsified-assumption`** (**I12-gated, falsified-assumption-only** — emit on entry when discussion was routed via trigger 3 (mid-flight, falsified assumption). Triggers 1 (pre-spec), 2 (mid-spec), 4 (mid-flight, obstacle), and 5 (explicit operator request) do **not** emit; obstacle-triggered halts are a distinct signal that slice 1 does not capture (revisit in slice 2 if signal demands). Payload: `artifact_path` (the spec/plan path carrying the falsified assumption), `triggered_by` (`"implementer-pushback"` / `"wip-inspection"` / `"dispatch-blocked"` / `"health-check-drift"` / `"orchestrator-self-detected"` / `"operator-flagged"` — pick from the halt context), `assumption_summary` (one-sentence; `null` when not summarised at entry), plus envelope fields. See [`events.md` § `falsified-assumption`](../drive-record-traces/events.md#falsified-assumption) and [`emission.md` § Append protocol](../drive-record-traces/emission.md#append-protocol).
