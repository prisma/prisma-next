...
- **In unattended mode, keep the branch local.** The interactive-mode default of auto-pushing and opening the PR at slice DoD met (per Behavioral rules § "On slice DoD met, auto-push and open the PR") does **not** apply when no operator is on-hand. Hold the branch locally and surface the readiness to open the PR in the return-to-interactive handoff. The operator can authorise PR-open as part of the unattended scope when handing off; in that case, proceed as in interactive mode.
...
- **On slice DoD met, auto-push and open the PR.** Once the reviewer reports `SATISFIED` across all dispatches in the slice, the orchestrator does **not** halt to ask the operator for permission to push or open the PR. It runs the team's PR-opening sequence (`git push -u origin HEAD`, then `gh pr create` with the body generated by `drive-pr-description`) and reports the PR URL back. The operator's intent has already been expressed through the spec, plan, and per-round review-triage; a consent gate at PR-open replays the question for no informational gain. Skip auto-open only if the operator declared a manual-PR scope when invoking the workflow (e.g. *"land everything locally but don't push"*) or when in unattended mode without an explicit PR-opening grant.
...
| this skill                     | team's PR-opening skill           | All dispatches SATISFIED + branch review-clean → orchestrator auto-pushes and opens the PR with the generated body. Skip only if the operator pre-declared a manual-PR scope (e.g. unattended mode opting out). |
