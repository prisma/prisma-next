...
- **Breaking change to a downstream surface without an upgrade record.** If the diff edits `examples/**/src` or `packages/3-extensions/**/src` in a way that required a consumer-side code change (the substrate was edited to typecheck/pass against a framework change — renamed/retyped public API, changed SPI shape, moved on-disk artefact), confirm the PR also carries a matching `record-upgrade-instructions` entry (`skills/upgrade/prisma-next-upgrade/upgrades/<in-flight>/` for `examples/`, `skills/extension-author/prisma-next-extension-upgrade/upgrades/<in-flight>/` for `3-extensions/`). The `check:upgrade-coverage` CI gate is **directory-existence only** — it passes whenever the transition directory exists, even if *this* PR added no entry ([TML-2738](https://linear.app/prisma-company/issue/TML-2738)) — so the reviewer is the real backstop here. Flag a downstream-source breaking change that ships with no entry. Do **not** demand a no-op `changes: []` placeholder for test-only or regenerated-fixture churn that needs no consumer action — only genuine consumer migrations need an entry.
