...
- **Phase identifier:** `<phase-id>` (e.g. `phase-3`)
- **Tasks in scope:** `<T-numbers>` (e.g. `T3.1`-`T3.7`)

Re-read the named phase in `plan.md`. Treat its task list as your contract; treat the validation gate as your bar.
...
The plan documents the phase's validation gates explicitly. Restated here so you don't have to navigate:
...
- **Commit organization:** <one suggested split, e.g. "one commit per task; or, one commit for T3.1-T3.5 as a unit and one commit for T3.7"> — use your judgment if a different split reads cleaner; surface the choice in your report.
...
6. **Anything surprising** — pre-existing issues uncovered, infrastructure gaps, escapees from prior phases.
...
> Use this trimmed shape on rounds where the implementer subagent is being **resumed** via the `Task` tool's `resume` parameter (every round after the first by default). The subagent retains its prior transcript; you don't need to re-state the persona, spec/plan locations, phase identifier, or task scope unless they have changed.
...
- <e.g. "the user accepted F<N>'s deferral; T3.5 is now scoped down to ..."; "plan.md gained T3.8 — see commit <sha>">
