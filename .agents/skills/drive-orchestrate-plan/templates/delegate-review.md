...
>
> **Fresh mode**: the subagent has no transcript yet. The prompt must be self-contained — inline persona pointer, spec/plan locations, phase scope, the implementer's full structured report, and the items to triage.
>
> **Resume mode**: the subagent retains its full prior transcript via the `Task` tool's `resume` parameter, including the AC scoreboard it has been maintaining and every finding it has filed. The prompt is a follow-up message — skip persona/spec/plan re-pointers and lean lighter on context-restating. Keep the round identifier, the new commits since the last round, the implementer's report for this round, items to triage, and any orchestrator-specific concerns.
>
...
- **Phase identifier under review:** `<phase-id>`
- **Round number:** `<R1 / R2 / ...>`
...
Use the checklist in `<skill-dir>/agents/reviewer.md § The acceptance bar for SATISFIED`. Do not relax the bar. The phase is SATISFIED only when:

- Every phase-owned AC is PASS, verified against on-disk code.
- No FAIL or unaccepted-deferral on any AC.
- All phase validation gates pass (verify by running them, or trust the implementer's report if you have no reason to distrust it).
- All flagged items triaged.
- The findings log is empty of opens. All severities (`must-fix`, `should-fix`, `low / process`) block phase close — severity is for within-round prioritization, not for carrying forward.
...
- The action is addressable in this PR (current phase or an explicitly named later phase of this PR).
...
- **R1** of a phase: write substantively if the phase establishes design; write a "Round 1 — design stable" note if the phase only implements an existing design. The walkthrough always reflects HEAD's diff against the project base.
- **R2+** of a phase: append a "Round N delta" section to each, or rewrite affected sections if substance shifted.
- The walkthrough must follow the project's `/walkthrough` rule (`~/.cursor/rules/walkthrough.mdc` or the project's `.claude/skills/drive-pr-walkthrough/SKILL.md`).
...
> Use this trimmed shape on rounds where the reviewer subagent is being **resumed** via the `Task` tool's `resume` parameter (every round after the first by default). The subagent retains its prior transcript including the AC scoreboard, every F-numbered finding, and every prior verdict; you don't need to re-state the persona, spec/plan, phase identifier, or scoreboard format.

```markdown
## Resume — `<project-name>`, `<phase-id>` `<round-id>` (e.g. m3 R2)
...
- <e.g. "the orchestrator promoted F3 from `low / process` to `should-fix` after consulting the user — see ## Orchestrator notes block in code-review.md"; "plan.md gained T3.8 — review the new task as part of phase scope">
