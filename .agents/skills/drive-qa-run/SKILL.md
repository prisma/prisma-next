...
**Hard-error contract.** If `drive/qa/README.md` does not exist, do not proceed. Tell the operator the file is missing and offer to invoke `drive-bootstrap-context` inline (or ask them to run it). The README captures project-specific facts the generic skill cannot know — consumer audiences, substrate locations, known coverage-gate gaps, fixture catalogue — and the severity rubric / artefact-capture conventions in this skill depend on them.

If the README exists but a project-specific fact you need is missing (e.g. the script references a fixture not in the catalogue), surface it now. When the run completes, append the new fact to `drive/qa/README.md` (see the Checklist) so the next run inherits it.

### 2. Read the whole script before running anything
...
### 3. Set up the environment per the script's pre-flight
...
### 4. Execute scenarios in script order, one at a time
...
### 5. Capture findings as they happen, not at the end
...
### 6. After the last scenario, assemble the report
...
12. **Skipping the project-context read** or **not writing back surfaced facts**. Symptom: the run classifies severities, captures artefacts, or hunts for fixtures without a consistent view of the project's audiences / substrates / catalogue, OR a fact the operator had to feed mid-run never makes it back to `drive/qa/README.md`. The next runner repeats the same correction loop. Fix: step 1 is non-negotiable — if the file is missing, stop and bootstrap. At the end of the run, audit for facts that surfaced during execution; append any to `drive/qa/README.md` so the next run inherits them.
...
- [ ] Read `drive/qa/README.md` before starting; if missing, ran `drive-bootstrap-context` and filled in audiences / substrates / fixture catalogue before executing scenarios.
...
- [ ] At the end of the run, audited for project-specific facts that surfaced during execution; appended any to `drive/qa/README.md` so the next run inherits them.
