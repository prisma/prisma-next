 ...
 *
 * The shape matches `ColumnTypeDescriptor` from
 * `@prisma-next/framework-components/codec` but is defined locally to
 * keep test-utils dependency-free (`framework-components` transitively
 * depends on packages that devDepend on test-utils, so a runtime dep
 * here would create a turbo build cycle).
 */
