 ...
 * Postgres. Exercises the **April stop condition** for TML-2143 / VP4:
 * a repeated query is served from cache without hitting the database.
 *
 * Tests in this file:
 * - 3.16 Stop condition — second annotated query skips the driver.
 * - 3.17 Composition with a `beforeCompile`-style rewriter (soft-delete).
 * - 3.18 Composition with telemetry — `source` field round-trips.
 * - 3.19 Concurrency regression — two parallel calls of the same plan
 *        don't cross-talk through the per-exec WeakMap buffer.
 */
...
function activeUsersOnly(): SqlMiddleware {
  return {
    ...
    async beforeCompile(draft) {
      ...
      const invitedByPresent = BinaryExpr.gte(
        ColumnRef.of('users', 'id'),
        ParamRef.of(2, { name: 'soft_delete_min_id', codecId: 'pg/int4@1' }),
      );
...
describe(
  'integration: middleware-cache against real Postgres',
  ...
  () => {
    ...
    describe('stop condition (M3.16)', () => {
      ...
      it('does not cache a query when its cacheAnnotation has skip: true', async () => {
        ...
        const buildPlan = () =>
          db.users
            .select('id')
            .annotate(cacheAnnotation.apply({ ttl: 60_000, skip: true }))
            .build();
    ...
    describe('composition with beforeCompile rewriter (M3.17)', () => {
    ...
    describe('composition with telemetry (M3.18)', () => {
      it('telemetry observes source: "driver" on miss and source: "middleware" on hit', async () => {
        const events: TelemetryEvent[] = [];
        const telemetry = createTelemetryMiddleware({ onEvent: (e) => events.push(e) });
        const cache = createCacheMiddleware({ maxEntries: 100 });
        // Cache first so its `intercept` runs upstream of telemetry's
        // `beforeExecute`. Telemetry's `afterExecute` still fires on
        // both paths and observes the `source` field.
        const runtime = buildRuntime([cache, telemetry]);
        const db = sql({ context });
        ...
        // A `beforeExecute` event fires only when the cache misses
        // (cache's intercept returned undefined → driver path runs).
        ...
        // beforeExecute is suppressed on the intercepted hit path.
        expect(events.find((e) => e.phase === 'beforeExecute')).toBeUndefined();
      ...
      it('telemetry rowCount and latencyMs populate correctly on both paths', async () => {
        const events: TelemetryEvent[] = [];
        const telemetry = createTelemetryMiddleware({ onEvent: (e) => events.push(e) });
        const cache = createCacheMiddleware({ maxEntries: 100 });
        const runtime = buildRuntime([cache, telemetry]);
    ...
    describe('concurrency regression (M3.19)', () => {
      it('two parallel executes of the same plan do not cross-talk via the per-exec buffer', async () => {
        ...
        const buildPlan = () =>
          db.users
            .select('id', 'name')
            .annotate(cacheAnnotation.apply({ ttl: 60_000, key: 'concurrency-test' }))
            .build();
      ...
      it('parallel executes of two different plans land in distinct cache slots', async () => {
        ...
        const planA = db.users
          .select('id')
          .annotate(cacheAnnotation.apply({ ttl: 60_000, key: 'parallel-A' }))
          .build();
        const planB = db.posts
          .select('id', 'title')
          .annotate(cacheAnnotation.apply({ ttl: 60_000, key: 'parallel-B' }))
          .build();
