  ...
  describe('Journey: Data Transform Authoring', () => {
    ...
    it(
      ...
      async () => {
        ...
        // Step 5: Fill in migration.ts with descriptors
        const migrationTs = `
import { addColumn, dataTransform } from "@prisma-next/target-postgres/migration-builders"

export default () => [
  addColumn("user", "name"),
  dataTransform("backfill-user-name", {
    check: { kind: "raw_sql", sql: "SELECT 1 FROM \\"public\\".\\"user\\" WHERE \\"name\\" IS NULL LIMIT 1" },
    run: { kind: "raw_sql", sql: "UPDATE \\"public\\".\\"user\\" SET \\"name\\" = split_part(email, '@', 1)" },
  }),
]
`;
