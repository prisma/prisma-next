 ...
 *
 * `pgvector` exposes the real package descriptor. After pgvector
 * migrated off `databaseDependencies.init` to the contract-space
 * mechanism (see TML-2397), the descriptor no longer participates in
 * the legacy `dependency_missing` schemaVerify path. The
 ...
 * declared dependencies. When the `databaseDependencies` mechanism is
 * removed entirely, this synthetic + its consumers go away with it.
 */
