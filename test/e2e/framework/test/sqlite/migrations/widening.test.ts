...
// ---------------------------------------------------------------------------
// Widening operations (fanned out)
// ---------------------------------------------------------------------------
    ...
    it('changes a column default', async () => {
      await runMigration(
        ...
        async ({ driver }) => {
          ...
          expect(
            (
              await driver.query<{ status: string }>(
                'SELECT status FROM "Setting" WHERE id = ?',
                [1],
              )
            ).rows[0]!.status,
          ).toBe('active');
    ...
    it('round-trips a string default with an apostrophe', async () => {
      await runMigration(
        ...
        async ({ schema, driver }) => {
          ...
          const row = (
            await driver.query<{ nickname: string }>(
...
// ---------------------------------------------------------------------------
// SQLite-specific (now() canonicalization through datetime() spelling)
// ---------------------------------------------------------------------------

describe('Migration E2E - SQLite-specific widening', () => {
  ...
  // must be one of the SQLite-native spellings.
