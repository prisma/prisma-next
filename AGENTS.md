...
- **Drive (`drive-*`) skills and methodology live upstream in ignite, not here:** the Drive methodology skills are maintained in [`prisma/ignite`](https://github.com/prisma/ignite) under `skills/.pilot/`, and the canonical methodology docs live alongside at [`docs/drive/`](https://github.com/prisma/ignite/tree/main/docs/drive). Install the skills into your user-level skills dir with `npx skills add prisma/ignite/skills/.pilot --skill '*'`. Only the `drive/` project context (this team's calibration overlays) lives in this repo.
