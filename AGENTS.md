...
- **Drive (`drive-*`) skills live upstream in ignite, not here:** the Drive methodology skills are maintained in [`prisma/ignite`](https://github.com/prisma/ignite) under `skills/.pilot/` and are no longer carried in `skills-contrib/`. Install them into your user-level skills dir with `npx skills add prisma/ignite/skills/.pilot --skill '*'`. The `drive/` project context and `docs/drive/` methodology docs still live in this repo.
