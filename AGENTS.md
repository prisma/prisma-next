...
Canonical homes (only tracked copies — **edit here**):

- **Skills:** `skills-contrib/<skill>/SKILL.md`
- **Rules:** `.agents/rules/<rule>.mdc` (must be `.mdc`; `.md` files are never loaded)

Harness-specific trees (`.claude/skills/`, `.agents/skills/`, `.cursor/rules/`, `.claude/rules/`) are gitignored symlink mirrors, regenerated by the `prepare` script on `pnpm install`. Editing through a symlink writes to the canonical file but garbles `git status` — always address the canonical path. CI enforces consistency via `pnpm lint:skills` and `pnpm lint:rules:symlinks`.

**Drive (`drive-*`) skills live upstream** in [`prisma/ignite`](https://github.com/prisma/ignite) under `skills/.pilot/`; methodology at [`ignite docs/drive/`](https://github.com/prisma/ignite/tree/main/docs/drive). Install with `npx skills add prisma/ignite/skills/.pilot --skill '*'`. Only this team's `drive/` calibration overlays live here.
