...
describe('ORM integration tests', () => {
  ...
  it('orm pagination: ormGetUsersByIdCursor returns paginated users with gt cursor', async () => {
    await withTempSqliteDatabase(async ({ connectionString }) => {
      await initTestDatabase({ connection: connectionString, contractIR: contract });
      const runtime = getRuntime(connectionString);
  ...
  it('orm pagination: ormGetUsersBackward returns users before cursor with lt operator', async () => {
    await withTempSqliteDatabase(async ({ connectionString }) => {
      await initTestDatabase({ connection: connectionString, contractIR: contract });
      const runtime = getRuntime(connectionString);
