...
3. If the schema changed: `pnpm exec prisma-next migration plan --name <slug>`, then hand-edit the generated `migrations/feature-flags/<dir>/migration.ts` so each op carries the package's stable invariantId, and re-emit `ops.json` from this directory with:
