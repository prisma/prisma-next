...
The cipherstash extension's runtime registers `cipherstashEq` / `cipherstashIlike` on cipherstash columns via `context.queryOperations`, but its emitter export does not yet ship a static `QueryOperationTypes` surface analogous to [`@prisma-next/extension-pgvector/operation-types`](../../packages/3-extensions/pgvector/src/exports/). Tracked under [TML-2435](https://linear.app/prisma-company/issue/TML-2435/cipherstash-ship-queryoperationtypes-export-so-cipherstasheq). Until it lands, `src/index.ts` casts the column accessor through a small local `CipherstashColumnSurface` shape so the `where(...)` callbacks typecheck. The runtime call works regardless — the cast only bridges compile-time visibility.
