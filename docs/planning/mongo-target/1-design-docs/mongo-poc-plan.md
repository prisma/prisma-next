...
- **Discriminator values are untyped strings**: The contract stores discriminator values as strings, but something must convert them to native DB values. Affects column defaults too. See [ADR 2 open questions](../adrs/ADR%202%20-%20Polymorphism%20via%20discriminator%20and%20variants.md).
