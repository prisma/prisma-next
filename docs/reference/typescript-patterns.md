...
- **Polymorphic dispatch today** (verifiers / walkers dispatch on `kind`): each leaf class declares an enumerable literal `kind = '<family>-<leaf>' as const`. The leaf literal dominates union narrowing; framework consumers and target consumers both narrow through it. Reference: `PostgresEnumType.kind = 'postgres-enum' as const`.
