## Skill: set up Prisma Next in a TS app (contract → context → runtime)

### When to use

Use this when you need to:

- add Prisma Next to an app/service
- wire up a working runtime against a real database
- add an extension pack (e.g. pgvector) and avoid “missing pack” failures
...
- **Dev**: `mode: 'onFirstUse'`, often `requireMarker: false` (lets you iterate before stamping markers).
- **Prod**: prefer `mode: 'startup'` and `requireMarker: true` (fail fast before serving traffic).

### Checklist for agents

- [ ] `validateContract<Contract>(contractJson)` uses the `.d.ts` type, not an inferred JSON type
- [ ] `createExecutionStack` includes the same packs your contract declares
- [ ] `createExecutionContext` is built from **descriptors** and is safe to import broadly
- [ ] `createRuntime` uses an instantiated stack + driver and has explicit `verify` policy
