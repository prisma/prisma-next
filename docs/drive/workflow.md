...
| Retro (trigger-based) | Cross-cutting | Sprint retro / blameless post-mortem | `drive-run-retro` | Trigger-based: post-mortem, dispatch failure, slice close (if learning), project close (mandatory) | Agile orchestrator | Canonical-skill update OR project-context update OR ADR (if neither, retro failed) | — |
