...
`package.json` has `"prepare": "husky && node scripts/sync-rules.mjs --quiet"`, which runs on every `pnpm install` and:

- Installs the git hooks under `.husky/`. The `pre-commit` hook runs `pnpm lint-staged`, which runs `biome format --write`, `biome check --write`, and `lint-deps-focused.mjs` against staged files. Cloud-agent commits are therefore auto-formatted and dependency-cruiser-checked the same way human commits are.
- Mirrors the canonical rule cards from `.agents/rules/` into `.cursor/rules/` (see [Rule sync](#rule-sync)).

You don't need to disable either. They're features: they keep cloud-agent commits consistent with human commits, and they ensure agents see the same rule set on every fresh checkout.

## Rule sync

Cursor only discovers project rules under `.cursor/rules/` ([Cursor docs](https://cursor.com/help/customization/rules)), which is gitignored in this repo. The committed source of truth for vendor-neutral rule cards is `.agents/rules/`. To bridge the two, `pnpm install` runs `node scripts/sync-rules.mjs --quiet`, which symlinks every `.agents/rules/*.mdc` into `.cursor/rules/`.

Day-to-day commands:

| Command | Purpose |
|---|---|
| `pnpm sync:rules` | Manually re-run the symlink mirror (idempotent). Useful if you added a new rule under `.agents/rules/` and don't want to re-run `pnpm install`. |
| `pnpm lint:rules:sync` | CI-style check: exits non-zero if `.cursor/rules/` is missing any canonical mirror. |
| `pnpm lint:rules` | Validates frontmatter on the resolved `.cursor/rules/` set (existing). |

Authoring guidance:

- Place team-shared, vendor-neutral rule cards in `.agents/rules/` (committed). They mirror into `.cursor/rules/` automatically.
- Cursor-only rules (e.g. `read-agents-md.mdc`, `no-heredoc-commit-messages.mdc`) live directly in `.cursor/rules/` and stay local. The sync script ignores files in `.cursor/rules/` that don't have a `.agents/rules/` source.
- If a target in `.cursor/rules/` exists as a regular file (not a symlink), `sync:rules` leaves it alone and warns. Resolve manually: delete the local copy to adopt the canonical version, or move it into `.agents/rules/` to share it.

This keeps the cross-runtime story working: Claude Code reads `CLAUDE.md` (also a symlink to `AGENTS.md`); Cursor reads `.cursor/rules/`; both ultimately point at the same content under `.agents/`.
...
| `.cursor/rules/`, `.cursor/commands/` | Cursor desktop | Local-only (gitignored). `.cursor/rules/` is auto-populated from `.agents/rules/` by `pnpm sync:rules` (see [Rule sync](#rule-sync)). |
...
| `.agents/skills/`, `.agents/rules/` | Vendor-neutral skills/rules | Committed. Source of truth for rule cards mirrored into `.cursor/rules/`. |
...
- **Linting & layering**: prefer `pnpm lint`, `pnpm lint:deps`, `pnpm lint:rules`, `pnpm lint:rules:sync`, `pnpm lint:docs`. Treat layering violations as errors to fix, never to bypass — see the layering rule under [`AGENTS.md` § Boundaries & Safety Rails](../../AGENTS.md#boundaries--safety-rails) (committed; the optional `.cursor/rules/import-validation.mdc` is a local-only mirror).
