...
| Optional Dockerfile | `.cursor/Dockerfile` (committed when present) | Only when you opt out of agent-driven snapshots |
| Optional hooks | `.cursor/hooks.json` (committed when present) | Same hook system as desktop Cursor |
...
`.gitignore` ignores most of `.cursor/` (rules, commands, etc. are local-only) but explicitly un-ignores `environment.json`, `Dockerfile`, and `hooks.json` so cloud-agent config is committed.
...
- The [`.cursor/rules/no-direct-lockfile-edits.mdc`](../../.cursor/rules/no-direct-lockfile-edits.mdc) rule applies in cloud the same as locally — never hand-edit the lockfile, always go through pnpm.
