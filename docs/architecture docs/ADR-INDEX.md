...
| 239 | Contract snapshots live in a content-addressed store | Every distinct migration contract is stored once per migrations root at `migrations/snapshots/<hex>/contract.{json,d.ts}`, keyed by storage hash; the old per-package sibling contract files and per-space head copies are gone, and `migration.ts` imports resolve through the store. Amends ADR 197 and ADR 232. | [ADR 239 - Contract snapshots live in a content-addressed store.md](adrs/ADR%20239%20-%20Contract%20snapshots%20live%20in%20a%20content-addressed%20store.md) |
