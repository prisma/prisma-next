...
| 220 | Plan execution identity for middleware correlation | Adds `meta.planExecutionId` to `PlanMeta`, assigned by the runtime via `crypto.randomUUID()` at the entry of every `execute()` call. Per-execute identity (not per-plan), shared across all hooks within one call, distinct across two calls of the same plan. Excluded from content hashing and fingerprinting. Distinct from ADR 013's content-based `planId` and ADR 160's `groupingKey`. | [ADR 220 - Plan execution identity for middleware correlation.md](adrs/ADR%20220%20-%20Plan%20execution%20identity%20for%20middleware%20correlation.md) |
