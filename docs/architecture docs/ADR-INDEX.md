...
| 163 | Postgres JSON and JSONB typed columns | Adds first-class PostgreSQL `json`/`jsonb` codec and column support with Standard Schema-based typed emission in `contract.d.ts` | [ADR 163 - Postgres JSON and JSONB typed columns.md](adrs/ADR%20163%20-%20Postgres%20JSON%20and%20JSONB%20typed%20columns.md) |
