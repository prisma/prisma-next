...
| 186 | Codec-dispatched type rendering | Codecs own TypeScript type rendering via `renderTypeParams` and `parameterizedOutput`; removes `EmissionSpi.generateModelsType?` override and legacy renderer infrastructure | [ADR 186 - Codec-dispatched type rendering.md](adrs/ADR%20186%20-%20Codec-dispatched%20type%20rendering.md) |
