...
Context
	•	We need consistent, low-overhead telemetry to understand performance, safety, and usage across lanes and adapters
	•	Telemetry must be privacy-preserving by default and configurable to meet stricter org policies
	•	Agents and PPg benefit from structured signals like sqlFingerprint, budgets, and lint outcomes without exposing SQL text or PII
