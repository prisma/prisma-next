...
import { createIndex } from '@prisma-next/mongo-migration-ops';

class BackfillStatus extends Migration {
  override readonly targetId = 'mongo';
...
A `migration.ts` that is run via shebang yields the same artifacts the CLI would produce. There is no draft state, no `migrationId: null` left for someone else to fill in, and no two-step "emit then verify" handshake. The file is a self-contained authoring surface; running it produces the contract ([ADR 192](ADR%20192%20-%20ops.json%20is%20the%20migration%20contract.md)).
