...
Outputs
	•	A fully-populated RuntimeError per ADR 027
code, message, severity, retryable, planId, sqlFingerprint, coreHash, profileHash, details

Mapping model

1) Normalization pipeline
	1.	Capture the raw failure plus context
{ source, vendor, plan, sql, params, adapterMeta, timings }
	2.	Classify with ordered matchers
db → driver → adapter → compileProfile → lane
