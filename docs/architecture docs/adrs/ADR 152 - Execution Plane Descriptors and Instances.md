...
export interface RuntimeFamilyInstance<TFamilyId extends string = string> {
  ...
  // Family-specific methods such as createRuntime, executePlan, etc.
}
...
4. ✅ Updated runtime assembly code to use `RuntimeFamilyDescriptor.create({ target, adapter, driver, extensions })` pattern
5. ✅ Updated examples and test utilities to use runtime descriptors
