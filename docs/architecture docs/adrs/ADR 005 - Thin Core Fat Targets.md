...
Alternatives considered
	•	One grand unified abstraction that hides SQL vs document differences
	•	Simplifies APIs but collapses useful power and becomes the lowest common denominator
	•	Keep everything in one package with if (dialect) branches
	•	Faster to start but creates tight coupling and slows evolution
	•	Push everything into adapters including plan and runtime semantics
	•	Maximally flexible but loses a coherent platform and safety guarantees
