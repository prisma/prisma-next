...
Preflight behavior matrix
	•	Shadow mode
PPg provisions a shadow DB with required packs enabled. If a required pack is unavailable in the region, the job fails with E_PACK_UNAVAILABLE
	•	EXPLAIN-only mode
Requires the same capability profile to produce realistic plans. If packs are unavailable, PPg fails with E_CAPABILITY_NEGOTIATION_FAILED rather than attempting a degraded parse-only mode
