...
The CLI persists a small JSON config file under the user's config directory (`$XDG_CONFIG_HOME/prisma-next/config.json` on Unix per the XDG Base Directory Specification, defaulting to `~/.config/prisma-next/config.json` when `$XDG_CONFIG_HOME` is unset; platform-equivalent under `%APPDATA%` on Windows) that records both the user's telemetry consent answer (`enableTelemetry: boolean`) and a v4 random installation UUID (`installationId: string`). The UUID is generated together with the first persist of `enableTelemetry: true` — typically the affirmative answer to the `prisma-next init` consent prompt — and reused thereafter as the deduplication key for monthly-active-user counts. The value is not derived from any system identifier (no MAC address, no `/etc/machine-id`, no Windows MachineGuid, no IORegistry UUID, no hostname). Editing or deleting the file resets the relevant fields; the CLI never recreates it without explicit consent and never deletes it implicitly.
...
The trust posture is *we count installations — only those whose owners explicitly opted in*, not *we identify your machine*.
...
The installation ID is a v4 random UUID. It is generated at the moment `enableTelemetry: true` is first persisted to the user-level config file — typically when the user answers the `prisma-next init` consent prompt affirmatively. No bytes of its value are derived from any system identifier. It is opaque to anyone other than the team operating the telemetry backend.

### Storage

The UUID is persisted alongside the user's telemetry consent answer in a single JSON file under the user's per-user config directory: `$XDG_CONFIG_HOME/prisma-next/config.json` on Unix (with `$XDG_CONFIG_HOME` defaulting to `$HOME/.config` per the XDG Base Directory Specification), and the platform-equivalent path under `%APPDATA%` on Windows. The file holds two fields: `enableTelemetry: boolean` (the stored consent answer) and `installationId: string` (the UUID). Additional fields may be added in future versions; readers tolerate and writers preserve unknown fields for forward compatibility.

### Lifecycle

- The file is **created** when the user answers the `prisma-next init` consent prompt (or any future flow that persists `enableTelemetry` for the first time). An affirmative answer writes `{ enableTelemetry: true, installationId: <new v4 UUID> }`; a negative answer writes `{ enableTelemetry: false }` without an `installationId` — the negative answer is a stored state, distinguishable from the never-answered case.
...
- Env-var opt-out (`PRISMA_NEXT_DISABLE_TELEMETRY`, `DO_NOT_TRACK`) is purely runtime: it suppresses the telemetry code path for that invocation and never mutates the file on disk.
- Flipping the stored `enableTelemetry` from `true` to `false` (via manual file edit; or a future opt-out command) updates the field but does **not** delete the `installationId`. A later opt-back-in reuses the prior UUID, preserving MAU continuity.
- Deleting the file manually is the user's full reset path. The CLI treats a missing file as the never-answered state: telemetry stays default-off, and on the next interactive `prisma-next init` run, the consent prompt re-appears; an affirmative answer regenerates a fresh `installationId` together with the new `enableTelemetry: true` persist.

### Disclosure

The disclosure-and-consent surface is the interactive `prisma-next init` prompt. The CLI asks an explicit yes/no question as the last item in the init prompt sequence, persists the user's answer to `config.json`, and never collects telemetry without an affirmative `enableTelemetry: true` having been stored. No other CLI command prints a telemetry banner; users who never run `init` (or who skip the prompt under `--yes` or in a non-TTY shell) stay in the default-off state. Documentation links describe the stored fields, the two env-var opt-outs, and the manual-edit path for flipping the choice later.
...
- **No filesystem-mutation-on-config-change surface.** Env-var opt-out is purely "don't enter the branch"; the CLI never deletes or mutates the user-level config file in response to an env-var change. The only writes that touch the file are explicit user-initiated actions (the `init` consent prompt's confirm, or a future opt-out command). This avoids an entire class of accidents where a runtime setting change implicitly deletes filesystem state the user wanted to keep.
- **Strictly opt-in default.** Telemetry is never collected without `enableTelemetry: true` having been explicitly written to the file. Users who skip `init`, who answer "no", or who run only non-init commands stay in the default-off state and never appear in the telemetry stream. This trades some adoption signal for a stronger trust contract — a position the project judged worth the cost for an EA-stage OSS release.
