...
read user config + env
  ...
  installationId stored?  ──yes──► send (silent)
        │ no
        ▼
  interactive `init` first run?  ──yes──► show consent prompt instead
        │ no
        ▼
...
On the first command that resolves enabled with `installationId === undefined`, the parent fire path (`discloseAndMintOnFirstRun` in `cli/src/utils/telemetry.ts`) prints a one-time disclosure to **stderr** (never stdout, so piped output is never corrupted), then mints the `installationId` and sends. The notice text, with the resolved absolute config-file path substituted in, is exactly (`firstRunNotice`):

```text
Prisma Next collects anonymous CLI usage data, enabled by default. What's collected and why: docs/Telemetry.md. Opt out anytime: DO_NOT_TRACK=1, PRISMA_NEXT_DISABLE_TELEMETRY=1, or set "enableTelemetry": false in <resolved config.json path>.
```
...
### Retain the interactive `init` consent prompt as that path's disclosure

The interactive `prisma-next init` consent prompt (`TELEMETRY_CONSENT_MESSAGE` in `init/inputs.ts`) is unchanged — same wording, still default-**Yes**, still shown only on an interactive, non-`--yes`, non-CI, non-env-opted-out first `init` where `enableTelemetry` is still `undefined`. On that one path the prompt **substitutes** for the first-run notice: `interactiveInitPromptWillFire` detects it in the preAction hook and stands the notice/mint/send down, so the user sees the prompt (not the stderr notice) and the event is sent only after they answer. A **non-interactive** `init` (`--yes` / piped / `--no-interactive`) does not show the prompt and gets the first-run notice like any other command.

### Mint the `installationId` on first enabled send

The id is minted by `ensureInstallationId`, which persists the id **alone** and leaves `enableTelemetry` absent. Persisting only the id (rather than coupling it to an `enableTelemetry: true` write) is what keeps the opt-out default intact *and* keeps the `init` consent prompt live, since that prompt fires only while `enableTelemetry` is `undefined`. The identifier design is unchanged from [ADR 216](./ADR%20216%20-%20CLI%20telemetry%20installation%20ID%20is%20a%20stored%20random%20UUID%20not%20a%20system%20fingerprint.md); only its *minting trigger* moved from "first `enableTelemetry: true` persist" to "first enabled send".
...
- **Disclosure still happens before the first send.** Opt-out does not mean undisclosed: the first enabled command prints the notice (or, on interactive init, shows the prompt) *before* the event leaves the machine, and points the user at the docs and all three opt-out signals.
...
- **No telemetry in CI, ever.** CI remains hard-disabled and sees neither the notice nor the prompt.
...
- **Leave telemetry opt-in and add more prompts (e.g. prompt on first non-`init` command).** Rejected: more interactive prompts do nothing for the non-interactive and agent-driven population, which is exactly the gap, and they add friction to every entry point rather than the one (`init`) where a setup prompt already belongs.
