...
Step (2) relies on the emit pipeline being deterministic: instantiating the class and serializing `instance.operations` must produce byte-identical artifacts whether driven directly by the developer's shebang, by `migration plan`'s inline emit, or by the verifier's import path ([ADR 196](ADR%20196%20-%20In-process%20emit%20for%20class-flow%20targets.md)). If the pipeline left any field unset (e.g. wrote a draft `migrationId: null`), the freshly computed hash would never match the on-disk hash and the staleness check would always trigger.
