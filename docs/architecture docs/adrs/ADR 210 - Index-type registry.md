...
When an extension pack declares its index types, the factory call site is the single point where the type literal is named. The builder produces both a runtime value (a registration record) and a TypeScript-only derivation (the phantom map of literal → option shape). The pack attaches the phantom map under its `__indexTypes` field and invokes `register` at module load.

When a contract is defined, the contract-definition pipeline walks the attached packs, gathers each pack's `__indexTypes`, and intersects them. The resulting merged map is what narrows `options` per-`type` for `constraints.index(...)`. There is no name resolution against a global symbol; every contract sees only the packs it asked for.

When a contract is validated at runtime, `validateSqlStorage` consults the runtime registry — the same value the packs registered into at module load. Lookup is by `type` literal; validation of `options` is one arktype invocation per index in strict mode. The framework owns the Postgres-shaped renderer for `options`; per-entry validators have no rendering responsibility and no rendering surface area.
