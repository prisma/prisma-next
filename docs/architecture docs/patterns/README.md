...
| Interface + factory function (stateful services) | `interface-plus-factory.md` | Stateful services are exposed as an exported `interface` plus a `createXxx()` factory; the implementing class is private. | Pending |
| Adapter SPI for target-specific behaviour | `adapter-spi.md` | Target-specific behaviour is encapsulated behind an adapter interface the framework consumes uniformly; the framework never branches on `target === 'postgres'`. | Pending |
| Capability gating | `capability-gating.md` | Optional or target-varying features are declared as capabilities, verified against the database at runtime, and gated at every consumption site. | Pending |
