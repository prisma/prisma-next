...
A parameterized codec (`vector(N)`, `char(N)`, `numeric(p, s)`, `json(schema)`) is a **higher-order codec** (HoC): a curried `(params) => (ctx: Ctx) => Codec<…, Js>` function. Its TypeScript signature is the type-level surface; its body is the runtime implementation. A small sister `ParameterizedCodecDescriptor<P>` registers the function with the framework and carries the metadata that the codec object itself doesn't own (`paramsSchema` for JSON-boundary validation, optional `renderOutputType` for the emit path). See [ADR 205 — Higher-order codecs for parameterized types](../adrs/ADR%20205%20-%20Higher-order%20codecs%20for%20parameterized%20types.md).

Three resolution surfaces consume different pieces of the same codec model:

- **No-emit type resolution** (`@prisma-next/sql-contract-ts`'s `FieldOutputType`) reads the curried factory's TypeScript return type. The `ColumnTypeDescriptor` carries the factory on its `type?: (ctx: Ctx) => Codec` slot; the resolver synthetically applies a `Ctx` at the type level and reads the resulting `Codec<…, Js>`'s `Js` slot. Literal numeric inference (`vector(1536)` → `Vector<1536>`) and schema-derived inference (`json(productSchema)` → `InferOutput<typeof productSchema>`) flow through standard TypeScript inference applied to the curried factory.
- **Emit-path rendering** consults a `ParameterizedCodecDescriptorLookup` keyed by `codecId` and reads `descriptor.renderOutputType(typeParams)` to stamp a TypeScript source string into the `FieldOutputTypes` map. The lookup is assembled by `extractParameterizedCodecLookup` in `@prisma-next/framework-components/control` from each component's `types.codecTypes.parameterizedCodecs` contribution. When the descriptor or its renderer is absent, the emitter falls back to `CodecTypes[codecId]['output']`.
- **Runtime materialization** (`@prisma-next/sql-runtime`'s `initializeTypeHelpers`) walks `storage.types` (named instances) and inline-`typeParams` columns (anonymous instances), validates each instance's params via `descriptor.paramsSchema['~standard'].validate`, and calls `descriptor.factory(validatedParams)({ name, usedAt })` once per instance. The resulting `Codec` is indexed by instance name; per-column dispatch resolves through this index, and any per-instance helper state lives on the resolved codec (gated by a `CodecTrait` such as `'json-validator'` so the read is structurally typed).
