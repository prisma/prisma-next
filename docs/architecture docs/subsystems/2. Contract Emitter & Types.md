...
import type { Contract as ContractType, StorageHashBase, ProfileHashBase } from '@prisma-next/contract/types';
...
type ContractBase = ContractType<
  { readonly tables: { /* … storage tables with columns, PKs, FKs, indexes … */ }; readonly storageHash: StorageHash },
  { readonly User: { readonly fields: { /* … { codecId, nullable } per field … */ }; readonly relations: { /* … */ }; readonly storage: { readonly table: 'user'; readonly fields: { /* … field-to-column mappings … */ } } } }
> & {
...
### Parameterized codecs and the `FieldOutputTypes` map

A parameterized codec (`vector(N)`, `char(N)`, `numeric(p, s)`, `arktypeJson(schema)` from `@prisma-next/extension-arktype-json`) is a **higher-order codec** (HoC): a curried `(params) => (ctx: Ctx) => Codec<…, Js>` function. Its TypeScript signature is the type-level surface; its body is the runtime implementation. A small sister `CodecDescriptor<P>` registers the function with the framework and carries the codec-id-keyed metadata (`traits`, `targetTypes`, `paramsSchema` for JSON-boundary validation, optional `renderOutputType` for the emit path). Non-parameterized codecs are the degenerate `P = void` case under the same descriptor type. See [ADR 205 — Higher-order codecs for parameterized types](../adrs/ADR%20205%20-%20Higher-order%20codecs%20for%20parameterized%20types.md).

Three resolution surfaces consume different pieces of the same codec model:

- **No-emit type resolution** (`@prisma-next/sql-contract-ts`'s `FieldOutputType`) reads the curried factory's TypeScript return type. The `ColumnTypeDescriptor` carries the factory on its `type?: (ctx: Ctx) => Codec` slot; the resolver synthetically applies a `Ctx` at the type level and reads the resulting `Codec<…, Js>`'s `Js` slot. Literal numeric inference (`vector(1536)` → `Vector<1536>`) and schema-derived inference (`arktypeJson(ProductSchema)` → schema's inferred output) flow through standard TypeScript inference applied to the curried factory.
- **Emit-path rendering** consults the codec-id-keyed descriptor map and reads `descriptor.renderOutputType(typeParams)` to stamp a TypeScript source string into the `FieldOutputTypes` map. The descriptor map is assembled at context-construction time in `@prisma-next/sql-runtime` from each component's `types.codecTypes.parameterizedCodecs` contribution (and synthesized for non-parameterized codecs registered through the legacy `codecs:` slot). When the descriptor's renderer is absent, the emitter falls back to `CodecTypes[codecId]['output']`.
...
- **Runtime materialization** (`@prisma-next/sql-runtime`'s `initializeTypeHelpers` and `buildContractCodecRegistry`) walks `storage.types` (named instances) and inline-`typeParams` columns (anonymous instances), validates each instance's params via `descriptor.paramsSchema['~standard'].validate`, and calls `descriptor.factory(validatedParams)({ name, usedAt })` once per instance. The resulting `Codec` is indexed by instance name and surfaced through `ContractCodecRegistry.forColumn(table, column)` for column-aware dispatch. Per-instance helper state (e.g. a rehydrated arktype schema for `arktypeJson` columns) lives in the factory's closure on the resolved codec; codec-id-keyed metadata is read through `descriptorFor(codecId)` on the descriptor map.

Both authoring and runtime invoke the same factory function with the same params and the same `Ctx`. There is no parallel runtime function and no opportunity for drift between the type-level surface and the runtime body.

### Anonymous and named storage type instances

Inline `typeParams` on a column produce an *anonymous* `storage.types` instance with the deterministic name `<anon:${table}.${column}>`. Named instances (a `storage.types` entry referenced by `typeRef`) collect every column that references them in `Ctx.usedAt`, which lets a codec derive shared per-instance state (e.g. a CipherStash key bound to a column set) from the aggregated set. The framework's `Ctx` is `{ name: string; usedAt: ReadonlyArray<{ table: string; column: string }> }`; pack authors never construct it.
