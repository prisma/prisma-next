...
The `output` field in `ContractConfig` (and the matching `output?: string` option on the `defineConfig` helpers from `@prisma-next/mongo` and `@prisma-next/postgres`) sets where `contract.json` and `contract.d.ts` are written. When omitted, the path is derived from the contract source file by replacing its extension with `.json`.

The `prisma-next contract emit` command accepts `--output <path>` to override the configured value at call time. Precedence: `--output` flag > `output` in config > derived default. Paths are resolved relative to the current working directory. If the supplied path does not end with `.json`, the CLI issues a warning but proceeds; the companion `.d.ts` is derived by replacing the extension.
