...
- Write and verify the marker `{ coreHash, profileHash }` (see [ADR 021 — Contract Marker Storage](../adrs/ADR%20021%20-%20Contract%20Marker%20Storage.md))
...
- **Marker:** Database record of `{ coreHash, profileHash }`
...
3. Apply the edge’s operations to reach `to.coreHash`
4. Evaluate pre/post checks and any policy gates
5. Write the contract marker to `{ coreHash: to.coreHash, profileHash }` and verify it
...
Bundles contain:
- `contract.json` (`to.coreHash`, `profileHash`)
...
4. Apply the edge to reach `to.coreHash`
...
**Inputs:** `contract.json` for the target (`to.coreHash`, `profileHash`), the current marker (`from`), and the edge ops with pre/post checks.
...
```json
{
  ...
  "marker": { "coreHash": "sha256:cd34…", "profileHash": "pg/default@1" },
  "mode": "shadow",
...
CI query checks (EXPLAIN/lints for query plans) are documented separately. This document focuses exclusively on migration preflight.
