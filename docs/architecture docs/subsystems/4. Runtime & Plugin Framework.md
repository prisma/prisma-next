...
// Create execution stack and instantiate
...
const rt = createRuntime({
  ...
  contract,
  context,
  driverOptions: { connectionString: process.env.DATABASE_URL },
  verify: {
...
1. Create
   ...
   - `createExecutionContext({ contract, stackInstance })` - Creates context with validated contract, codec and operations registries
   - `createRuntime({ stackInstance, contract, context, driverOptions, verify, plugins })` - Creates runtime from stack instance
