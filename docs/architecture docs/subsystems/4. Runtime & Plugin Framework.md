...
1. Create
   - `createExecutionStack({ target, adapter, driver, extensionPacks })` - Creates stack of descriptors
   ...
   - `instantiateExecutionStack(stack)` - Instantiates all components in the stack
   - `createRuntime({ stackInstance, context, driver, verify, plugins })` - Creates runtime from stack instance. The caller provides the driver instance and context explicitly.
