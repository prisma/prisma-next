...
```typescript
import { createRuntime, createRuntimeContext } from '@prisma/runtime'
import { postgresAdapter } from '@prisma/adapter-postgres'
import { sql, schema, validateContract } from '@prisma/sql'
...
const context = createRuntimeContext({
  ...
  extensionPacks: [],     // Extensions provide codecs and operations programmatically
})
...
1. Create
   - `createRuntimeContext({ contract, adapter, extensions })` - Creates context with validated contract, codec and operations registries (caller validates contract before passing)
   - `createRuntime({ adapter, driver, verify, context, plugins, mode })` - Contract comes from context, not separate parameter
   - Validates `contract.json` and caches `coreHash`/`profileHash` (contract is already validated before being passed to context).
   ...
   - Uses `RuntimeContext` for contract, codec and operations registries (decoupled from runtime).
   - Registers plugins in order.
2. Warmup
   - Optional `driver.warmup()` and built‑in contract verification if configured.
   - Codecs and operations are composed in `RuntimeContext` from adapter and extensions programmatically ([ADR 030](../adrs/ADR%20030%20-%20Result%20decoding%20&%20codecs%20registry.md), [ADR 114](../adrs/ADR%20114%20-%20Extension%20codecs%20&%20branded%20types.md)).
3. Acquire
