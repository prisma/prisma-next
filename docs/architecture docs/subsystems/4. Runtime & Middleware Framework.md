...
**Runtime context: `identityKey` and `scope`.** Two `RuntimeMiddlewareContext` fields support the annotation-driven middleware ecosystem:

- `identityKey(exec): string` — the family runtime returns an opaque, bounded digest identifying the `(storage, statement, params)` tuple of an execution. SQL composes `meta.storageHash + '|' + exec.sql + '|' + canonicalStringify(exec.params)` and pipes through `hashIdentity` (BLAKE2b-512); Mongo composes `meta.storageHash + '|' + canonicalStringify(exec.command)` and pipes the same way. Two semantically equivalent executions return the same digest. Used by middleware that need per-execution identity (caching, request coalescing). The cache middleware uses the returned string directly as a `Map` key — it is not (and should not be) further hashed by callers.
...
- `RuntimeMiddlewareContext` carries `identityKey(exec)` (a bounded opaque digest of the execution identity) and `scope` (`'runtime' | 'connection' | 'transaction'`) for middleware that need per-execution identity or scope-aware behavior.
...
- Family `identityKey` implementations: SQL and Mongo unit tests cover stability across object-key order, discrimination on `storageHash` and params/command, fixed-size opaque output (`blake2b512:HEXDIGEST`), and round-trip determinism.
