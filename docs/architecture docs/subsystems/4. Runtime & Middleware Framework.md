...
The middleware system keeps logic out of the core and makes behavior composable. First‑party and community middlewares can implement lints, budgets, and telemetry. Every Plan is presented to middlewares before execution so policies and checks run consistently.
...
This sequence shows the runtime's tight feedback loop at execution time: the runtime verifies its loaded contract against the database marker before any work, middlewares apply policy and budgets deterministically, and the driver streams rows back while middlewares observe per‑row and aggregate outcomes.
...
- Optionally, the runtime or middlewares may check that `plan.meta.storageHash` matches the loaded contract for diagnostic clarity.
...
Plan (immutable)
  ...
  └─▶ beforeExecute         // middlewares: lints, budgets, annotations checks
...
1. Create
   ...
   - `createRuntime({ stackInstance, context, driver: stackInstance.driver, verify, middlewares })` - Creates runtime with now-bound driver.
   ...
   - Validates middleware compatibility (familyId/targetId) and registers middlewares in order.
2. Warmup
...
Notes:
- Plans are immutable; middlewares must not mutate in place. See [ADR 011](../adrs/ADR%20011%20-%20Unified%20Plan%20model%20across%20lanes.md).
- Behavior is composed, not configured: middlewares make policy explicit and testable; the only global tuning is a transparent `mode` (`strict`/`permissive`).
...
- Generic middlewares (no `familyId`) work across all families. Family-specific middlewares (`SqlMiddleware`, `MongoMiddleware`) use narrowed plan/context types.

## Guardrails via Middlewares

Guardrails are applied by a deterministic middleware pipeline that runs before, during (per‑row), and after execution. With zero middlewares, the runtime only verifies the contract and executes the Plan. When middlewares are present, lints and budgets enforce policy while telemetry records outcomes. This keeps behavior composable and explicit, with middlewares executing in registration order and clear failure semantics.
...
- < 1 ms median overhead for the hook pipeline with first‑party middlewares enabled and no EXPLAIN is issued.
...
- Middleware API contract tests with mock middlewares and error injection.
