...
2. **`lower(plan)`** — abstract. Each family produces its `*ExecutionPlan` (SQL via `lowerSqlPlan`, Mongo via `adapter.lower`). Raw-SQL plans (already lowered) bypass this in `SqlRuntime` via the existing branch in `executeAgainstQueryable`.
...
flowchart TB
  ...
  PR[PostgresRuntime] -.extends.-> SR
  SLR[SqliteRuntime] -.extends.-> SR
  SupR[SupabaseRuntime] -.extends.-> PR
```

**SQL family.** [`SqlRuntime extends RuntimeCore<SqlQueryPlan, SqlExecutionPlan, SqlMiddleware>`](../../../packages/2-sql/5-runtime/src/sql-runtime.ts) is **abstract** and overrides:
...
`SqlRuntime` also wraps the inherited `execute` path with codec row-decoding, marker verification (via `RuntimeFamilyAdapter`), and telemetry fingerprinting (via `computeSqlFingerprint`); these stay SQL-side rather than living in the abstract base.

`SqlRuntime` also exposes two `protected` primitives for target subclasses: `executeWithSessionBootstrap` and `executeTransactionWithBootstrap`. See [Session Bootstrap](#session-bootstrap) below.

**SQL target classes.** [`PostgresRuntime`](../../../packages/3-extensions/postgres/src/runtime/postgres-runtime.ts) and [`SqliteRuntime`](../../../packages/3-extensions/sqlite/src/runtime/sqlite-runtime.ts) extend `SqlRuntime` and are currently identity-thin — their value is structural. Future Postgres-specific runtime behaviour (prepared-statement caching, `LISTEN`/`NOTIFY`, etc.) lands on `PostgresRuntime` and flows into `SupabaseRuntime` by inheritance.
...
`SqlRuntime` exposes two `protected` primitives for target subclasses that need to issue SQL on the raw transaction before the typed query runs — below the user middleware chain.
...
`RawSessionConnection` exposes only `query(sql, params?)` — no lifecycle methods. Both primitives: acquire a connection, `BEGIN`, run `bootstrap(rawConn)` directly against the raw transaction (no middleware), then execute the typed query/callback through the standard middleware chain on the same transaction, then `COMMIT` or `ROLLBACK`, then release. The connection lifecycle is identical to `withTransaction`.

**Why below-middleware matters.** Bootstrap SQL runs before `runWithMiddleware` sees the typed plan. User middleware cannot observe, reorder, or strip it. This makes bootstrap SQL structurally non-bypassable — the property that makes Postgres RLS enforcement sound.

```
acquire connection → BEGIN
  ├─ bootstrap(RawSessionConnection)          ← raw tx.query(); invisible to user middleware
  └─ executeAgainstQueryable(plan, tx)        ← standard middleware chain
       ├─ intercept?
       ├─ beforeExecute
       ├─ runDriver (same tx)
       └─ afterExecute
...
[`SupabaseRuntime`](../../../packages/3-extensions/supabase/src/runtime/supabase-runtime.ts) is the canonical user of these primitives: it issues `SELECT set_config('role', $1, true)` and `SELECT set_config('request.jwt.claims', $2, true)` in the bootstrap closure to apply a Postgres role and JWT claims before the ORM/SQL query runs. The session-bootstrap ADR is at [`projects/runtime-target-layer/specs/adr-runtime-target-layer.md`](../../../projects/runtime-target-layer/specs/adr-runtime-target-layer.md).
...
**Verification ordering.** Contract-marker verification happens upstream of `runWithMiddleware` (in `SqlRuntime.executeAgainstQueryable`, and would in any other family that needs it). A stale-schema query still throws `contract/hash-mismatch` before any interceptor sees it. The `intercept` hook does not bypass verification.
...
1. Create
   ...
   - Target factory (e.g. `postgres()`) constructs `new PostgresRuntime({ context, adapter, driver, verifyMarker, middleware })` — a concrete subclass of `SqlRuntime` / `RuntimeCore`. There is no family-level `createRuntime` factory.
...
`SqlRuntime.runBeforeCompile` overrides the abstract base's identity default to delegate to `runBeforeCompileChain` ([source](../../../packages/2-sql/5-runtime/src/middleware/before-compile-chain.ts)), which iterates the registered middleware in order.
