...
**Error path.** A middleware that throws inside `intercept` surfaces via the standard `runtimeError` envelope. `afterExecute` fires with `completed: false` and `source: 'middleware'` — the failure originated in the intercept chain, not in the driver. Errors thrown by `afterExecute` during the error path remain swallowed (existing semantics, unchanged).
...
The framework provides four pieces:
...
- `defineAnnotation<Payload, Kinds>({ name, applicableTo, namespace? })` — produces a callable handle. The handle is itself a function: invoking it (`cacheAnnotation({ ttl: 60_000 })`) produces a frozen, branded `AnnotationValue`. The handle additionally exposes `name` (registry key, also default namespace), `namespace` (storage key on `plan.meta.annotations`), `applicableTo` (frozen `ReadonlySet<Kinds>`), and a `read(plan)` accessor.
- `AnnotationRegistry` — a per-runtime container of annotation handles. `RuntimeCore` walks every middleware's `annotations` field and registers each handle into `runtime.annotationRegistry`. Two middleware contributing the same handle (by identity) is a no-op; two middleware contributing different handles under the same name throws at construction time.
- `AnnotationBuilder<K, Registry>` and the runtime factory `createMetaBuilder(registry, kind)` — the structural surface lane-terminal `.annotate(callback)` callbacks consume. Methods on the builder come from `RegistryFor<K, Registry>` — a kind-filtered subset of the registry where each registered handle whose `applicableTo` includes `K` becomes a callable method. Calling a method extends the immutable `values` array and returns a new frozen builder. The framework brands the builder with the unique `[ANNOTATION_BUILDER]: true` symbol so it can distinguish chained-builder returns from the array escape hatch (`readonly AnnotationValue[]`).
...
export const cacheAnnotation = defineAnnotation<{ ttl?: number }, 'read'>({
  name: 'cache',
  applicableTo: ['read'],
})

// Write-only annotation (illustrative).
export const auditAnnotation = defineAnnotation<{ actor: string }, 'write'>({
  name: 'audit',
  applicableTo: ['write'],
})

// Applicable to both kinds (e.g. tracing).
export const otelAnnotation = defineAnnotation<{ traceId: string }, 'read' | 'write'>({
  name: 'otel',
  applicableTo: ['read', 'write'],
})

// The handle is callable directly:
const value = cacheAnnotation({ ttl: 60_000 })
//    ^? AnnotationValue<{ ttl?: number }, 'read'>
```

**Registration.** Middleware that own annotations declare them on an `annotations` field; `createCacheMiddleware` returns `{ name: 'cache', annotations: { cache: cacheAnnotation }, intercept, onRow, afterExecute }`. `RuntimeCore`'s constructor walks `options.middleware`, registers each handle into the runtime's `AnnotationRegistry`, and exposes the assembled registry as `this.annotationRegistry`. The family runtime also threads the registry into the lane builder context (`BuilderContext.annotationRegistry` for SQL, `CollectionContext.annotationRegistry` for ORM) so terminals can derive the kind-filtered builder at `.annotate(callback)` time.

**Type-level projection.** The family factory captures the middleware tuple via a `const Mw extends readonly SqlMiddleware[]` generic and projects `PostgresClient<TContract, AnnotationsOf<Mw>>`, threading `AnnotationsOf<Mw>` (a `UnionToIntersection`-flattened registry shape) through `Db<TContract, Registry>` and `OrmClient<TContract, Collections, Registry>`. At every lane terminal that exposes `.annotate(callback)`, the callback's `meta` parameter is structurally typed as `AnnotationBuilder<K, Registry>` — only the kind-applicable methods exist on it.

**Lane integration.** Each lane terminal exposes a single optional trailing `annotateFn` callback. The callback receives a kind-filtered `AnnotationBuilder<K, Registry>` and returns either the chained builder or a `readonly AnnotationValue<unknown, OperationKind>[]` (the array escape hatch for ad-hoc / closure-captured handles).

- SQL DSL — `.annotate(callback)` is chainable on every builder kind: `SelectQueryImpl` / `GroupedQueryImpl` receive `AnnotationBuilder<'read', Registry>`; `InsertQueryImpl` / `UpdateQueryImpl` / `DeleteQueryImpl` receive `AnnotationBuilder<'write', Registry>`. Read-applicable handles like `cache` are structurally absent on a write builder's `meta`, so `meta.cache(...)` on a write terminal is a property-not-found error from the type system, not a chained `ValidAnnotations` failure.
- ORM `Collection` — each terminal method (`first`, `all`, `aggregate`; `create`, `update`, `delete`, `upsert`, `*Count`, `*All`) accepts a single optional trailing `annotateFn` callback with the same kind-filtered shape. There is no chainable `Collection.annotate()` — annotations attach via the terminal call only. This is an intentional scope cut: chainable form would have forced an in-middleware mutation classifier.

**Runtime applicability check.** Every lane terminal calls `assertAnnotationsApplicable(values, kind, terminalName)` after extracting the callback's `values` array. The helper iterates the array and, on any annotation whose `applicableTo` set lacks `kind`, throws `RUNTIME.ANNOTATION_INAPPLICABLE` naming the namespace and terminal. This is belt-and-suspenders: the type system fails closed for type-aware callers, and the runtime check fails closed for casts / `any` / dynamic invocations / the array escape hatch with a wrong-kind handle.

**Storage.** Produced annotations land under `plan.meta.annotations[namespace]` as branded `AnnotationValue<Payload, Kinds>` objects. Multiple `.annotate(callback)` calls compose; duplicate namespaces use last-write-wins. Reading via `handle.read(plan)` defensively checks the `__annotation` brand so framework-internal metadata (e.g. `meta.annotations.codecs` written by the SQL emitter) is not mistaken for a user annotation.
...
- `contentHash(exec): Promise<string>` — the family runtime returns an opaque, bounded digest of the post-lowering execution **content** (the `(storage, statement, params)` tuple as it is about to be sent to the driver). SQL composes `meta.storageHash + '|' + exec.sql + '|' + canonicalStringify(exec.params)` and pipes through `hashIdentity` (SHA-512 via the Web Crypto API); Mongo composes `meta.storageHash + '|' + canonicalStringify(exec.command)` and pipes the same way. The Web Crypto choice keeps the runtime portable across Node, Bun, Deno, Cloudflare Workers, Vercel Edge, and browsers (none of which expose BLAKE2 through a standard API), and forces the digest — and therefore the SPI — to be asynchronous. Two semantically equivalent executions resolve to the same digest, while a `beforeCompile` rewriter that mutates the plan changes the content and therefore the hash — this is content identity, not a rewrite-stable logical identity. Used by middleware that need per-execution content identity (caching, request coalescing). The cache middleware uses the resolved string directly as a `Map` key — it is not (and should not be) further hashed by callers.
...
This sequence shows the runtime's tight feedback loop at execution time: the runtime first runs `runBeforeCompile` and lowers the plan, then verifies its loaded contract against the database marker (timing controlled by `verify.mode` — `startup` runs once at runtime construction, `onFirstUse` runs on the first execute, `always` runs on every execute), middleware apply policy and budgets deterministically, and the driver streams rows back while middleware observe per-row and aggregate outcomes.
...
- `RuntimeMiddlewareContext` carries `contentHash(exec)` (a bounded opaque digest of the post-lowering execution content) and `scope` (`'runtime' | 'connection' | 'transaction'`) for middleware that need per-execution content identity or scope-aware behavior.
...
- Annotations: unit + type tests for `defineAnnotation`, `AnnotationRegistry`, `RegistryFor<K, Reg>` and `AnnotationBuilder<K, Reg>` (positive + negative on both kinds), `assertAnnotationsApplicable` (cast-bypass coverage), `createMetaBuilder`, and lane-side `.annotate(callback)` on every SQL DSL builder kind plus every ORM terminal. Negative type tests verify the structural property-not-found shape (e.g. `meta.cache` is structurally absent from `AnnotationBuilder<'write', Registry>`).
