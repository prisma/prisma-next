...
- Kysely lane: Uses Kysely query builder; transforms `compiledQuery.query` (Kysely AST) into PN `QueryAST` via `transformKyselyToPnAst`, attaches `ast`, and populates lane metadata including `meta.refs` plus Kysely-lane-specific parameter descriptors. Pre-transform guardrails reject unqualified refs and ambiguous `selectAll` in multi-table scope. See [ADR 162](../adrs/ADR%20162%20-%20Kysely%20lane%20emits%20PN%20SQL%20AST.md) and `@prisma-next/sql-kysely-lane`.
...
`storage.tables.<t>.primaryKey` is optional in the SQL contract. Id-less SQL tables are emittable from PSL (`model X { ... }` without `@id`/`@@id`) and from introspection. The ORM lane works end-to-end on id-less tables for every operation that does not depend on a primary-key column:

- Predicate-based reads and writes — `where(...).first()`, `where(...).all()`, `where(...).update(...)`, `where(...).delete()`, `where(...).updateAll(...)`, `where(...).deleteAll()` — target the table by the caller's predicate.
- Count helpers — `where(...).updateCount(...)` and `where(...).deleteCount()` — execute a single `UPDATE … RETURNING` / `DELETE … RETURNING` and return the streamed row count. No primary-key column is read or assumed.
- Nested mutations with `.select(...)` / `.include(...)` reload the just-mutated row via a row-identity criterion: when the table declares a primary key, the criterion is PK-keyed (composite PKs include every PK column); when the table has no primary key, the criterion is built from the RETURNING row's mapped non-null column values. The reload runs in the open mutation transaction against a row just produced by RETURNING, so the criterion matches by construction. The nested-update path additionally requires the table to declare either a primary key or at least one unique constraint — without one, the row-identity criterion cannot guarantee single-row updates and would broaden to all rows matching the row tuple; the ORM rejects this case with an operation-tagged error.
- The SQL DSL query-builder lane (`db.sql` / `sql()`) is unaffected and works on id-less tables — callers state the table, predicates, and projection directly.

Multi-table-inheritance (MTI) polymorphism is the one case that genuinely requires a primary key on the base table. Variant tables join onto the base table's primary-key column at write time (`#resolveMtiCreateContext`) and at read time (`buildMtiJoins`); without a stable identity column, the variant join cannot be expressed in one statement. The ORM lane throws an operation-tagged error from `resolvePrimaryKeyColumn` if MTI is invoked on an id-less base table — this is a design constraint, not a fallback.

SQLite caveat: SQLite `RETURNING` does not reflect AFTER-trigger column rewrites (Postgres does). For id-less mutation reload, this means a row-identity criterion built from RETURNING values may fail to match if AFTER triggers rewrote columns present in the criterion. Tables with primary keys are unaffected because their criterion is PK-keyed.
...
Views and other read-only sources are exposed through the DSL identically to tables, with full type safety and read-only enforcement (see [ADR 126](../adrs/ADR%20126%20-%20PSL%20top-level%20block%20SPI.md)). Mutations on read-only sources are blocked by lints and at runtime.
...
- **ADR 162** — Kysely lane emits PN SQL AST
  - Transform Kysely compiledQuery.query -> PN QueryAST; attach plan.ast, meta.refs, and lane-specific param descriptors
