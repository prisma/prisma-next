...
`storage.tables.<t>.primaryKey` is optional in the SQL contract. Id-less SQL tables are emittable from PSL (`model X { ... }` without `@id`/`@@id`) and from introspection. This is not a general restriction on ORM collection operations: predicate-based reads and writes such as `where(...).first()`, `where(...).update(...)`, and `where(...).delete()` can target id-less tables because the caller provides the row-matching predicate explicitly. The SQL DSL query-builder lane (`db.sql` / `sql()`) is also safe for the same reason: callers state the table, predicates, and projection directly.

The unsafe case is primary-key fallback behavior. Today the ORM lane reaches for the table's primary key in four places:

- mutation reload (re-SELECT after `update`/`delete`) via `buildPrimaryKeyFilterFromRow`
- `updateCount()` and `deleteCount()` helpers, which currently select the primary-key column before mutating to count matched rows
- MTI polymorphism, which joins variant tables onto the base table's primary key

Each of these calls `resolvePrimaryKeyColumn(contract, tableName, operation)`. If the table has no `primaryKey`, the helper throws an operation-tagged error rather than silently falling back to a literal `'id'` column that may not exist (the previous behavior). For id-less models, callers must either use the SQL DSL query-builder lane or use the predicate-based ORM operations that do not depend on PK fallback.

Future work, intentionally out of scope for the current gate:

- A per-model capability flag (e.g. `sql.ormRequiresPrimaryKey`) so consumers can declare PK-less ORM intent at authoring time rather than discovering the constraint at the failing call site. Per-model capability gating does not exist as a primitive today; capabilities are namespaced per target.
- Explicit-predicate variants of the affected helpers (e.g. `updateCount(where, { matchedBy: [...] })`) that name the unique columns to use instead of relying on a single PK.
- `findUnique`-style APIs that key on a unique constraint other than the primary key.
