...
Query lanes are the authoring surfaces that turn user intent into query plans. They exist because one query API cannot be good at every shape of work: a fluent DSL is pleasant for composition, raw SQL is still the clearest form for some advanced statements, ORM macros are useful for relationship traversal, and Kysely or TypedSQL can be better fits for teams that already organize query logic around those tools.
...
A lane is a compiler from one authoring surface to the plan contract understood by the runtime. The input can be a builder chain, raw SQL with annotations, an ORM collection call, a `.sql` file, or a Kysely query. The output is always structured enough for the runtime to answer the questions that matter before execution: which storage contract is this pinned to, what tables and columns are referenced, what row shape is expected, and which capability-gated features are being requested.
...
1. **Compile.** DSL, ORM, and Kysely plans carry AST structure that the adapter lowers to SQL while preserving refs and projection. Raw SQL and TypedSQL provide SQL text, params, and annotations directly.
