...
@prisma/adapter-postgres/
  src/
    profile.ts         # profile id, capabilities
...
- Mongo DSL lane. Target-native builder that constructs an Aggregation Pipeline AST and lowers to `{ kind: 'mongo', pipeline, collection }`
...
4. **Pinned Profile Validation**: Runtime validates environment satisfies the contract‑pinned profile; no profile hash recomputation (see ADR 004)
...
- Capability schema
  - Canonical keys for join (`join.lateral`, `join.semi`, `join.anti`), projection (`projection.distinct`, `projection.distinctOn`), distribution (`distribution.shardKey`), and partial indexes (`index.partial`). See ADR 065 and capabilities reference
...
1. Negotiation: Runtime loads the adapter profile by id and validates capabilities against the contract‑pinned profileHash (no recomputation)
...
- ADR 067 — Adapter lowering determinism & golden tests
  Determinism requirements for lowering, normalization rules, golden rendering policy per target, and when goldens may change. Complements ADR 016 but focuses on adapter conformance
- ADR 068 — Error mapping to RuntimeError
  Stable mapping from engine/driver errors to the RuntimeError envelope and codes, including SQLSTATE handling and non-SQL equivalents. Complements ADR 027
- ADR 069 — Codec registration precedence & collisions
  Precedence between contract codecs, adapter defaults, and lane hints, with conflict detection and safe overrides. Complements ADR 030
- ADR 070 — Non-SQL Plan body kinds & execution semantics
  Formalizes { kind: 'mongo', pipeline, collection } and other non-SQL bodies, required annotations, and execution guarantees under the unified Plan model. Complements ADR 011
- ADR 071 — Migration operation namespaces per target
  vendor/op@v naming, versioning, and validation for SQL vs non-SQL op families, including cross-family no-op semantics and risk classes. Complements ADR 041 and ADR 044
- ADR 072 — Adapter packaging & manifest
  Required metadata (profileId, capabilities, conformance level, versions), how manifests are discovered and verified, and publishing guidelines for community adapters. Complements ADR 026 (conformance kit)
...
- ADR 075 — Profile selection & pinning
  Rules to select an adapter profile by engine version/features, how profiles are pinned in profileHash, and upgrade pathways without breaking determinism. Complements ADR 004
- ADR 076 — EXPLAIN normalization across targets
  Minimal normalized explain schema for budgets, mapping from target-specific explain outputs, and adapter obligations for units and fields. Complements ADR 023
