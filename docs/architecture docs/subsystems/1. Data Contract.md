...
## Read-only queryables (e.g. views)

Pack-owned blocks like views are target-specific constructs and remain extension-owned. They can still be made queryable in a type-safe way, but **without** embedding a top-level `sources` map (or other provenance-like registry) in the canonical artifact.

**Goals:**

- Keep view definitions in packs (target-specific) under `extensions.<namespace>`
- Enable type-safe querying where the lane/adapter supports it
- Ensure read-only enforcement is explicit (lint/runtime) and deterministic
- Keep canonical artifacts free of authoring provenance and avoid top-level “registries” that are easy to overload

**Current direction:**

- Canonical `contract.json` has **no** top-level `sources` field.
- Any view/queryable surfacing must be derived from extension-owned blocks and adapter/lane capabilities, rather than stored as a canonical root field.
- If we expose additional type surfaces for views, they are emitted as types-only and must not affect hashes.

Historical background and earlier design notes live in [ADR 127 — Views as extension-owned read-only sources](../adrs/ADR%20127%20-%20Views%20as%20extension-owned%20read-only%20sources.md); that ADR should be revisited to reflect the “no top-level `sources` field” guarantee.
