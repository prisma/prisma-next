...
- Separation of concerns
  ...
  - The capability profile is pinned by the contract via `profileHash`; the runtime does not compute a new profile at startup
...
Verification flow:
- `prisma-next verify` connects to the remote database, checks that observed capabilities satisfy the contract's declared requirements, and on success writes the contract's `coreHash` and `profileHash` to the marker.
- The query runtime compares `coreHash` and `profileHash` from the contract (or plan meta) to the database marker and enforces equality.
...
## Tagged content in the contract
