...
- **Family**: SQL ships `abstract class SqlNode extends IRNodeBase`, the family-shared concrete IR-node classes (`SqlStorage`, `SqlTable`, `SqlColumn`, `SqlForeignKey`, `SqlIndex`, `SqlPrimaryKey`, `SqlUnique`) plus the family abstract bases that earn an abstract+target split (`SqlEnumType`). Mongo ships `MongoSchemaNode extends IRNodeBase` and the family-shared collection/index/options classes.
- **Target**: Postgres / SQLite / Mongo target packages ship concrete classes that extend the family abstract bases (`PostgresEnumType extends SqlEnumType`) plus target-only kinds that extend `IRNodeBase` directly when no family parent fits (future `PostgresRlsPolicy`, `PostgresFunction`).
