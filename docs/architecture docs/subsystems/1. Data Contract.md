...
SQL column defaults are encoded as either `literal` values or `db-generated` expressions. The `db-generated` kind represents database-side expressions like `now()` or `gen_random_uuid()` and is stored as a raw expression for DDL and verification.

**Not yet supported:** standardized, cross-target default functions (e.g., `uuid()`) that lower to target-specific expressions. This will be designed alongside application-level default generation.
