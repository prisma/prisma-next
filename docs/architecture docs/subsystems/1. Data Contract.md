...
**Scope:** models/relations; storage (tables, columns, indexes, FK/PK); type identifiers and branded types; capability flags; contract hashing; authoring via PSL or a TS builder; deterministic emission to `contract.json` and `.d.ts` types.
...
To ensure stable hashing and reproducibility across machines and toolchains, emission follows strict canonicalization rules.
...
Core represents portable relational structure, while extension packs add target-specific capabilities and constructs through a standardized encoding.
...
Multiple subsystems read the contract and apply it in different phases; each uses the artifact for verification, planning, or execution in a well-defined way.
...
- **Runtime:** verifies `coreHash` and `profileHash` equality against the database marker; streams results as `AsyncIterable<Row>` with codecs applied per-row for extension types; lints/budgets enforced incrementally during streaming
...
2. Emit artifacts (new `profileHash`, unchanged `coreHash`)
3. Verify against the remote database:
   - `prisma-next verify --db <url>` checks that observed capabilities satisfy the contract
   - On success, the runner writes the contract's `coreHash` and new `profileHash` to the marker (no migration edge)
4. Restart or hot-reload runtimes as needed: caches keyed by `(sqlFingerprint, coreHash, profileHash)` roll naturally
