...
The contract's domain-level structure generalizes across database families. The MongoDB PoC (see [10. MongoDB Family](10.%20MongoDB%20Family.md)) validated that a shared `ContractBase` is viable when domain and storage concerns are separated.
...
- **Embedded types** — embedding is a relation property (`"strategy": "embed"`), not a model property. Embedded models in Mongo and typed JSON columns in SQL are the same contract-level problem. See [ADR 174](../adrs/ADR%20174%20-%20Aggregate%20roots%20and%20relation%20strategies.md).
- **Entity vs value type** — models are entities (have identity and lifecycle). Value types (no identity, defined by their properties) are a separate concept that belongs in a future `types`/`composites` section. See [cross-cutting learnings](../../planning/mongo-target/cross-cutting-learnings.md).
...
- `MongoContract` extends `ContractBase` with Mongo-specific storage (collections, indexes)
