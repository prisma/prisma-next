...
The pattern's worked example is Postgres enums: pre-mechanism, enums lived as codec-typed entries with a per-codec verification + migration glue hook (`pgEnumControlHooks` registered against the Postgres adapter's `descriptor-meta`). With the entities mechanism in place, the Postgres pack contributes `helpers.entities.enum` as the canonical authoring surface; the postgres serializer registers `'type'` in the entries registry and hydrates `kind: 'postgres-enum'` entries into `PostgresEnumType` instances; the SQL verifier walks them natively; the planner consumes the IR directly; `pgEnumControlHooks` was deleted. Future target-specific entity kinds (RLS policies, custom functions, views) follow the same recipe.
