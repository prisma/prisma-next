...
graph LR
  ...
  P["Plan{ sql|pipeline, params, meta(shape, coreHash, profileHash) }"]
  R[Runtime]
  A["Adapter (dialect)"]
...
- **Plan**: immutable execution artifact `{ sql|pipeline, params, meta }` with `meta` including shape/projection, refs, target, `coreHash`, and optional `profileHash`
...
**Runtime**
- Orchestrates Plans through verification and hooks; validates `coreHash`/`profileHash` against the DB marker; composes the codec registry from adapter and packs; streams rows to callers. Contains no dialect or transport logic
...
- The adapter exposes `target`; the runtime verifies `contract.target === adapter.target`
- The driver declares compatibility with the same `target` (or a superset); the runtime verifies `adapter.target === driver.target` (or compatibility)

Specific adapter/driver selection is runtime configuration and is not encoded in the contract. `profileHash` derives from declared capability keys/variants; `coreHash` includes the target selection but not adapter/driver details
...
sequenceDiagram
  ...
  Runtime->>Marker: validate coreHash/profileHash
...
sequenceDiagram
  ...
  Lane-->>Runtime: Plan (shape, coreHash, profileHash?)
