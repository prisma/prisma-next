...
- **Naming and equivalence.** A policy's wire name is content-addressed: the authored name is a prefix, the target appends an 8-hex SHA-256 suffix over the canonical content tuple, and equivalence is wire-name identity — the verifier never compares predicate bodies ([ADR 234](../adrs/ADR%20234%20-%20Content-addressed%20wire%20names%20for%20Postgres-normalized%20objects.md)).
