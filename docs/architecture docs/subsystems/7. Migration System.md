...
Schema-contributing extensions own their own contract spaces, not a node in the application's `SqlSchemaIR`. See [Per-space planner / runner / verifier](#per-space-planner--runner--verifier) below and [ADR 211 — Contract spaces](../adrs/ADR%20211%20-%20Contract%20spaces.md) for how extension schema is planned and verified.
...
1. **On-disk consistency.** For every loaded bundle, recompute `migrationId` from the on-disk manifest + `ops.json` (`verifyMigrationBundle`) and compare against the stored `migrationId`. A mismatch aborts apply with a structured runtime error pointing at the offending directory and asks the developer to re-emit the package (`node migrations/<dir>/migration.ts`) or restore from version control. This catches FS corruption, partial writes, and post-emit hand edits before the runner touches the database.
...
Schema-contributing extensions go further — they own a complete contract space (a `(contract.json, migrations, headRef)` triple) that the framework treats with the same per-space planner / runner / verifier the application uses. See [Per-space planner / runner / verifier](#per-space-planner--runner--verifier) and [ADR 211 — Contract spaces](../adrs/ADR%20211%20-%20Contract%20spaces.md). The legacy `databaseDependencies` mechanism (one-shot init SQL on the extension descriptor) was removed in TML-2397; [ADR 154](../adrs/ADR%20154%20-%20Component-owned%20database%20dependencies.md) is preserved for historical context.
...
The planner runs per space. App-space diffs the prior pinned root contract against the new emitted root contract; each extension space diffs its prior pinned `migrations/<space-id>/contract.json` against its descriptor's current `contractJson`. The producer-side helpers (`planAllSpaces`, `concatenateSpaceApplyInputs`, `emitPinnedSpaceArtefacts`, `detectSpaceContractDrift`) live in `@prisma-next/migration-tools/exports/spaces` as target-agnostic primitives — the SQL family wires them at consumption sites. Plan-time codec hooks ([ADR 212](../adrs/ADR%20212%20-%20Codec%20lifecycle%20hooks.md)) emit app-space-bound ops alongside the user's own structural ops.

### Per-space runner: `executeAcrossSpaces`

`SqlMigrationRunner` exposes three entry points (per ADR 211 §"Apply-time atomicity and ordering"):
...
A migration always exists or it doesn't — there is no on-disk "draft" state. Every migration package on disk is fully attested: `migration.json` carries a non-null `migrationId` content-addressed over `(manifest, ops)`, and `ops.json` is always written (as `[]` when the planner could not lower any calls because of placeholders).

- Attested: `migrationId` matches current content; optional signature present (the only on-disk state)
...
3. Emit: run `./migration.ts` (or `node <dir>/migration.ts`) → rewrites `ops.json` with the authored operations and rewrites `migrationId` to match
...
- `prisma-next migration plan [--from <hash>] --name <slug>` — diff contracts and write a fully attested package (`migration.ts` + `migration.json` + `ops.json`) offline. Re-run `./migration.ts` after filling any `placeholder(...)` slots to rewrite `ops.json` and the `migrationId`.
...
- Concatenate the per-space paths and apply them inside `executeAcrossSpaces` (single outer transaction).
...
- Any edit to `ops` changes `migrationId`; re-run `./migration.ts` (or `node <dir>/migration.ts`) to re-attest. Non-storage contract edits do not change `migrationId`. See [ADR 199](../adrs/ADR%20199%20-%20Storage-only%20migration%20identity.md).
