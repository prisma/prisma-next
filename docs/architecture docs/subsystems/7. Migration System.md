...
- `prisma-next migrate --db <url> [--to <contract>] [--advance-ref <name>]` — execute pending migrations against a live database. Ref advancement is **opt-in only** via `--advance-ref`; plain `migrate` does not advance any ref. The `<contract>` argument accepts the full [contract-reference grammar](#contract-references-and-migration-references): hash / prefix, ref name, migration directory name, `<dir>^`, or filesystem path.
...
- `MIGRATION.MARKER_MISMATCH` — live DB marker hash is not reachable in the on-disk graph (apply-time, pre-DDL)
