...
`strippedManifest` contains: `from`, `to`, `labels`, `authorship?`, `createdAt`. These plus `ops` are the full input to the hash. `from` is `string | null`: a non-null value is the prior-state storage hash, and `null` denotes a baseline edge with no prior storage state. `to` carries the destination storage-projection commitment.
