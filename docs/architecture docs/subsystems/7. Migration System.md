...
```mermaid
flowchart LR
  E["∅"] -- "e1:createCollection(users)" --> H1[H1]
```
...
flowchart LR
  ...
  ts -->|"migration emit / self-emit"| ops
...
5. **Emit.** `migration emit` (or the inline emit at the end of `migration plan`) dynamically imports `migration.ts` in-process via the target's `emit` capability. The target instantiates the class, reads `operations`, writes `ops.json`. The framework then computes and persists the content-addressed `migrationId`. See [ADR 196 — In-process emit for class-flow targets](../adrs/ADR%20196%20-%20In-process%20emit%20for%20class-flow%20targets.md).
...
Before applying, the runner must verify that the `migrationId` stored in `migration.json` matches the recomputed hash of the manifest + ops. This prevents tampered or corrupted `ops.json` from being applied silently. This check is architecturally required but not yet implemented.
