...
- `prisma-next migration status [--db <url>] [--ref <name>] [--space <id>] [--graph] [--limit <n>] [--all]` — answer "what will `migration apply` do?". By default focuses on the application's contract space and additionally lists any loaded extension space with `pendingCount > 0`; pass `--space <ext-id>` to render an extension space's graph / history instead. `--ref`, `--graph`, `--limit`, and `--all` all scope to the focused space; `--ref` is app-space only and combining it with `--space <non-app>` is rejected (`PN-CLI-5021`). `--json` always returns the full per-space aggregate regardless of `--space`.
