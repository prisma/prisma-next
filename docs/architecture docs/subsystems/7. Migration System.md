...
5. **Emit.** `migration.ts` produces `ops.json` by **self-emission**: running the file via `./migration.ts` (shebang) or `node migration.ts` causes `Migration.run(...)` to detect it is the main module and write artifacts in-process. The framework then computes and persists the content-addressed `migrationId`. Self-emission is the only path that produces `ops.json`. See [ADR 196 — In-process emit for class-flow targets](../adrs/ADR%20196%20-%20In-process%20emit%20for%20class-flow%20targets.md).
...
1. Edit contract
2. Plan: `prisma-next migration plan` (see `migration-plan.ts`) → scaffolds `migration.ts` (Draft) and hints the developer to self-emit. If any operation still contains an unfilled `placeholder(...)`, the command short-circuits into a successful `pendingPlaceholders` envelope (a warning, not a failure) asking the developer to fill in the slots before self-emitting. `PN-MIG-2001` is **not** raised here — it is raised only when the developer subsequently runs the draft `migration.ts` (self-emit) with an unfilled `placeholder(...)`.
3. Emit: run `./migration.ts` (or `node <dir>/migration.ts`) → writes `ops.json` and attests `migrationId` (Attested). Unfilled `placeholder(...)` calls fail here with `PN-MIG-2001`.
...
3. Emit: run `./migration.ts` (or `node <dir>/migration.ts`) → writes `ops.json`, computes `migrationId` (Attested)
...
- `prisma-next migration plan [--from <hash>] --name <slug>` — diff contracts and scaffold `migration.ts` (offline). Run `./migration.ts` to emit `ops.json` and attest `migrationId`.
