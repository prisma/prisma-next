...
1. Edit contract
2. Plan: `prisma-next migration plan` → scaffolds `migration.ts` (Draft). If the plan has no placeholders, the CLI also hints the developer to self-emit; if placeholders remain, the CLI reports `PN-MIG-2001` and the developer fills them in.
3. Emit: run `./migration.ts` (or `node <dir>/migration.ts`) → writes `ops.json` and attests `migrationId` (Attested)
