...
- `prisma-next migrate --db <url> [--to <contract>]` — execute pending migrations against a live database. The `<contract>` argument accepts the full [contract-reference grammar](#contract-references-and-migration-references): hash / prefix, ref name, migration directory name, `<dir>^`, or filesystem path.
...
- `prisma-next migration show [<migration>]` — inspect a single migration's operations and metadata. The `<migration>` argument accepts the [migration-reference grammar](#contract-references-and-migration-references).
...
- `migration check <migration>` — per-migration. Validates a single named package via the [migration-reference grammar](#contract-references-and-migration-references).
