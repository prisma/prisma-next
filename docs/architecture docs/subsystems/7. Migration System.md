...
`migration plan` is fully offline (no database connection). It converts the "from" contract to a `SqlSchemaIR` via `contractToSchemaIR(SqlStorage)` in the SQL family tooling layer, then feeds it into the existing `PostgresMigrationPlanner`. This reuses the same planner that `db init` uses — no second diff engine is required. The conversion is intentionally lossy (drops codec metadata), but the planner only needs structural information. See [ADR 169](../adrs/ADR%20169%20-%20On-disk%20migration%20persistence.md).
