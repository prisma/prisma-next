...
migrations/
  20260118T1205_add_users/
    migration.json            # from/to hashes, migrationId, parentMigrationId, embedded contracts
...
### Parent-pointer chain ordering

Each migration carries a `parentMigrationId` field: the `migrationId` of the migration it follows in the chain, or `null` for the first migration. This creates a singly-linked list that provides unambiguous ordering even when contract hashes are revisited (e.g., `A → B → A`). Without parent pointers, node-level graph analysis cannot distinguish which "version" of a revisited hash a migration was planned against.

Branch detection: two migrations sharing the same `parentMigrationId` are a structurally detectable branch. The system reports `MIGRATION.AMBIGUOUS_LEAF` and requires explicit resolution. See [ADR 169 — On-disk migration persistence](../adrs/ADR%20169%20-%20On-disk%20migration%20persistence.md).
