...
Migrations form a DAG via their `fromStorageHash` / `toStorageHash` edges. Ordering is determined by shortest-path traversal from a source hash to a target hash, with deterministic tie-breaking when multiple shortest paths exist. Branch detection: two migrations sharing the same `fromStorageHash` but different `toStorageHash` are a structurally detectable fork. The system reports `MIGRATION.AMBIGUOUS_LEAF` and requires explicit resolution. See [ADR 169 — On-disk migration persistence](../adrs/ADR%20169%20-%20On-disk%20migration%20persistence.md).
