...
`migration plan` is fully offline (no database connection). It converts the "from" contract to a `SqlSchemaIR` via `contractToSchemaIR(contract, { expandNativeType, frameworkComponents })` in the SQL family tooling layer, then feeds it into the existing `PostgresMigrationPlanner`. This reuses the same planner that `db init` uses — no second diff engine is required. The conversion reads `contract.storage` for tables, derives database extensions from `frameworkComponents` (each component's `databaseDependencies.init[].extension`), and populates type annotations from `contract.storage.types`. Codec metadata is intentionally dropped since the planner only needs structural information. See [ADR 169](../adrs/ADR%20169%20-%20On-disk%20migration%20persistence.md) and [ADR 154](../adrs/ADR%20154%20-%20Component-owned%20database%20dependencies.md).
