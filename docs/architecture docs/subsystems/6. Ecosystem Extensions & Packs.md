...
#### The optional `spaceId` field on FK reference carriers

The FK reference carrier in Contract IR (`ForeignKeyReference`) uses a presence-based optional `spaceId` to distinguish local from cross-space references:
...
`spaceId` absent means the reference is local — the referenced table is in the same contract-space. `spaceId` present means the reference is cross-space — the referenced table is in the contract-space identified by `spaceId`. This keeps local-FK JSON byte-identical to contracts authored before cross-space support was added. When the planner encounters a `spaceId`-present carrier, it looks up the named space in the loaded contract aggregate to resolve the target's exact table/column coordinates.
