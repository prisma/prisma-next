...
`packages/3-extensions/test-contract-space/` is a private workspace package that mirrors pgvector's package shape and is consumed by integration tests to exercise the per-space planner / runner / verifier; it's a useful starting point for a new extension.
