...
- **`packages/3-extensions/better-auth/`** — the **managed-space (table-DDL-shipping) precedent**: its space contract declares four domain models (`User`, `Session`, `Account`, `Verification`) with tables, uniques, and cascading FKs, and ships the planner-emitted baseline migration that creates them — the framework owns the auth tables' whole DDL lifecycle. Apps reference the models through branded cross-space FK handles (`/contract`) and integrate BetterAuth through a contract-typed adapter (`/adapter`); a descriptor-only `/runtime` export satisfies the aggregate client's requirement check.
...
See `packages/3-extensions/pgvector/` and `packages/3-extensions/paradedb/` for the canonical flat on-disk layout (`migrations/refs/head.json`, `migrations/<dirName>/...`, `src/contract.{ts,json,d.ts}`, `prisma-next.config.ts` at the package root), and `packages/3-extensions/supabase/` / `packages/3-extensions/better-auth/` for the PSL-authored `src/contract/` grouped variant (see the layout amendment in [ADR 212](../adrs/ADR%20212%20-%20Contract%20spaces.md)). See [`.cursor/rules/contract-space-package-layout.mdc`](../../../.cursor/rules/contract-space-package-layout.mdc) for the rule spelled out.
