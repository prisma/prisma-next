...
#### The `source` discriminator on FK reference carriers

The FK reference carrier in Contract IR carries a `source` discriminator:
...
`source: 'local'` is what within-space cross-namespace FKs produce. `source: 'space'` adds the explicit `spaceId` plus namespace coordinate so the reference is fully qualified without lexical context. When the planner sees `source: 'space'`, it looks up the named space in the loaded contract aggregate to resolve the target's exact table/column coordinates.

See [ADR 226](../adrs/ADR%20226%20-%20Cross-contract%20foreign-key%20references.md) for the full decision record.
...
Cross-space resolution happens at the lowering boundary (Contract IR → Schema IR). The same aggregate the framework already builds from `extensionPacks` is the resolver — there is no separate PSL `use` directive or TS resolver call. If the named space is not in the aggregate, lowering fails fast with a diagnostic naming the missing pack.

A future `use ... as` aliasing directive is reserved as additive on top of this implicit resolution; it is not required for v0.1.
...
export const contract = defineContract(
  { family: sqlFamily, target: postgresPack, namespaces: ['public'], extensionPacks: [supabasePack] },
  ({ field, model }) => {
...
The `AuthUser` handle is branded with `spaceId: 'supabase'` — that brand is what causes `constraints.foreignKey` to lower to a `source: 'space'` carrier. No separate call surface (`refExt`, `belongsToExternal`) exists.

See [`.agents/rules/cross-contract-fk-authoring.mdc`](../../../.agents/rules/cross-contract-fk-authoring.mdc) for the authoring conventions rulecard.

#### `__unspecified__` and DDL qualification

The `source: 'space'` carrier records the target model's declared namespace. The planner uses that to decide whether to emit a qualified or unqualified `REFERENCES` clause:
...
This is symmetric with the table-creation DDL rule for `__unspecified__` introduced alongside contract spaces.

#### Cross-space relations are declared but non-navigable in v0.1

Both PSL (`user supabase:auth.AuthUser @relation(...)`) and TS (`rel.belongsTo(AuthUser, ...)`) declare a relationship, not merely a column constraint. The emitter renders cross-space relations so that ORM traversal (`db.public.Profile.find({ include: { user: true } })`) is a compile-time error in v0.1. The FK drives the database constraint (referential integrity and cascade); the query/traverse semantics are deferred.
