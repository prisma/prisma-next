...
6. **Mongo** — out of scope for *this* ticket's code; in scope for the model. Mongo already
   ...
   inline, self-contained validator); type document fields from the value set + codec. Not
   blocked (Mongo enums landed in #834). Could be folded into a Mongo milestone.
...
- **A4 — Output unchanged for identity codecs.** For the demo/fixtures (text-backed enums),
  ...
  **byte-identical** to before. `fixtures:check` shows no observable type change;
...
- Field→type connection: already solved by the existing `FieldOutputTypes` map + field→column
  mapping; the refactor changes the computation, not the connection.
