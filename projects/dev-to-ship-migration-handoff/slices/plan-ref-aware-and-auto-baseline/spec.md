...
1. **Bundle ordering for auto-baseline.** The two bundles must sort baseline-first by directory name. Options: (a) same timestamp, distinct slugs sorted lexicographically (`baseline-...` < `migration-...`); (b) different timestamps (baseline = `now`, delta = `now + 1ms`); (c) explicit ordinal in the directory name. Working position: (b) — least surprising for users reading `git log` later, and the framework already uses timestamp-prefixed names. Decide at dispatch time after grepping `formatMigrationDirName` to confirm the prefix format makes (b) viable.
