...
| `deleteRefPaired` — pointer missing, snapshot present (orphan from partial earlier delete) | **Handle** | Self-healing: delete the orphan snapshot, return success. Matches § Approach's "recoverable by re-running" framing. Test covers. |
| `deleteRefPaired` — pointer missing, snapshot also missing (genuine typo / non-existent ref) | **Handle** | Throw `MIGRATION.UNKNOWN_REF` (preserves existing `deleteRef` semantics for the genuine-typo case; doesn't silently no-op a delete on a name that never existed). Test covers. |
