...
| `deleteRefPaired` called on a non-existent ref | **Handle** | Existing `deleteRef` throws `MIGRATION.UNKNOWN_REF` — preserve that behaviour at the paired layer (don't silently no-op a delete on a non-existent ref). Test covers. |
