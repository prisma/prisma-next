...
| `isGraphNode(EMPTY_CONTRACT_HASH, graph)` on any graph (empty or non-empty) | **Handle** | Returns `true` — always reported as a node by `isGraphNode` (which short-circuits the sentinel before consulting `graph.nodes`; `reconstructGraph([])` yields a zero-node graph, so the predicate's short-circuit is what guarantees the universal-baseline semantics). Test covers both empty + non-empty graphs. |
