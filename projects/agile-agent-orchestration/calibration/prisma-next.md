...
**Reference incident.** 2026-05-18, TML-2520 M5c Phase 3 — orchestrator (this calibration's author) proposed a `FlatTablesOf<C>` bridge inside `sql-builder` to defer the emitter migration to TML-2550, claiming the bridge would contain the substrate change. (Continued in § 3.14.) Composer-2's bridge dispatch (`c2903c314`) executed the brief correctly and cleared `@prisma-next/sqlite` typecheck (the original visible failure). The cascade then unmasked 7 sibling packages failing with the same root cause — `@prisma-next/postgres`, `@prisma-next/sql-orm-client`, plus 4 demo apps and `paradedb-demo` — each one a consumer passing a generated flat-shape `Contract` to a function expecting `Contract<SqlStorage>` (nested). The bridge couldn't shield those consumers because `Contract<TStorage>.storage.tables` is a framework-parameterised type, not a DSL-local one. Correction: emitter migration brought into PR2 scope as Phase 3; bridge in `sql-builder` retained for DSL ergonomics only; full DSL redesign deferred to TML-2550 (which is the genuinely separable part — the bridge's `Db<C>` flatten could have been scoped to "contained" honestly because `Db<C>` is exposed by sql-builder only).

### 3.14 Orchestrator drifts into implementation after a subagent dies mid-dispatch

**Symptom.** A long-running subagent terminates without sending its final report (killed by environment, transcript timeout, OOM, etc.). The orchestrator picks up its own thread of "I'll just check the DoD myself" → "while I'm here I'll fix the one obvious issue" → "and the cascade error from that" → and ends up performing the next M-sized dispatch directly across multiple files and packages over 30+ minutes, never re-dispatching.

**Why this is structurally worse than running the dispatch.**

- The orchestrator's value is in *holding the wider plan, calibration debts, scope-control gates, and dispatch routing decisions*. Every minute spent on implementation is a minute not spent on those.
- Implementation work performed by the orchestrator escapes the dispatch's brief-discipline (no recon, no DoR/DoD, no anti-pattern guards, no time-box) and therefore tends to drift into the exact failure modes the protocol exists to prevent.
- The orchestrator role doesn't accept anti-pattern guards from itself the way a dispatch does — there's nobody to interrupt at T+10 when the orchestrator is iterating reactively.
- It also makes the next subagent dispatch harder: the next implementer has to triage a half-finished work product instead of starting from a clean slice.

**Detection signal.**

- Subagent terminates without the structured 5-line report. Orchestrator's first reaction is "let me just check DoD" rather than "let me dispatch a finisher."
- Orchestrator makes more than ~2 tool calls in a row that edit source/test files rather than the project/calibration/spec docs.
- The orchestrator's wall-clock since the dead subagent crosses 15 min with zero new dispatches.

**Mitigation.**

- **Hard rule: when a subagent dies mid-dispatch, the orchestrator's very next action is to dispatch a *finisher subagent* — never to "just verify state and tidy up."** If the remaining work is genuinely < 5 min of bracket-notation edits the orchestrator is willing to time-box and immediately return to orchestration mode, that's permitted, but the standard is "dispatch."
- The finisher brief takes the SAME shape as a fresh dispatch: recon, scope boundaries, anti-pattern guards, DoR/DoD gates, time-box, structured report-back. The brief explicitly enumerates the wins the dead subagent landed (so the finisher doesn't re-do them) and the known remaining failure surface (so the finisher doesn't start from scratch).
- If the orchestrator catches itself mid-drift (e.g. at the third source-file edit in a row), it must stop, commit whatever is committable as a cleanly-scoped slice, and dispatch from the new HEAD.
