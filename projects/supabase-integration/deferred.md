...
- **Pack ownership of `realtime` / `vault` / `pgsodium` / `extensions` / `graphql*` schemas.** The v0.1 pack contract declares `auth` + `storage` only — the schemas users actually integrate with; the rest is platform-internal machinery with no user-facing data surface (and no FKs from owned space into them, verified against supabase/postgres 17.6.1.106). They stay undeclared: `db verify` tolerates them as extras under `external`, and extension-aware `contract infer` must never leak them into a user's contract (extension-supabase Slice F tests both properties).
