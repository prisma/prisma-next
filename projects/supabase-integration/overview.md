...
5. **Runtime target-layer class.** [runtime-target-layer](../../docs/architecture%20docs/adrs/ADR%20230%20-%20Runtime%20target%20layer%20session-coupled%20connections.md). Export `SqlRuntime`; introduce `class PostgresRuntime extends SqlRuntime` (the missing target layer); add `protected withRawConnection(callback)` below the user middleware chain. The substrate Supabase's runtime uses to enforce role binding structurally rather than procedurally.
