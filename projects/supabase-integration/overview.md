...
1. **Control policy: managed / tolerated / external / observed.** A generic, target-agnostic `control` field on IR nodes that declares how much the framework's control plane participates in each object's migration lifecycle. The Supabase contract declares `auth.users` with `control: 'external'`; the verifier checks it exists with the expected shape, the planner emits no DDL for it. **This is its own project — see [`projects/control-policy/spec.md`](../control-policy/spec.md).** Supabase consumes it; it does not introduce it.
...
- **TML-2459 carries the IR machinery; control-policy adds the dispatch primitive.** None of the IR work below requires inventing new framework infrastructure beyond what TML-2459 (3-layer IR + SPIs) and the control-policy project (per-node `control` field + verifier/planner dispatch) provide. This project adds new IR *kinds* (Postgres RLS policy, framework-level cross-space FK carrier) within the established shape.
