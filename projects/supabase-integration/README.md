...
| [`cross-contract-refs.md`](cross-contract-refs.md) | Cross-contract-space FK references. TS: unified surface — model handles value-imported from extension `/contract` subpaths (e.g. `import { AuthUser } from '@prisma-next/extension-supabase/contract'`; then `AuthUser.refs.id`) work with existing `constraints.foreignKey` / `rel.belongsTo` call sites. PSL: colon-prefixed dot-qualified type refs (`supabase:auth.User`). Implicit resolution via `extensionPacks`. Ownership rules, dependency graph, `__unspecified__` × cross-contract DDL. |
| [`rls.md`](rls.md) | RLS policies as first-class Postgres IR. TS: `.rls([...])` staged-builder method, array of named descriptors. PSL: top-level `policy <name> { ... }` named-block declarations. Migration ops via `OpFactoryCall`; verifier against `pg_policies` using content-addressed wire names ([C9/C10](decisions.md)). |
| [`extension-package.md`](extension-package.md) | The `@prisma-next/extension-supabase` package shape. Subpath-only entrypoints (`/pack`, `/contract`, `/runtime`) per [C6](decisions.md). Hand-authored `contract.json` shipped under `/contract` alongside hand-written typed handles. `SupabaseRuntime` is a `PostgresRuntime` subclass ([C12](decisions.md)) exposing `asUser` / `asAnon` / `asServiceRole` role helpers; `SET LOCAL` issued below the user-middleware chain (structurally non-bypassable). |
...
- Cross-namespace FK references **within a single contract space** via the existing `constraints.foreignKey(cols.x, OtherModel.refs.y, …)` and `rel.belongsTo(OtherModel, …)` call sites — the target model handle carries the namespace coordinate.
