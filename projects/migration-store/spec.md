...
- [ ] No second *representation* of migration state leaks into a shared package: any command-render view-model (e.g. `migration list`'s `MigrationListEntry`) is CLI-private presentation, not exported from `@prisma-next/migration-tools`; mappings from the aggregate live in the consuming command. The CLI fallback used when the live app contract is unreadable is named for what it carries (an identity-only app-contract stand-in), with a cast reason stating read commands consume only its storage hash + target. (See [`design-decisions.md` § DD-1](./design-decisions.md).)
