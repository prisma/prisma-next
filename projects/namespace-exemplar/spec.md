...
- [x] **AC3. SQLite + Mongo reject explicit namespace blocks.** Authoring `namespace foo { model X { … } }` in a SQLite or Mongo PSL contract fails interpretation with a diagnostic naming the target and pointing at the offending block span. Implicit (top-level, no block) declarations continue to work.

- [x] **AC4. Postgres reserves user-declared `unbound`.** Authoring `namespace unbound { … }` in a Postgres PSL contract that also declares a sibling `namespace public { … }` fails interpretation with a diagnostic explaining that `unbound` is reserved for the late-binding sentinel mapping and naming the offending block span.

- [x] **AC5. Multi-tenancy via `namespace unbound { … }`.** A Postgres contract declares `namespace unbound { … }` and emits unqualified DDL. A live PGlite test sets `search_path` to a per-tenant schema, runs the contract's migration plan against it, and verifies that tables are created in the tenant schema and FKs resolve correctly. Single-tenant Postgres contracts (top-level models, no `unbound` block) continue to lower to `public` and emit qualified DDL — verified separately.

- [x] **AC6. All in-repo demos + extensions emit cleanly.** `pnpm fixtures:check` exits 0 (no diff after `fixtures:emit`). Every example app's `contract.d.ts` reflects the canonical shape. `examples/multi-extension-monorepo`'s aggregate `build:contract-spaces` succeeds end-to-end and produces a namespace-keyed app contract.
