...
**Type-level assertions are ported, not dropped.** prisma-next has a typed public surface (the emitted `contract.d.ts` + the ORM's inferred result/where/create types) and first-class type-test infrastructure: `vitest typecheck` runs `test/**/*.test-d.ts`, and `expectTypeOf` / `@ts-expect-error` are available (negative type tests are explicitly sanctioned by CLAUDE.md). So an upstream test's type assertions have a faithful home:
- Put the type assertions in a sibling `test/ports/prisma/functional/<suite>.test-d.ts` (they are typechecked, not run). Keep the runtime assertions in the `.test.ts`.
- `expectTypeOf(result).toMatchTypeOf<{ id: string; name: string }>()` / `.not.toBeAny()` → assert the prisma-next ORM's inferred row type the same way (see `test/integration/test/dsl-type-inference.test-d.ts` for the pattern: `expectTypeOf<Row>().toHaveProperty(...)`, `.toEqualTypeOf<...>()`).
- Upstream `@ts-expect-error` on an invalid query (wrong field, wrong operator, wrong type) → `@ts-expect-error` on the equivalent invalid prisma-next ORM call. A whole `query-validation`-style suite of `@ts-expect-error` cases ports to a `.test-d.ts` of the equivalent invalid ORM calls — it is **portable**, not "type-only → non-ported".
...
- Porting is done by parallel **Sonnet 5 implementer sub-agents**, each given **at most 10 source files** per batch, with the faithfulness contract and accounting rules above in their brief.
...
- After each implementer batch, an **Opus reviewer sub-agent** evaluates every ported test in the batch against the faithfulness criteria (diffing against the upstream source). If unsatisfied, the review is forwarded to the implementer for fixes; the loop repeats until the reviewer is satisfied. A batch is done only on reviewer satisfaction.
