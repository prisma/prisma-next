...
> **Be the user.** The bug was discovered by `cd`-ing into the demo and running real `prisma-next` CLI commands. This script does the same: every scenario operates the demo through its actual CLI surface and observes what a developer using Prisma Next would see.
>
> **Out of scope of this script.** Re-running the unit tests, e2e tests, the `migration:plan:check` regression script, or `lint:no-contract-cast` against today's clean tree. Those run in CI; passing them again here proves nothing a real user would catch. The scenarios that touch the lint and the strict deserializer use **negative controls** (plant a violation, prove the gate fires, restore) — that's the only legitimate way to exercise a gate from the user's seat.
>
> **Spec:** `projects/tml-2536-contract-deserializer-seam/spec.md`
> **Plan:** `projects/tml-2536-contract-deserializer-seam/plans/plan.md`
> **PR:**  (re-cut from PR #520; new PR opened at D7)
...
3. The test-coverage gap is closed: per-`kind` snapshot-read fixtures (`codec-instance`, `postgres-enum`), a workspace lint that rejects `as Contract` in production code, a `.cursor/rules/` rule pair documenting the seam and the smell, and a new CI job that runs the demo's `migration plan` against its checked-in history.
4. Both upgrade-instruction skills (`prisma-next-upgrade` for end users; `prisma-next-extension-upgrade` for extension authors) ship a `0.9-to-0.10/` entry with a codemod that stamps the `kind` discriminator on every committed `*-contract.json` snapshot. This is a breaking change for any project (app or extension package) with committed migration history under 0.9; the codemod is the only supported path forward.
...
| - | -------- | -------------- | ------ |
| 1 | Greenfield init + schema verification | A developer can init a fresh DB from the demo contract and every status command reports healthy | AC-1, AC-4, AC-5 |
| 2 | Schema-evolution journey (the daily developer loop) | Plan → new → apply → status → verify all round-trip through the serializer cleanly when the user edits the schema | AC-1, AC-2 (observed), AC-5 |
| 3 | Replay the original-bug surface against checked-in history | The literal CLI flow from the original bug report now succeeds end-to-end | AC-1, AC-4 |
| 4 | Strict deserializer fires loudly on a corrupted on-disk shape **(negative control)** | A `storage.types[*]` entry with no `kind` produces a clear diagnostic at the deserializer boundary, not a downstream crash | AC-3 |
| 5 | Repo lint gate catches a freshly-planted `as Contract` cast **(negative control)** | The lint would reject a future PR re-introducing the bypass, not just pass against today's clean tree | AC-8 |
| 6 | Rules read coherently to a fresh developer **(judgement)** | The two `.cursor/rules/*.mdc` files name the serializer seam and the `as Contract` smell unambiguously to a cold reader | AC-6, AC-7 |
| 7 | Demo CI gate is wired into PR checks | The `demo-migration-plan` job runs on every PR, so future demo-rot is caught before merge | AC-10 |
| 8 | Demo app boots and serves against a signed database **(judgement)** | The runtime contract-load path still works — we didn't accidentally break the product to fix the CLI | (broad confidence; AC-4 indirectly) |
| 9 | Exploratory: probe the contract-read diagnostic surface **(exploratory, charter)** | Surfaces unanticipated degradation modes and judges diagnostic quality across them | (no specific AC; charter) |
| 10 | End-user upgrade journey via `prisma-next-upgrade` 0.9→0.10 entry | A real user with committed migration history under 0.9 can apply the codemod and successfully advance to 0.10 from their app | (upgrade-instructions intent) |
| 11 | Extension-author upgrade journey via `prisma-next-extension-upgrade` 0.9→0.10 entry | An extension author can apply the codemod to seed migrations AND apply the source-level rules to their TypeScript without their build / tests breaking | (upgrade-instructions intent) |

> Scenarios marked **(negative control)** plant a violation, observe the gate fire, then restore. Scenarios marked **(judgement)** require human evaluation that no test can assert. Scenarios marked **(exploratory)** are time-boxed charters with no scripted steps. Scenarios 10 and 11 are **upgrade-journey** scenarios — they exercise the breaking-change-mitigation surface (the codemods + prose) from the consumer's point of view.
...
1. Checkout + build the branch from repo root:
   ...
   pnpm build
   ```
2. Confirm a local Postgres is reachable and your demo `.env` is set:
   ```bash
   cd examples/prisma-next-demo
   cat .env   # expect DATABASE_URL=postgres://...
   ```
   If `.env` is missing, copy/edit one before continuing — every scenario below needs it.
3. Confirm `git status` is clean. The scenarios mutate the tree in a few spots; you'll be asked to restore each time. A pre-clean baseline makes "did I restore everything" obvious.
4. The CLI binary in this workspace lives at `node_modules/@prisma-next/cli/dist/cli.js` inside the demo (pnpm workspace link). All scenarios use:
   ```bash
   CLI="node node_modules/@prisma-next/cli/dist/cli.js"
   ```
   Set that once in your shell so the rest reads naturally.
...
**Oracle:** the demo's checked-in `migrations/app/20260422T0720_initial/end-contract.json` represents the canonical contract state after init. After `db init` against the demo's current `contract.prisma`:
- `migration status` should report that checked-in migration as the current head.
- `migration plan` should propose zero operations (the live DB now structurally matches the contract that produced the checked-in migration).
...
- `git status` clean.
- No previous scenario state to restore.
...
- `migration status` reports the demo's checked-in head migration as the current state.
- `migration plan` reports a no-op (no proposed operations).

### Failure modes

- Any `PN-CLI-4999`, `PN-CLI-49xx`, or "failed to deserialize" / "missing kind" error from any of the commands above.
...
No state to restore beyond the database, which will be re-baselined by Scenario 2 or 3. Confirm `git status` is clean before moving on.

---

## Scenario 2 — Schema-evolution journey (the daily developer loop)

**What you're proving:** a developer editing the schema, planning, authoring, and applying a new migration walks through the CLI without hitting the bug class TML-2536 fixed. Every step reads or writes an on-disk `end-contract.json` or `contract.json`; every read crosses the serializer seam.

**Covers:** AC-1, AC-2 (observed indirectly — if any command still bypassed the serializer, behaviour would diverge from a real deserialization round-trip), AC-5
...
- `git status` clean apart from any pending DB state.
...
- Any command produces a deserialization-flavoured diagnostic (`PN-CLI-4999`, "failed to deserialize", "missing kind", etc.).
...
**Oracle:** the pre-bug expected behaviour — `migration apply` against the demo's checked-in `20260422T0720_initial` should run cleanly, `migration plan` should report a no-op, and `migration show` should print the migration's bookend contracts without error. Pre-fix, this exact sequence produced `PN-CLI-4999`; post-fix it must match the pre-bug-introduction behaviour. The checked-in migration *is* the artefact that originally triggered the bug, so re-running this flow against it is the bug-report-fidelity check.
...
pnpm db:drop
pnpm emit
$CLI migration apply                            # applies the checked-in initial migration
$CLI migration status
$CLI migration plan                             # no-op
$CLI migration show 20260422T0720_initial       # inspect the checked-in migration
$CLI db verify
```

### What you should see

- `migration apply` runs the checked-in `20260422T0720_initial` migration cleanly. It reads `end-contract.json` from disk — the exact code path TML-2536 broke.
- `migration status` reports the migration as applied and the head as up-to-date.
- `migration plan` reports a no-op.
- `migration show` prints the migration's operations + contract bookends without errors.
- `db verify` passes.

### Failure modes

- Any `PN-CLI-4999` or deserialization error from `migration apply`, `migration plan`, `migration status`, or `migration show`. That *is* TML-2536 reproducing.
- An unhandled exception (rather than a structured envelope) from any of the four commands.
...
**What you're proving:** the strict `normaliseTypeEntry` actually behaves the way the spec promised — a `storage.types[*]` entry missing the `kind` discriminator now produces a clear diagnostic at the deserializer boundary instead of silently slipping past and crashing the planner later. This is the *behavioural* proof of AC-3 from a user vantage point, not a unit-test re-run.

**Covers:** AC-3

**Coverage boundary.** This scenario proves the strict deserializer fires on **one** specific corruption: a known codec-instance entry (`Embedding1536`) with its `kind` field stripped, read via `migration plan` and `migration show`. It does **not** prove every possible malformed `storage.types` shape is rejected, every kind is covered, every command that reads a snapshot rejects it, or that the diagnostic is well-formed under arbitrary mutation. The exploratory charter (Scenario 9) probes broader degradation modes; the unit tests for `normaliseTypeEntry` and the snapshot-read fixtures (AC-9) own the per-kind invariant.
...
1. From the demo dir, with a clean tree, pick a checked-in `end-contract.json` and back it up:
   ```bash
   cd examples/prisma-next-demo
   END=migrations/app/20260422T0720_initial/end-contract.json
   cp "$END" "$END.bak"
   ```
...
4. Try a different command for a second data point:
   ```bash
   $CLI migration show 20260422T0720_initial
   echo "exit=$?"
   ```
...
- Either command throws an unhandled exception with no entry name in the diagnostic (regression on diagnostic quality, even if the throw itself is correct).
- The diagnostic names the entry but not the file path (a partial regression on actionability).
...
**What you're proving:** the durable record of the decision lives in two `.cursor/rules/*.mdc` files. A developer hitting an `as Contract` cast in a future PR should be able to read those rules and immediately understand both *why* it's a smell and *what* to do instead, without going back to TML-2536.
...
**Preconditions:**
- Working copy of `main`/branch checked out so the rule files are accessible.
...
- Job present in the workflow but not appearing in `gh pr checks 520` output (something is preventing the live run).
...
- Pre-flight complete.
- `git status` clean in `examples/prisma-next-demo/migrations/app/`.
- A copy of the intact `migrations/app/20260422T0720_initial/` somewhere outside the repo (or a `git stash`) so restoration after any mutation is one command.

**Notes capture.** Write what you tried, what surprised you, and anything that "felt off" but you can't yet name (e.g. "the diagnostic was technically correct but I didn't realise which file it meant until I re-read it"). Findings from this charter get severity-classified in the report the same way scripted-scenario findings do — but a finding here is often "diagnostic X is unhelpful in shape Y", which the runner records as a Follow-up unless it masks a structural problem.
...
**What you're proving from the user's seat:** an end user with a 0.9 app and committed migration history can run the user-facing upgrade skill, apply the `0.9-to-0.10/stamp-storage-types-kind` codemod, and successfully end up with a project whose strict deserializer accepts every snapshot — the precise breaking-change scenario this PR ships.
...
- Re-running `migration plan` against the demo now surfaces the snapshot read crossing the seam successfully (TML-2536's deserializer crash is gone).

The "migration plan no-op" part is *not* the oracle — the demo's authored `src/prisma/contract.json` carries an unrelated `storage.namespaces` field added by TML-2521 that has no follow-up migration. That residual `PN-CLI-4020` is pre-existing and out of scope for this scenario. The oracle is the *deserializer* crossing the seam without crashing.

**Preconditions:**
- Pre-flight complete.
- `git status` clean.
- Pre-codemod state of the demo migrations available (either via `git stash push examples/prisma-next-demo/migrations/` after a checkout-then-revert, or via `git checkout HEAD~N -- examples/prisma-next-demo/migrations/` to a pre-codemod commit).
...
2. Confirm the pre-codemod state actually surfaces TML-2536 cleanly under the strict deserializer:
   ```bash
   cd examples/prisma-next-demo
...
3. Read the user-skill entry's prose as if you have just been pointed at it for the first time:
   ```bash
   $EDITOR /Users/wmadden/Projects/prisma/prisma-next-ws/worktrees/tml-2536-v2/skills/upgrade/prisma-next-upgrade/upgrades/0.9-to-0.10/instructions.md
   ```
...
8. Confirm the deserializer crash is gone:
   ```bash
   node ../../packages/1-framework/3-tooling/cli/dist/cli.js migration plan --json | head -5
   ```
   You should **not** see `PN-CLI-4003 Contract validation failed`. (You may still see `PN-CLI-4020` from the unrelated TML-2521 namespaces drift — that's expected and out of scope for this scenario.)
...
- Step 8: the deserializer no longer rejects the substrate; whatever residual error surfaces is unrelated to TML-2536.

### Failure modes

- Step 2 doesn't reproduce the bug (means the stash didn't actually revert, or the strict deserializer is silently re-stamping — TML-2536 has structurally regressed).
- Step 3 surfaces a comprehension gap (mark as ⚠️ High; the prose is the user's first line of defence).
- Step 4 misses an affected snapshot (mark as 🛑 Blocker; the codemod's glob is wrong).
- Step 5 throws on a snapshot the codemod's dispatch rules don't anticipate (mark as ⚠️ High; either the codemod needs broader dispatch or the snapshot is hand-edited and needs explicit prose guidance).
- Step 6 shows non-empty diff (mark as 🛑 Blocker; the codemod's output diverges from the PR-branch state — validation-by-execution failed).
- Step 7 reports any `FIXED` (mark as 🛑 Blocker; the codemod is not idempotent and would oscillate on repeat runs).
- Step 8 still shows `PN-CLI-4003` (mark as 🛑 Blocker; the codemod missed a structural case the strict deserializer rejects).

### Restore

```bash
cd "$(git rev-parse --show-toplevel)"
git stash pop                      # restores the codemod-applied state (PR-branch)
git status                         # must be clean
```
...
- Pre-flight complete.
- `git status` clean.
- pgvector extension's typecheck + tests are green on `main` (sanity baseline — if they're failing on `main`, this scenario can't isolate D3's effects).
...
1. From repo root, locate the pgvector extension's source surface:
   ```bash
   cd "$(git rev-parse --show-toplevel)"
   find packages/3-extensions/pgvector -name "*.ts" -path "*/src/*" -not -path "*/node_modules/*" | xargs rg -l 'storage.types|new SqlStorage|codecId' 2>/dev/null | head -10
   ```
...
2. Read the extension-skill entry's prose:
   ```bash
   $EDITOR /Users/wmadden/Projects/prisma/prisma-next-ws/worktrees/tml-2536-v2/skills/extension-author/prisma-next-extension-upgrade/upgrades/0.9-to-0.10/instructions.md
   ```
...
3. Confirm pgvector's tests are green on the current branch as the baseline:
   ...
   ```
   Both should pass. If they don't, stop — this scenario can't isolate D3's effects, and the failure is a baseline issue worth surfacing separately.
...
- Step 2 (judgement): the prose reads coherently; the four source-code shapes (object-literal, spread, destructure, `new SqlStorage(...)`) cover what pgvector actually has.
...
- Step 2 (judgement): the source-rule guidance doesn't cover a shape that exists in pgvector's tree (mark as ⚠️ High; the rule prose is incomplete from the extension author's vantage point).
- Step 3: pgvector baseline fails (mark as 🛑 Blocker — out of scope for D3 but blocks the slice's DoD: extension audience can't be verified end-to-end).
- Step 4: the codemod fails to stamp a snapshot pgvector ships (mark as 🛑 Blocker; the codemod's glob or dispatch is wrong for extension layouts).
- Step 5: pgvector source carries a shape D3's rules don't cover (mark as ⚠️ High).
- Step 6 (optional): the `SqlStorage` constructor diagnostic doesn't name the offending entry, or doesn't point at `toStorageTypeInstance(...)` (mark as 📝 Follow-up; the diagnostic could be more actionable).
...
| --- | --- |
| AC-2 (lint over today's tree is clean) | Re-running `lint:no-contract-cast` against the current branch only proves CI did its job. Scenario 5 is the user-meaningful version (does the gate gate?). |
| AC-9 (per-`kind` snapshot fixtures exist) | Pure unit-test infrastructure. A QA pass that re-ran them would be re-running our own tests — exactly the antithesis of manual QA. CI covers this. If you want to spot-check, `ls packages/3-targets/3-targets/postgres/test/fixtures/snapshot-read-shapes/` should show `codec-instance.json` and `postgres-enum.json`. |
| AC-11 (`pnpm typecheck && test && lint:deps`) | CI runs this on every push. Re-running locally proves only that your machine matches CI; it doesn't probe anything user-facing. |
...
| ------ | ----------------------------- |
| AC-1   | 1, 2, 3                       |
| AC-2   | 2 (observed), 5 (gate-of-gate)|
| AC-3   | 4                             |
| AC-4   | 1, 3, 8                       |
| AC-5   | 1, 2                          |
| AC-6   | 6                             |
...
The runner's report (`drive-qa-run`) owns per-scenario results and the overall verdict. Findings are classified there with the Blocker / High / Follow-up rubric in the runtime context; this script intentionally enumerates only failure-mode *categories*.
