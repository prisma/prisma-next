...
- [ ] **T4.2** Update [`packages/2-mongo-family/1-foundation/mongo-codec/src/codecs.ts`](../../packages/2-mongo-family/1-foundation/mongo-codec/src/codecs.ts): adopt the unified `Codec` interface (re-export from `framework-components` or align the local shape), adopt the same factory entry point (`mongoCodec` accepts sync-or-async `encode` / `decode`, lifts sync), ensure query-time methods on the constructed codec are Promise-returning. **As part of this reshape, address the pre-existing `as unknown as TTraits` cast at the empty-traits default site (line 32 on m1 HEAD)**: either drop the cast (the empty-traits default can be expressed without a double-cast) or add a one-line `// why` comment per `AGENTS.md` § Typesafety rules. (Originally tracked as F1 in `reviews/code-review.md`; re-recorded here under the new `drive-orchestrate-plan` findings discipline that disallows non-actionable findings.)
