...
> Final round of m5. F8 (T5.4 deferred-test header comment overpromising) closed by commit `a8ea4dbd1` via Option B (rewrite the comment to match what's on disk). Phase verdict: **SATISFIED**. The codec-async-single-path project is ready for the create-pr handoff, gated only by the orchestrator's existing T5.11 / T5.12 close-out PR sequencing.

### Intent (m5 R3)

Make the deferred-test artifact at T5.4 honestly describe its own state. m5 R1's first reviewer pass framed the include-aggregate child-codec deferral correctly, but the header comment in [`collection-dispatch.test.ts`](../../../packages/3-extensions/sql-orm-client/test/collection-dispatch.test.ts) overstated the deferred-test discipline by claiming "the assertions themselves are preserved verbatim against the single-path contract" while the three `it.skip` bodies carried only `expect(true).toBe(true)` stubs. The fix aligns the artifact's documentation with what's on disk so future implementers reading the test file are not misled about whether the assertion shape needs re-derivation.

### Narrative (m5 R3 — semantic step)

1. **Replace the T5.4 header comment with an honest description of the placeholders.** [`collection-dispatch.test.ts L395–L426`](../../../packages/3-extensions/sql-orm-client/test/collection-dispatch.test.ts:395-426) (commit `a8ea4dbd1`) now explicitly states that "the titles describe what each case would assert, but the bodies are stubbed (`expect(true).toBe(true)`) — assertions are not carried over verbatim", and points future implementers at "PR #375 § collection-dispatch.test.ts (tests 5–7)" for re-derivation when child-row codec dispatch lands. The structural-deferral rationale is preserved (dispatcher does not invoke codecs on `jsonb_agg` child cells; verified by `rg 'codec\.(encode|decode)' packages/3-extensions/sql-orm-client/src` returning zero matches; child-row codec dispatch is a separate ORM feature project, orthogonal to async-shape scope), as is the rationale for dropping PR #375 test 8 (it asserts the inverse of AC-RT2's plain-`T` guarantee). No test bodies changed; suite count unchanged at 463 passed | 3 skipped. **F8 closed.**

### Behavior changes & evidence (m5 R3)

| Change | Before (m5 R2 HEAD `a4aeba917`) | After (m5 R3 HEAD `a8ea4dbd1`) | Evidence |
|---|---|---|---|
| T5.4 deferred-test header comment | "The assertions themselves are preserved verbatim against the single-path contract" — overpromise; bodies are `expect(true).toBe(true)` stubs | Honest description: "the titles describe what each case would assert, but the bodies are stubbed (`expect(true).toBe(true)`) — assertions are not carried over verbatim"; points at PR #375 § collection-dispatch.test.ts tests 5–7 for re-derivation | [`collection-dispatch.test.ts L395–L426`](../../../packages/3-extensions/sql-orm-client/test/collection-dispatch.test.ts:395-426) |
| T5.4 `it.skip` block bodies | `expect(true).toBe(true)` × 3 | unchanged — `expect(true).toBe(true)` × 3 | [`collection-dispatch.test.ts L427–L440`](../../../packages/3-extensions/sql-orm-client/test/collection-dispatch.test.ts:427-440) |
| Deferred-test discipline consistency across the project | T5.2 honest (full assertion shape preserved); T5.4 inconsistent (header overpromises, bodies stub) | T5.2 unchanged; T5.4 now honestly describes its bodies as stubs and routes future implementers to PR #375 for re-derivation | T5.2 at [`json-schema-validation.test.ts L613–L685`](../../../packages/2-sql/5-runtime/test/json-schema-validation.test.ts:613-685); T5.4 at [`collection-dispatch.test.ts L395–L440`](../../../packages/3-extensions/sql-orm-client/test/collection-dispatch.test.ts:395-440) |

### Compatibility / migration / risk (m5 R3)

- **No source code touched, no test bodies touched.** Doc-only round inside one test file's header comment; runtime behavior, types, and test coverage are unchanged at HEAD `a8ea4dbd1` vs HEAD `a4aeba917`. Targeted `pnpm --filter @prisma-next/sql-orm-client test` re-run confirms 463 passed | 3 skipped (unchanged). `pnpm lint:deps` and `pnpm typecheck` re-run as a sanity check; both PASS workspace-wide.
- **No spec changes.** All m5 ACs are PASS or PASS-with-scope-note; AC-SE4 stays PASS-with-scope-note on substance (the include-aggregate-child-codec deferral is unchanged); AC scoreboard totals unchanged at 26 PASS / 0 FAIL / 0 NOT VERIFIED.
- **No plan changes.** T5.4 substantively complete at m5 R1; R3 closes the doc-quality follow-up. T5.11 / T5.12 remain orchestrator-deferred to the close-out PR.
- **Inline-comment consistency follow-up surfaced, not filed.** The implementer flagged that the inline comment inside the first `it.skip` block at [`collection-dispatch.test.ts L428–L430`](../../../packages/3-extensions/sql-orm-client/test/collection-dispatch.test.ts:428-430) still reads "Translated from PR #375", which the rewritten header now establishes is technically inconsistent (the body is a stub, not a translation). The reviewer accepted this as honestly-resolved within scope — the comprehensive header at L395–L426 governs and a future implementer encounters it first; filing a new finding for a one-line edit-only doc nit is below the findings-discipline threshold. Surfaced to `user-attention.md` for the orchestrator to fold into the close-out PR alongside T5.11 / T5.12.

### Follow-ups / open questions (m5 R3 — final)

1. **Phase SATISFIED; create-pr handoff unblocked.** All findings are closed (F1, F2, F3, F4, F5, F6, F7, F8); 26 PASS / 0 FAIL / 0 NOT VERIFIED on the AC scoreboard; walkthrough is project-spanning (m1 → m5); SDR is up to date through m5 R3; `spec.md` untouched since project start; worktree clean at HEAD `a8ea4dbd1`; branch ahead of `origin/main` by 37 commits and ready for `git push -u origin HEAD`.
2. **Close-out PR queued behind `user-attention.md` review (orchestrator-owned).** T5.11 / T5.12 (strip `projects/` references repo-wide and delete `projects/codec-async-single-path/**`) remain in a separate close-out PR after the user reviews `user-attention.md`. The close-out PR can also fold in the inline-comment consistency follow-up as a one-line edit at L428–L430 of `collection-dispatch.test.ts`.
3. **Future projects identified during this project.** (a) Include-aggregate child-codec dispatch (closes the include-aggregate redaction-correctness hole at the ORM layer; orthogonal to async-shape scope; the T5.4 placeholders are positioned for activation when this project lands and the assertions are re-derived from PR #375). (b) Mongo row decoding (next-family extension; should mirror SQL's `decodeRow` per ADR 204). (c) Redaction-trigger spelling (orthogonal redaction-policy work; § Non-goals item; T5.2's `it.skip` is positioned for activation).
