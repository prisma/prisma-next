...
didChange  ──▶  mark the document's project dirty            (cheap; no parse)
                (the version bump comes free from TextDocuments)
...
- `ensureCurrent(project, uri)` is the only place raw text becomes derived state, it is **synchronous**, and it recomputes only when `TextDocument.version` differs from the last-parsed version. A read whose version is unchanged does zero parsing.
...
- The defensive per-request reparse (`currentDocumentArtifact`) and whole-document text compare are removed; freshness is keyed on the version `TextDocuments` already maintains.
...
- [ ] `ensureCurrent` recomputes only when `TextDocument.version` advanced; a read at an unchanged version performs no parse (covered by a test).
