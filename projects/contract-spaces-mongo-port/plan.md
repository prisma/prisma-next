...
- [ ] **T3.2** `[checkpoint: high reasoning — CKPT-2]` Generalise `projectSchemaToSpace` (or wire a Mongo extractor) to handle `MongoStorage.collections` and `MongoSchemaIR.collections`. Decide between option A (extractor callbacks) and option B (duck-type both shapes) at this point; record the choice in the commit message. (TC-11)
  ...
  - **On completion (covers T3.2 + T3.3, since T3.3 reuses the decision):** agent prompts operator: *"CKPT-2 complete (T3.2 + T3.3 both landed under the chosen shape). Please downgrade reasoning effort back to medium before T3.4."* Wait for confirmation before resuming.
- [ ] **T3.3** Generalise the loader's `extractTableNames` disjointness check to extract Mongo collection names. Same shape decision as T3.2. (TC-10)
- [ ] **T3.4** Wire the multi-space runner's per-space verify step: project the introspected schema to the space's slice via `projectSchemaToSpace` (or the Mongo extractor) and call `verifyMongoSchema` on the projection. Marker advances iff verify passes. (TC-8, TC-9)
