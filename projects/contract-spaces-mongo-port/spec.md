...
- [ ] **AC2** (FR2, FR3, FR4). On a fresh DB: marker writes produce `{_id: 'app', space: 'app', ...}`. On a DB with a legacy `{_id: 'marker', ...}` doc: first marker access detects the legacy shape and rewrites it idempotently to `{_id: 'app', space: 'app', ...}`. Detection and rewrite-trigger may share one round-trip (single aggregate); the convergence property is what matters. Idempotency tests cover all reachable states (fresh / legacy-only / already-upgraded / mid-upgrade / concurrent) green.
