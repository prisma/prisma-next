...
- **Outcome:** The contract IR can represent a domain enum and a field's restriction reference. `ValueSetRef` (the space-aware coordinate `{ kind, namespaceId, name, spaceId? }`, matching the `ForeignKeyReference` carrier and the `UNBOUND_NAMESPACE_ID` sentinel) is defined in the framework layer and exported. `ContractEnum { codecId, members: readonly {name,value}[] }` and an `enum?: Record<string, ContractEnum>` slot exist on `ApplicationDomainNamespace`. `ContractField` gains optional `valueSet?: ValueSetRef`. Constructs directly; `pnpm typecheck` passes for the foundation/framework packages; a unit test constructs a domain enum + a field with a `valueSet` and reads the members back in order.
...
- **Focus:** `packages/1-framework/0-foundation/contract/src/{domain-envelope.ts,domain-types.ts}` + the `ValueSetRef` type's framework home (`framework-components`). Direct-construction unit test only. Out: storage shapes (D2), authoring (D3), serialization (D4). Do **not** touch the native `PostgresEnumType` path or PSL.
...
- **Builds on:** Dispatch 1's exported `ValueSetRef`.
...
- **Outcome:** A contract carrying the new domain `enum` + storage `valueSet` serializes to JSON and hydrates back identically, and arktype validators accept the valid shape + reject malformed (missing `values`, bad member). The SQL-family serializer hydrates the `valueSet` slot via the entity registry; `StorageValueSet`/`ContractEnum` validator fragments are registered. **Slice-DoD gates:** `pnpm fixtures:check` is clean with **zero** fixture changes (the dark path alters no emission), round-trip + validator + type-tests green, full `pnpm typecheck` clean, no new bare casts.
