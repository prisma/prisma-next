...
The nearest `prisma-next.config.ts` is the authority for *which* files belong to a project and which of those files are schema inputs, so the server keeps that mapping **live per project**: it resolves `contract.source.inputs` for each known config, watches config files, and re-resolves the affected project when its config changes — re-publishing diagnostics for newly-added inputs and clearing them for removed ones. Opening a file whose nearest config has not been seen yet creates a new in-memory LSP project keyed by that config path. A config edit does not require a server restart.
...
- **No editor-side one-server-per-project assumption.** The server may be launched for a workspace containing files covered by multiple `prisma-next.config.ts` files; it must group schema documents by the nearest config that covers each file rather than assuming `initialize.rootUri` names the only project.
...
- **Config watching (decided):** the server registers file watchers for `prisma-next.config.ts` files (via `client/registerCapability` for `workspace/didChangeWatchedFiles`, or static registration in `InitializeResult` if the client supports it) and re-resolves the affected project on change. This is the one watched-files surface in scope; no other on-disk paths are watched.
...
- **Config agreement with the CLI.** For each discovered project, the set of files the server treats as schema must equal what `format` / `contract emit` treat as schema for that config — same config loader, same `sourceFormat === 'psl'` gate, same resolved `inputs`. The server must not invent its own notion of "which file is the schema."
...
- **Config stays live per project.** The server resolves the schema-input set for each known config and reacts to subsequent config changes via `workspace/didChangeWatchedFiles` (watchers registered for `prisma-next.config.ts`). After a config change: that project's inputs are re-resolved; documents that became inputs gain diagnostics; documents that stopped being inputs (or fell under a non-`psl` `sourceFormat`) have their diagnostics cleared. No restart required. On-disk changes to the config count, whether or not it is open in the editor.
...
- [ ] The server creates and maintains in-memory projects keyed by config file path, including creating a project when a newly opened file belongs to a previously unseen nearest `prisma-next.config.ts`.
- [ ] For each project, the server loads that project's `prisma-next.config.ts` and identifies schema documents by `sourceFormat === 'psl'` + membership in the resolved `contract.source.inputs` (all inputs, not just the first); a `.psl` file not listed in `inputs`, and any document under a `typescript`/absent `sourceFormat`, receive no diagnostics.
...
- [ ] An ADR (or a pointer to one) records the settled "lsp is a version-matched CLI subcommand with multi-project workspace handling keyed by config path" decision, authored at close-out per the ADR-cadence convention.
...
- **Schema documents are identified from the nearest config**, not by file extension: each open file belongs to the nearest `prisma-next.config.ts`; within that project, `sourceFormat === 'psl'` **and** path ∈ resolved `contract.source.inputs` (all inputs). The TypeScript contract source is unsupported here.
- **The server reacts to config changes** via `workspace/didChangeWatchedFiles` watchers for `prisma-next.config.ts`; each affected project's input set stays live without a server restart.
