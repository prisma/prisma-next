...
- **Each slice leaves a usable artefact.** Slice 1 (event vocab + build-loop + planning-chain instrumentation) is usable standalone: the operator can rev any of the seven instrumented skills, run it, and read the build-loop quality metrics (`rounds_per_dispatch`, brief stability) and the planning-chain quality metrics (spec-amendment rate, plan-amendment rate, I12-halt rate, triage stability) straight off the trace. Slice 2 (lifecycle + cadence + direct-change gap) closes the remaining trace blind spots. Slice 3 (full assertion library + diagnostic dashboard) is the close-out.
...
- [ ] All `drive-*` workflow + atomic skills instrumented per the vocabulary, split across slices: **slice 1** instruments `drive-build-workflow`, `drive-specify-project`, `drive-specify-slice`, `drive-plan-project`, `drive-plan-slice`, `drive-triage-work`, `drive-discussion` (build-loop + planning chain — the load-bearing quality surfaces). **Slice 2** instruments `drive-start-workflow`, `drive-deliver-workflow`, `drive-dispatch`, `drive-create-project`, `drive-check-health`, `drive-run-retro`, `drive-close-project` (lifecycle + cadence + direct-change gap). QA-side skills (`drive-pr-description`, `drive-pr-walkthrough`, `drive-qa-plan`, `drive-qa-run`, `drive-code-review`) are deliberately excluded — at the edges of the workflow, not load-bearing for methodology effectiveness; revisit at close-out if signal demands.
