...
- **Instrumentation footprint on skill bodies.** Each instrumented skill grows by an "Emit" section. Mitigation: factor a shared `docs/drive/trace-emission-protocol.md` (or similar) reference doc that skills link to; each skill body lists only its own event types and payload shape inline.
