...
3. **The IR contract is a framework-level interface; consumers depend on the interface, not on a class hierarchy.** `OpFactoryCall` is an interface (`{ factoryName, operationClass, label }`) exported from `framework-components`. Concrete call classes (`CreateTableCall`, `DataTransformCall`, …) implement the interface; type positions in renderer signatures, planner returns, and visitor inputs use the interface or a target-specific union of concrete classes. The Postgres target uses an internal abstract base class `PostgresOpFactoryCallNode` for shared plumbing — mirroring Mongo's `OpFactoryCallNode` (`packages/3-mongo-target/1-mongo-target/src/core/op-factory-call.ts`). That base class lives inside the Postgres package, is not exported, is not referenced in any cross-package signature, and is not referenced from any non-Postgres consumer. The visible contract for every other package is the framework-level `OpFactoryCall` interface; no SQL-family base class ships.

   `PostgresOpFactoryCallNode` carries the `OpFactoryCall` slots plus an abstract `toOp(): SqlMigrationPlanOperation<PostgresPlanTargetDetails>` method that each concrete subclass implements with a one-line delegation to its corresponding pure factory in `operations/`. This replaces the earlier `renderOps` visitor: lowering a call to a runtime op is a polymorphic method call on the node, not an external double-dispatch walk. `renderOps(calls)` itself survives as a thin `calls.map(c => c.toOp())` convenience.

   The TypeScript-rendering side of the IR is factored into a shared framework package, [@prisma-next/framework-ts-render](../../packages/1-framework/1-core/framework-ts-render/README.md). It exports `TsExpression` — an abstract base for anything renderable as a TypeScript expression, with two polymorphic members (`renderTypeScript(): string`, `importRequirements(): readonly ImportRequirement[]`) — plus `ImportRequirement` and `jsonToTsSource(value: JsonValue): string`. `PostgresOpFactoryCallNode` extends `TsExpression`, and `PlaceholderExpression` (the AST node for a scaffolded `dataTransform` body) also extends it directly. `PlaceholderExpression.renderTypeScript()` emits a bare `placeholder("slot")` call; `DataTransformCall.renderTypeScript()` wraps its `check` / `run` children with `() => …` at the call site where that wrapping is meaningful.

   Mongo shares the same framework-level TS rendering package and implements the `OpFactoryCall` interface via its own `OpFactoryCallNode` (`extends TsExpression implements OpFactoryCall`). Each Mongo concrete call class implements polymorphic `renderTypeScript()` / `importRequirements()`, and Mongo's `renderCallsToTypeScript` walks nodes polymorphically — no change to Mongo's rendered output. `PlaceholderExpression` has no Mongo counterpart; Mongo's planner doesn't emit `dataTransform`.
...
- **R2.8** `renderCallsToTypeScript(calls, meta)` is a polymorphic (non-visitor) traversal: it walks the node array, collects `node.importRequirements()` from every `TsExpression` it visits (including the `check` and `run` children of `DataTransformCall`), deduplicates the import list, and renders the module body by concatenating `node.renderTypeScript()` results inside the `plan()` method. The emitted source is a complete `migration.ts`: shebang, imports from `@prisma-next/family-sql/migration` and `@prisma-next/target-postgres/migration`, contract snapshot import, optional `sql(contract)` binding for data-transform closures, `export default class extends Migration { plan() { … } }`, `Migration.run(import.meta.url, …)`. `DataTransformCall` instances whose `check` or `run` is a `PlaceholderExpression` (per ADR 200) render the child expression as a bare `placeholder("slot")` via `PlaceholderExpression.renderTypeScript()`; the `DataTransformCall` site wraps each child with `() => …` so the rendered TypeScript is syntactically `dataTransform(label, { check: () => placeholder("slot"), run: () => placeholder("slot") })`. The `placeholder` symbol is automatically surfaced in the import block via `PlaceholderExpression.importRequirements()`, and the slot string is preserved end-to-end from planner construction through rendering.
- **R2.9** Lowering calls to runtime ops is polymorphic, not visitor-based. Each `PostgresOpFactoryCallNode` subclass implements `toOp(): SqlMigrationPlanOperation<PostgresPlanTargetDetails>` as a one-line delegation to its corresponding pure factory in `operations/`. `renderOps(calls)` is a thin `calls.map(c => c.toOp())` convenience. `DataTransformCall.toOp()` throws `errorUnfilledPlaceholder(label)` (`PN-MIG-2001`) directly — the call class accepts `check` / `run` slot strings, not `MigrationTsExpression` bodies, and is never materialized into a runtime op at plan time. When `migration plan` emits a plan carrying any `DataTransformCall`, the throw propagates through `TypeScriptRenderablePostgresMigration.operations` and is caught by the CLI's artifact-writing path (see R2.11). Hand-authored migrations with real closures use the pure `dataTransform` factory directly at execution time; they never re-enter the call-IR path.
...
- **R3.1** Factories exported from `@prisma-next/target-postgres/migration` (`createTable`, `addColumn`, `createIndex`, `dropIndex`, `alterColumnType`, `setDefault`, `dropDefault`, `setNotNull`, `dropNotNull`, `addPrimaryKey`, `addForeignKey`, `addUnique`, `dropConstraint`, `dropTable`, `dropColumn`, `createEnumType`, `addEnumValues`, `dropEnumType`, `renameType`, `createExtension`, `createSchema`, `rawSql`, `dataTransform`) are pure functions of literal arguments, organized by theme under `operations/`. No factory references `OperationResolverContext`, imports codec hooks, takes a `contract` parameter, or touches a `db` handle. `rawSql` is an escape hatch that wraps a pre-built `SqlMigrationPlanOperation<PostgresPlanTargetDetails>` — used by the planner's `liftOpToCall` helper for op sources the IR hasn't (yet) decomposed into structured call classes, and available to users authoring migrations by hand.
- **R3.2** Factory signatures align 1:1 with the constructor arguments of the corresponding concrete call class (per ADR 195). Each concrete call class implements the framework-level `OpFactoryCall` interface (`{ factoryName, operationClass, label }`); type positions in renderer signatures, planner returns, and visitor inputs use the `OpFactoryCall` interface or a target-specific union of concrete classes. Adding a new factory requires changes at exactly two sites — the factory in `operations/<theme>.ts` and the concrete call class in `op-factory-call.ts` (whose abstract `toOp()` forces implementation). The TypeScript renderer is automatically exhaustive because `renderTypeScript()` / `importRequirements()` are abstract on `TsExpression`, which `PostgresOpFactoryCallNode` extends.
...
- **Type-safe exhaustiveness.** Adding a new concrete call class is a compile-time forcing function on every dispatch site. The abstract `toOp()` method on `PostgresOpFactoryCallNode` forces the lowering-to-runtime-op case; the polymorphic `renderCallsToTypeScript` surfaces a missing TypeScript-rendering implementation via the abstract `renderTypeScript()` / `importRequirements()` methods on `TsExpression`, which the new class must override. The same applies to any planner-side dispatch over `OpFactoryCall`.
...
- [ ] `framework-components` exports an `OpFactoryCall` interface (`{ factoryName, operationClass, label }`). No abstract base class is shipped at framework or family level. Postgres uses an internal abstract base class `PostgresOpFactoryCallNode` (with abstract `toOp()`) for shared plumbing and extends `TsExpression` from `@prisma-next/framework-ts-render` for shared TS-rendering plumbing. Neither `PostgresOpFactoryCallNode` nor `TsExpression` is exported from the Postgres target; `TsExpression` is a framework primitive shared by both targets, not a cross-package Postgres-specific surface.
- [ ] `@prisma-next/target-postgres/migration` exports the full factory surface per R3.1 (and the `placeholder` helper re-exported from `@prisma-next/errors/migration`). The `PostgresOpFactoryCall` discriminated union with one frozen class per factory (each implementing `OpFactoryCall`), the polymorphic `renderCallsToTypeScript` function, and the `renderOps` helper all live inside the Postgres package (consumed by the planner and by the target's capability wiring); none of them are re-exported from the user-facing `/migration` subpath.
- [ ] `DataTransformCall` accepts `check` / `run` as slot strings and always throws `PN-MIG-2001` from `toOp()`. Real (non-placeholder) data-transform execution goes through the pure `dataTransform` factory at migration-runtime, never through the call-IR path.
...
- [ ] Adding a new factory requires changes at exactly two sites (factory function + concrete call class), verified by omitting `toOp()` on a new subclass and confirming a TypeScript error. The TypeScript renderer is automatically exhaustive because `renderTypeScript()` / `importRequirements()` are abstract on `TsExpression`.
...
2. **`CreateExtensionCall` ownership.** Resolved: `createExtension` and `createSchema` live under `@prisma-next/target-postgres/migration` as Postgres-specific factories in `operations/dependencies.ts`. Family-level lifting happens when a second SQL target needs it. The planner's `liftOpToCall` dispatcher reaches these structured call classes for ops whose `target.details.objectType` identifies them; anything the dispatcher doesn't recognize falls back to `RawSqlCall(op)`.
...
6. **Cross-target consolidation of the IR and renderable-migration class.** Resolved in this project: `TsExpression`, `ImportRequirement`, and `jsonToTsSource` live in `@prisma-next/framework-ts-render` and are shared by both targets. Each target still keeps its own `TypeScriptRenderableXMigration` concrete class; a further lift of a generic `TypeScriptRenderableMigration<TCall extends OpFactoryCall, TOp>` into `framework-components` is a follow-up and is purely structural (no behavior change, no ADR change). A sibling follow-up is the family-API retarget (Path A in `plan.md`): teach `planTypeOperations` / `ComponentDatabaseDependency.install` to return call IR directly so `liftOpToCall`'s op-first bridge can be dropped in favor of uniform call-IR production. Scheduled before a second SQL target (SQLite) lands.
