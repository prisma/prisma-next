...
| Phase 0 — pure factory extraction + placeholder-closure helper | [`specs/phase-0-factory-extraction.spec.md`](./specs/phase-0-factory-extraction.spec.md) | Phase 0 |
...
No SQL-family base class is added: `family-sql` ships no `SqlOpFactoryCallBase` and no `PlannerProducedSqlMigration`. The `OpFactoryCall` interface is lifted directly to the framework so any future target (sql or otherwise) can reuse it without going through a family layer. The Postgres target uses an internal abstract base class (`PostgresOpFactoryCallNode`) inside `op-factory-call.ts` to share the `accept()` plumbing across the concrete call classes — mirroring Mongo's `OpFactoryCallNode` (`packages/3-mongo-target/1-mongo-target/src/core/op-factory-call.ts`). That base class is private to the Postgres package: not exported, not referenced in cross-package signatures, not visible to non-Postgres consumers.
...
| `PostgresOpFactoryCallNode` | **Internal** abstract base class private to the Postgres package. Provides shared `accept()` / freeze plumbing for the concrete call classes. Mirrors Mongo's `OpFactoryCallNode`. Implementation detail — not exported, not in any cross-package signature. | `packages/3-targets/3-targets/postgres/src/core/migrations/op-factory-call.ts` |
| Concrete call class | Frozen class representing a single factory call (e.g. `CreateTableCall`, `DataTransformCall`) with literal args + planner-derived label/class. Discriminated union per target. Extends `PostgresOpFactoryCallNode` (Postgres) or `OpFactoryCallNode` (Mongo); satisfies `OpFactoryCall`. | ADR 195, `op-factory-call.ts` |
...
| Placeholder closure | A closure (`() => never`) carrying a hidden slot tag. When invoked it throws `errorUnfilledPlaceholder` (`PN-MIG-2001`). The renderer detects the tag and emits `() => placeholder("slot")` in TypeScript output; the runner detects nothing — invocation throws naturally. There is no `stub: boolean` flag on `DataTransformCall`; "is this slot a stub?" reduces to "is this closure a placeholder closure?". | ADR 200, `packages/1-framework/1-core/errors/src/migration.ts` |
...
| **2. Issue planner → class-flow IR** | Retarget the **issue planner** (`planDescriptors` → `planCalls`); strategies emit `OpFactoryCall[]`; data-safety scaffolding (`DataTransformCall` with placeholder closures) lights up. `renderCallsToTypeScript` is wired through `MigrationPlanWithAuthoringSurface`. CLI still on descriptor branch. | Additive; needs new exhaustive tests for strategies under the new IR | Medium |
...
| R2.7–R2.9 (`TypeScriptRenderablePostgresMigration`, two renderers, placeholder closures) | 1, 2 | Phase 1 introduces the renderable-migration class and renderers using walk-schema output; Phase 2 reuses them under the issue planner with placeholder closures for stubs |
...
- **Placeholder handling in `dataTransform`.** The pure `createDataTransform` factory takes `check` and `run` as closures — there is no `stub: boolean` parameter. When the wrapper encounters the `TODO` sentinel during descriptor resolution, it constructs **placeholder closures** via a new `placeholderClosure(slot: string): () => never` helper (added to `packages/1-framework/1-core/errors/src/migration.ts` alongside the existing `placeholder()` function). The closure throws `errorUnfilledPlaceholder(slot)` when invoked and carries a non-enumerable `[PLACEHOLDER_SLOT]` symbol property the renderer uses in Phase 1 to emit `() => placeholder("slot")` source. Detail in `projects/postgres-class-flow-migrations/specs/phase-0-factory-extraction.spec.md`.
...
- `op-factory-call.ts` — internal abstract base `PostgresOpFactoryCallNode` (private; mirrors Mongo's `OpFactoryCallNode`) plus the `PostgresOpFactoryCall` discriminated union. One frozen class per factory: `CreateTableCall`, `DropTableCall`, `AddColumnCall`, `DropColumnCall`, `AlterColumnTypeCall`, `SetNotNullCall`, `DropNotNullCall`, `SetDefaultCall`, `DropDefaultCall`, `AddPrimaryKeyCall`, `AddForeignKeyCall`, `AddUniqueCall`, `CreateIndexCall`, `DropIndexCall`, `DropConstraintCall`, `CreateEnumTypeCall`, `AddEnumValuesCall`, `DropEnumTypeCall`, `RenameTypeCall`, `CreateExtensionCall`, `DataTransformCall`. Each extends the internal base and satisfies the framework-level `OpFactoryCall` interface. Constructor args mirror Phase 0 factory signatures 1:1.
- `PostgresOpFactoryCallVisitor<R>` interface with one method per variant.
- `render-ops.ts` — `renderOps(calls)` visitor; dispatches each call to its factory.
- `render-typescript.ts` — `renderCallsToTypeScript(calls, meta)` visitor; emits class-flow `migration.ts` source. (Used in this phase only by tests; Phase 3 wires it into `migration plan`.)
- `planner-produced-postgres-migration.ts` (named to match Mongo's `planner-produced-migration.ts`) — `TypeScriptRenderablePostgresMigration` concrete class extending `Migration<SqlMigrationPlanOperation<PostgresPlanTargetDetails>>` and implementing `MigrationPlanWithAuthoringSurface`. Holds `readonly calls: readonly PostgresOpFactoryCall[]`, constructor-injects `renderOps` and `renderCallsToTypeScript`. No SQL-family base class.
...
- Strategies that emit a stub `DataTransformCall` construct it with `check` and `run` set to placeholder closures (`placeholderClosure(slot)` from Phase 0). The `slot` string is preserved in the rendered TypeScript output and in the runtime `errorUnfilledPlaceholder` thrown if the user forgets to fill the slot in.
...
- `commands/migration-apply.ts`: confirm no change (reads `ops.json`, flow-agnostic).
