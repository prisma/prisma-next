...
| **`prisma-next-upgrade` skill** + **`prisma-next-extension-upgrade` skill** + **`@prisma-next/extension-author-tools`** | Users; extension authors | Two deterministic, agent-executable instruction sets (one per audience) for each `(from-minor, to-minor)` Prisma Next transition, plus a small bin-only npm package shipping `prisma-next-check-pins` (and future tooling) for extensions' CI. Both skills install via `npx skills add prisma/prisma-next/skills/upgrade --all` and `prisma/prisma-next/skills/extension-author --all` (unpinned — always tracks `main`). The agent runs the instructions step-by-step, validates, and commits one transition per step — no manual outreach from the framework team. | **Implementation-ready** ([`specs/upgrade-skill.spec.md`](specs/upgrade-skill.spec.md)) |
...
$ pnpm dlx prisma-next init
  ...
  agent> (firing the prisma-next-upgrade skill description)
