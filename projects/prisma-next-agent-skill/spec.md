...
**Artifact 1 — `package.json`-versioning refactor.** Today, every publishable `package.json` on `main` carries the placeholder `0.0.1`; the actual version is computed at publish time by `scripts/determine-version.ts` against npm dist-tags. An agent authoring a PR on a topic branch can't tell what version their change will land at — the version isn't readable from repo state. This blocks the upgrade-skill mechanism (recipes need to be keyed to a version the PR author can see) and is awkward for any other tooling that wants to reason about the current version. The refactor moves the version source-of-truth into `package.json` itself, makes advancing the minor a deliberate version-bump PR pattern, and slims `determine-version.ts` to per-event publish-version construction only. Spec is complete at [`specs/package-json-versioning.spec.md`](specs/package-json-versioning.spec.md); this project spec defers all detail there.
...
1. **Task 1 — `package.json`-versioning refactor.** Implementation-ready ([`specs/package-json-versioning.spec.md`](specs/package-json-versioning.spec.md)). Lands first on this branch. After it lands (as the first commit or as a sibling PR), every `package.json` on the workspace advertises the current in-flight minor (e.g. `0.7.0`), and the publish workflow reads versions from `package.json` rather than computing them from npm dist-tags.
...
- **FR1.** Task 1 is fully scoped by [`specs/package-json-versioning.spec.md`](specs/package-json-versioning.spec.md). The project-level position is that task 1's spec is the contract; this document does not add or override requirements there.
- **FR2.** Task 1 lands on this branch ([`tml-2514`](https://linear.app/prisma-company/issue/TML-2514)) as a prerequisite to task 2. Whether it lands as the first commit on the branch or as a sibling PR that merges first is the implementer's call; the constraint is that task 2 cannot proceed against an unrefactored publish workflow.
...
- [ ] **AC1.** Task 1's acceptance criteria (AC1–AC10 in [`specs/package-json-versioning.spec.md`](specs/package-json-versioning.spec.md)) are met. After task 1 lands, every publishable `package.json` on the branch advertises the current in-flight minor, and `publish.yml` reads from `package.json`.
...
- [`specs/package-json-versioning.spec.md`](specs/package-json-versioning.spec.md) — task 1's full spec, ready for implementation.
