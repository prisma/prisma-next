...
> **Distribution channel was falsified during execution.** This spec was originally drafted assuming the cluster would ship as an npm package (`@prisma-next/agent-skill`). It does not — see § Falsified assumptions below and the project spec's [§ Falsified assumptions](../spec.md#falsified-assumptions). The shape of this spec (the `init` integration flow, `--no-skill` flag, target-agnostic invocation, structured error) is unchanged; only the literal install command and package id have been edited. **Every occurrence of `npx skills add @prisma-next/agent-skill` (and any package-manager-equivalent like `pnpm dlx skills add @prisma-next/agent-skill`) in this spec should be read as `npx skills add prisma/prisma-next/skills#v<cli-version> --all`**, where `<cli-version>` is read from the CLI's own `package.json` at module load. The `@prisma-next/agent-skill` strings are retained verbatim so the originally-written rationale below remains coherent. The `/skills` subpath is load-bearing — it isolates discovery from contributor skills under `/skills-contrib/`; see the project spec's [§ Falsified assumptions, round 2](../spec.md#falsified-assumptions-round-2--metadatainternal-true-is-not-a-defence).

Replace `prisma-next init`'s hand-rolled agent-skill template emission with a project-level install of the user-facing skill cluster from [`usage-skill.spec.md`](usage-skill.spec.md), via the upstream `vercel-labs/skills` CLI invoked as a subprocess. After this lands, every `prisma-next init` invocation registers the version-locked usage skill cluster with the agent runtime at the project level, by default; the two hand-rolled per-target templates (`agent-skill-postgres.md`, `agent-skill-mongo.md`) and their renderer (`templates/agent-skill.ts`) are deleted. The install is **always project-level** (pinned to the CLI's own version — see the decisions section); the change adds a single optional flag, `--no-skill`, to skip installation for restricted environments (air-gapped, no-network). The new behavior is target-aware in the trivial sense — both Postgres and Mongo projects install the same skill cluster, because the published skill handles target-keyed content internally.
...
This spec was drafted assuming the cluster would ship as an npm package (`@prisma-next/agent-skill`) installable via `npx skills add @prisma-next/agent-skill`, and that `init` would shell out to that command. The npm channel is falsified — the upstream `vercel-labs/skills` CLI parses `@a/b` identifiers as GitHub `owner/repo` references, not npm package ids. The corrected install command is `npx skills add prisma/prisma-next/skills#v<cli-version> --all`, with `<cli-version>` read from the CLI's own `package.json` at module load. The `/skills` subpath is load-bearing: it roots upstream's discovery at the user-facing cluster, which is the actual defence against contributor-skill leakage (the `metadata.internal` mechanism is bypassed by `--all`; see the project spec's [§ Falsified assumptions, round 2](../spec.md#falsified-assumptions-round-2--metadatainternal-true-is-not-a-defence)). See the project spec's [§ Falsified assumptions](../spec.md#falsified-assumptions) and the implementation contract in [`../plans/distribution-fix-plan.md`](../plans/distribution-fix-plan.md).

The structural shape of this spec — `init` shells out to a single `skills add` command, the install is unconditional and project-level, `--no-skill` is the only escape hatch, the failure produces a structured error with a fix line, target-awareness lives inside the skill bodies — is unchanged. The literal command string is the only thing that moves. Affected clauses (substantive, not stylistic): FR1 (the literal command), FR8 (the manual-install fix line), FR9 (the structured error's fix line), FR11 (the legacy template removal — unchanged), AC1 (verification of the new command), AC2 (Postgres/Mongo invocation parity — same shape, new command).

The implementer of the distribution-fix PR should treat the `@prisma-next/agent-skill` strings throughout this spec as a stand-in for the corrected URL; updating every occurrence in place is left to the close-out walk that lifts long-lived design decisions out of `projects/` (see the parent spec's project-workflow guidance).
