...
- **FR24. Skill-coverage check extends the upgrade-recipe coverage check.** The `check:recipe-coverage` CI step from [`upgrade-skill.spec.md`](upgrade-skill.spec.md) FR13 / FR14 gains a third sub-check: if either of the existing two sub-checks fires (i.e. the PR requires a user or extension upgrade recipe), the PR must also touch the user-facing skill cluster at `/skills/`. The failure error names the expected directory and points at this spec's rationale. Implementer's note: this requires a small amendment to the `check:recipe-coverage` implementation specified in [`upgrade-skill.spec.md`](upgrade-skill.spec.md); the cross-spec wiring is the follow-up tracked in [Open Questions](#open-questions) below.

### In-repo authoring

- **FR25. Repo layout.** ~~`packages/0-shared/agent-skill/` is a standard pnpm workspace package…~~ **Falsified — see § Falsified assumptions and the project spec's block.** The cluster is no longer a workspace package. User-facing skills are plain markdown content under `/skills/<skill-name>/SKILL.md` at the monorepo root (the `vercel-labs/skills` CLI auto-discovers under conventional `skills/` and `.agents/skills/` paths). No `package.json`, no `publishConfig`, no `pnpm-workspace.yaml` entry — the cluster ships by git tag, not by npm publish.

- **FR26. `architecture.config.json` entry.** ~~The package is classified as `framework` / `tooling` / `shared`…~~ **Falsified together with FR25.** With the cluster no longer a workspace package, no `architecture.config.json` entry is needed (and the previous entry for `packages/0-shared/skills/**` is removed by the distribution-fix PR — see [`../plans/distribution-fix-plan.md`](../plans/distribution-fix-plan.md) milestone 1 task 5).

- **FR27. Journey-test inventory.** Journey tests live as Markdown checklists at `/skills/journey-tests/<journey>.md` (or wherever the cluster's manual journey-tests directory lands relative to `/skills/` after the distribution-fix PR's relocation). Each file names the prompt the agent receives, the example app to point it at, and the expected end-state criteria. Implementation enumeration is the implementer's call; the spec pins the inventory (every Acceptance Criterion `AC4`+ corresponds to one journey test).
...
This spec was drafted assuming the cluster would ship as an npm package (`@prisma-next/agent-skill`) installable via `npx skills add @prisma-next/agent-skill`. That assumption is falsified — the upstream `vercel-labs/skills` CLI parses `@a/b` identifiers as GitHub `owner/repo` references, not npm package ids, and has no npm-package install path (the [pending PR upstream](https://github.com/vercel-labs/skills/pull/200) is not on the critical path).
