...
### Milestone 1-cleanup: M1 design-review remediation — **REOPENED for R4** (was SATISFIED at HEAD `15e0534e1`; F6 filed post-close)
...
**Status:** F6 surfaced in interactive review of R2's output: the `Authored*` prefix on the relocated types implies a structural distinction between contract spaces that doesn't exist (all spaces are equal at the type level — the app/extension distinction lives only in the orchestrator). T-cleanup.5 reopens the milestone for one round to flatten the typology. AC-D6 added.

**Round outcomes so far (3 implementer + 3 reviewer rounds; R4 in progress):**
...
- **R4 (in progress, addresses F6):** flatten the contract-space typology — drop the `Authored*` prefix; canonical `MigrationPackage` (structural; in `framework-components/control`) plus `OnDiskMigrationPackage extends MigrationPackage` (in `migration-tools/src/package.ts`) for the deserialised-from-disk variant; rename the struct-taking write helper.

**ACs PASS so far (6/7):** AC-D0 / AC-D1 / AC-D2 / AC-D3 / AC-D4 / AC-D5. AC-D6 (no `Authored*` prefix; one canonical contract-space typology) NOT VERIFIED until R4 closes.
...
- [ ] **T-cleanup.5** (F6 — own round) Flatten the contract-space typology; drop the `Authored*` prefix introduced under T-cleanup.3. The prefix implies a structural distinction between contract spaces that doesn't exist (all spaces are equal at the type level — the app/extension designation lives only in the orchestrator). Concretely:
