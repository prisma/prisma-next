...
- [x] **T2.5** Extend the synthetic test extension from T1.10 to exercise the codec hook + per-space db init/update. — landed M2 R5, commit `84305b6e1`. End-to-end coverage at [`packages/3-targets/6-adapters/sqlite/test/migrations/db-apply-per-space.cli.test.ts`](../../packages/3-targets/6-adapters/sqlite/test/migrations/db-apply-per-space.cli.test.ts) (3 tests covering atomic init across both spaces, single-space marker advance on head bump, multi-space rollback when any space's plan fails). Authored against an inline synthetic SQLite extension space rather than `@prisma-next/extension-test-contract-space` (which is currently postgres-targeted from M1 R1 and runs through PGlite on the existing R4 multi-space rollback test). The SQLite path was endorsed for the rollback test by the orchestrator (`wip/unattended-decisions.md` § 12) for the in-process / no-PGlite-startup-cost reason. Codec lifecycle hook coverage is locked at the planner unit level by the M2 R1 tests (`planner.codec-field-event.test.ts` for both Postgres + SQLite); `executePerSpaceDbApply` routes through the same `planner.plan(...)` call so the hook fires identically — duplicating it in the e2e was deemed redundant. Substantially advances AM11 + AM12; full PASS gated on T3.6 / T3.4 once cipherstash drives the hook on a real codec.
