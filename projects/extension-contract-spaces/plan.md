...
- [ ] **T-cleanup.3** (F3 + F4 — coupled refactor; new round) Hoist the contract-space identity surface into `@prisma-next/framework-components/control`:
  - **F3**: move the canonical `APP_SPACE_ID` const there; delete the four duplicate consts in `migration-tools/src/space-layout.ts`, `2-sql/5-runtime/src/sql-marker.ts`, the postgres `statement-builders.ts`, the sqlite `statement-builders.ts`; eliminate raw `'app'` string literals in SQL templates and adapter `params: ['app']` arrays across `packages/2-sql/**/src` and `packages/3-targets/**/src` (use `${APP_SPACE_ID}` interpolation in SQL templates and the import in TS code); add a `pnpm lint:deps` check (or equivalent) flagging future re-declaration / raw-literal use.
  - **F4**: move `ExtensionContractRef`, `ExtensionMigrationPackage`, `ExtensionContractSpace` to `framework-components/control`, renaming to `ContractSpaceHeadRef`, `AuthoredMigrationPackage`, `AuthoredContractSpace` respectively; rename `writeExtensionMigrationPackage` → `writeAuthoredMigrationPackage` in `migration-tools/exports/io`; delete the lampshaded `MigrationPackageContents` duplicate in `migration-tools/io.ts` and import the canonical type instead; update `SqlControlExtensionDescriptor.contractSpace?:` to import the renamed type; drop redundant re-exports from `@prisma-next/family-sql/control` (no backward-compat shims per repo convention); update test-contract-space fixture imports (depending on T-cleanup.1's outcome); update `projects/extension-contract-spaces/specs/framework-mechanism.spec.md` to match the new names. Resolve threads `PRRT_kwDOQM0QJc6AnBPZ`, `PRRT_kwDOQM0QJc6AnCgM`, `PRRT_kwDOQM0QJc6AnHN1`, `PRRT_kwDOQM0QJc6AnHZL`, `PRRT_kwDOQM0QJc6AnI_P`, `PRRT_kwDOQM0QJc6AnKaq`, `PRRT_kwDOQM0QJc6AnKiQ`, `PRRT_kwDOQM0QJc6AnK-h`, `PRRT_kwDOQM0QJc6AnLBa`, `PRRT_kwDOQM0QJc6AnDVv`, `PRRT_kwDOQM0QJc6AnErI` (11 threads).
- [ ] **T-cleanup.4** (F2 — own round) Delete `migrateMarkerSchemaStatements` (postgres + sqlite); replace with a runtime detection step at runner boot that throws a clear "legacy marker shape detected — re-sign your DB" error if the table exists in the pre-`space`-column shape; delete the dedicated integration test (`marker-schema-migration.integration.test.ts`); audit for analogous "in zero-range" transitional migrations elsewhere (ledger, codec tables) and surface findings if any are found. Resolve thread `PRRT_kwDOQM0QJc6AnH2D`.
