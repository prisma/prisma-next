...
### Milestone 1-cleanup: M1 design-review remediation — **SATISFIED** (HEAD `15e0534e1`)

Addresses the design-quality findings surfaced in `reviews/code-review.md` (post-M1, pre-PR-merge). M1's functional acceptance was already met; this milestone was the remediation pass for design-quality concerns that didn't surface during the implementation rounds. Each task below maps to one or more findings (F0–F5) recorded in the review log; closure of every linked GitHub PR thread was part of "done."

**Outcome (3 implementer rounds + 3 reviewer rounds):**
...
**ACs PASS (6/6):** AC-D0 (verifier discrimination is content-based) / AC-D1 (test fixtures live in test trees) / AC-D2 (no transitional schema migrations) / AC-D3 (`'app'` single-source) / AC-D4 (contract-space types are framework-domain, consumer-agnostic) / AC-D5 (15/15 GH threads closed).

The original task list and validation gates are preserved below for the audit trail.
