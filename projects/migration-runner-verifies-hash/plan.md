...
- [ ] **T3.8 — `migration new` tamper integration test (F14).** Same co-located pattern in `migration-tamper.test.ts`: set up a valid migration directory, mutate `ops.json` post-creation, invoke `migration new` (it calls `readMigrationsDir` to compute the `from` reference for the new migration). Assert non-zero exit + `MIGRATION.BUNDLE_CORRUPT` substring. Capture the rendered diagnostic into the shared collection so T3.5's uniformity assertion (`expect(new Set(...).size).toBe(1)`) automatically extends to 5 commands. The UX intent under test: when the user asks to *create* a new migration, the diagnostic surfaces the **existing** corrupt package without ambiguity — same `why`/`fix` text as the other four commands, no off-topic "couldn't generate new migration" framing leaking through. Note: `migration new` requires schema-related setup (it needs to know the current schema to plan the new migration); the implementer should examine `migration-new.test.ts` for the existing test-mock pattern and reuse it. If the schema-mock setup proves too divergent from the other four tests' shape, surface that finding rather than working around it silently.

**Validation gate for Phase 3:** `pnpm test:integration` passes; the **five** tamper cases all hit the new diagnostic; the cross-command equality assertion from T3.5 passes over all five captured diagnostics (proving uniformity, not just presence); `pnpm lint:deps` reports zero violations (no new cross-plane edges introduced).
...
**Validation gate for Phase 4:** `pnpm -F @prisma-next/migration-tools test` passes; `pnpm -F @prisma-next/cli test` passes; `pnpm test:integration` passes (catches consumer-surface regressions outside the package — see F13 below); `pnpm typecheck` passes across the workspace; all the previously-added integration tests in Phase 3 still pass with their assertion strings updated to `MIGRATION.HASH_MISMATCH`. Additionally, after T4.6's search-and-replace, run `rg '<old-symbol>' packages/ test/ examples/` for each renamed identifier (`MigrationBundle`, `MigrationManifest`, `verifyMigrationBundle`, `computeMigrationId`, `errorBundleCorrupt`, `MIGRATION.BUNDLE_CORRUPT`, etc.) and confirm zero hits in production source — Phase 1's `verifyMigration` removal escaped to `test/integration/test/cli-journeys/migration-plan-details.e2e.test.ts` because its validation gate ran only `pnpm -F`-scoped tests.
...
- [ ] **T6.3 — Final search audit.** Two audit greps:
  1. **Renaming completeness.** `rg 'migrationId|MigrationBundle|MigrationManifest|verifyMigrationBundle|computeMigrationId|loadMigrationBundles|loadAllBundles' packages/ examples/` should return zero hits in production source (the only acceptable hits are in test fixture *file names* like `attestation.test.ts` if tests weren't renamed — note as cleanup if any). `attestation.ts` should not exist.
  2. **Close-out durability.** `rg 'Phase [0-9]|TML-2264|F0[0-9]|F1[0-9]|spec\.md|plan\.md|projects/migration-runner-verifies-hash' packages/ test/ examples/` should return zero hits referencing this PR's project artifacts. Acceptable surviving hits: unrelated `Phase N` references that pre-date this PR (e.g. comments in unrelated subsystems referring to *their* internal phasing — the postgres `planner-produced-postgres-migration.ts:23` "Phase 2" reference belongs to a different ticket's phasing and is not in scope), and `// F12:` style comments where the F-number refers to an unrelated convention rather than this PR's review log. Walk every hit; explicitly justify any kept reference. The intent: after C4 deletes the project folder, no production code or test should carry a JSDoc/comment cross-reference to a path that no longer exists.
- [ ] **T6.4 — Lift inline-divergent CLI code into shared helpers (F12 + F15).** Two latent inconsistencies between `migration status` / `migration new` and the other three loading commands (`apply` / `plan` / `show`), both surfaced during Phase 3 review. Both are mechanical "fold inline code into the shared helper" cleanups. Land both in this task because they touch the same two files (`migration-status.ts`, `migration-new.ts`) and share the same architectural pattern; one PR-review pass over the consolidation is cleaner than two.
  - **F12 — `mapMigrationToolsError` extraction.** `migration apply` and `migration plan` define a private `mapMigrationToolsError(error)` that wraps `MigrationToolsError` into a `CliStructuredError` with the full `error.details` carried into `meta`. `migration status` and `migration new` instead inline a `MigrationToolsError.is(...)` check that drops `error.details` and surfaces only `{ code }` in `meta`. Identical user-facing diagnostic text across all five commands (proven by T3.5) but the structured `meta` payload diverges. Extract the helper to `packages/1-framework/3-tooling/cli/src/utils/cli-errors.ts`, replace the two existing call sites in `migration-apply.ts` and `migration-plan.ts`, and replace the four inline mapping sites in `migration-status.ts` (around lines 364-368, 385-389, 444) and `migration-new.ts` (around line 161). After this lands, all five CLI commands produce identical envelope shape on `MigrationToolsError`.
  - **F15 — `migration new` contract-path resolution.** `migration apply / plan / status / show` use `resolveContractPath(config)` (defaults to `src/prisma/contract.json`); `migration new` uses inline `config.contract?.output ?? 'contract.json'` (defaults to `contract.json` next to the config). In a project that doesn't explicitly set `contract.output`, `prisma-next contract emit` writes to one default and `prisma-next migration new` looks somewhere else — a real user-facing inconsistency surfaced during T3.8 (the test had to align all five commands by setting `contract.output` explicitly in the shared mock). Replace `migration-new.ts:71-75`'s inline contract resolution with `resolveContractPath(config)` (imported from wherever `migration apply/plan/status/show` get it). After this lands, all five commands resolve the contract path identically by default.
- [ ] **T6.5 — Rename `MigrationGraph.migrationById` → `migrationByHash` (F16).** Phase 4 review surfaced a public-API naming inconsistency: `MigrationGraph.migrationById` retains the `migrationId` fragment while indexing by what is now called `migrationHash`. Bounded mechanical change (~12 sites) across `packages/1-framework/3-tooling/migration/src/graph.ts` (the type), `dag.ts` (the producer), `dag.test.ts`, and `packages/1-framework/3-tooling/cli/test/utils/graph-helpers.ts`. After this lands, `MigrationGraph` reads as `migrationByHash: Map<string, MigrationChainEntry>` with the `migrationHash` keys it already had — type and field name aligned. Kept separate from T6.1 (CLI helpers) because this is a `migration-tools` type, not a `cli` helper; one PR-review pass per package surface reads cleaner.

**Validation gate for Phase 6:** `pnpm test` passes. The audit at T6.3 returns no production hits. `pnpm build` across the workspace passes. After T6.4 (F12 portion), a spot-check confirms the four previously-divergent commands carry full `details` in their JSON envelopes (`migration status --json` against a tampered package shows `meta.details.dir` populated). After T6.4 (F15 portion), a spot-check confirms `migration new` uses the same default contract path as `migration apply/plan/status/show` (run with no explicit `contract.output` and confirm both commands look at `src/prisma/contract.json`).
...
| `migration new` hard-fails on tampered package when reading the existing graph | Integration | T3.8 | `migration new` calls `readMigrationsDir` to compute `from`. The original plan deferred this on grounds that T1.2 + the unified diagnostic make it covered by construction. Decision overridden during Phase 3 round 1 review (F14): the **UX surface** of `migration new` is distinct enough to warrant its own test — when the user asks to *create* a new migration but the command surfaces an error about an *existing* corrupt package, that's a confusing-on-its-face failure mode that deserves an explicit pin. T3.8 also strengthens T3.5's uniformity assertion by adding a 5th data point. |
