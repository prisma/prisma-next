...
- [ ] **T6.4 — Lift `mapMigrationToolsError` into a shared CLI helper (F12).** `migration apply` and `migration plan` define a private `mapMigrationToolsError(error)` that wraps `MigrationToolsError` into a `CliStructuredError` with the full `error.details` carried into `meta`. `migration status` and `migration new` instead inline a `MigrationToolsError.is(...)` check that drops `error.details` and surfaces only `{ code }` in `meta`. The result is a JSON-envelope inconsistency: identical user-facing diagnostic text across all five commands (proven by T3.5), but the structured `meta` payload diverges. Extract the helper to `packages/1-framework/3-tooling/cli/src/utils/cli-errors.ts`, replace the two existing call sites in `migration-apply.ts` and `migration-plan.ts`, and replace the four inline mapping sites in `migration-status.ts` (around lines 364-368, 385-389, 444) and `migration-new.ts` (around line 161). After this lands, all five CLI commands produce identical envelope shape on `MigrationToolsError`.

**Validation gate for Phase 6:** `pnpm test` passes. The audit at T6.3 returns no production hits. `pnpm build` across the workspace passes. After T6.4, a quick spot-check confirms the four previously-divergent commands carry full `details` in their JSON envelopes (`migration status --json` against a tampered package shows `meta.details.dir` populated).
