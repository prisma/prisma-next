...
- **[FOLLOW-UP — re-examine spec decision #4] Move field/named-type resolution into the symbol table; delete the per-package view layer.** The legacy `parsePslDocument` resolved field types once into `PslField` (split `typeName`/`typeNamespaceId`/`typeContractSpaceId`, `optional`, `list`, `typeRef`). Decision #4 ("resolution stays in interpreters") made `buildSymbolTable` structure-only, so BOTH interpreters now re-derive that read-set into `CstFieldView`/`CstNamedTypeView` — near-exact `PslField` clones — duplicated across the SQL and Mongo `contract-psl` packages (`cst-read.ts` + `cst-read-views.ts` + `symbol-views.ts` in each) plus a view-build pass. A symbol table that carried the resolved field shape on `FieldSymbol` (as the parser did) would need no per-package views and no duplication. Supersedes the earlier "extract shared CST-read adapters" framing: the better fix is to move resolution into the symbol table and delete the views, not to share the duplicated adapters. Re-examine decision #4 at the project retro. Surfaced in post-delivery review (operator question: the views rebuild what `parsePslDocument` already produced).
...
- **[RESOLVED in slice 3, dispatch 5d]** ~~Descriptor-typed extension-block parameters have no symbol-table home.~~ Resolved: `packages/1-framework/2-authoring/psl-parser/src/extension-block.ts` provides descriptor-aware reconstruction + runs the framework `validateExtensionBlock` over reconstructed blocks. policy_select round-trips at full parity. (Originally:) The legacy parser parsed declared block parameters (`ref`/`option`/`list`) descriptor-awarely at parse time; the symbol table defers all block-parameter parsing (a `BlockSymbol` carries the raw `GenericBlockDeclarationAst`). The SQL enum descriptor uses `parameters:{} + variadicParameters:true`, so slice 2 reconstructs the descriptor-free `{kind:bare|value}` path with full parity — no gap there. But any future extension block shipping a typed-parameter descriptor would need that richer parsing rehomed (into a consumer-side reconstruction, or back into `buildSymbolTable`). Slice 3 must decide where `pslBlockDescriptors`-driven validation lives, or record it as a project follow-up if no current consumer needs it. (Surfaced at slice-2 dispatch 2; confirmed out of scope for the enum seam.)
