...
- **A2** — Biome's plugin DSL can express the `as`-token recognition surface with the `as const` carve-out and per-line `// biome-ignore` honoured. *Verification:* a small prototype before committing to the biome path. *Fallback:* ts-morph standalone script.
- **A3** — Biome's `--only=<rule-id>` flag works for plugin rules in addition to built-in rules in biome v2.4.14. *Falsified if:* the flag doesn't apply to plugins. *Fallback:* `--reporter=json` + `jq` filter on the diagnostic `category` field.
...
- **OQ1 — Biome plugin DSL expressiveness for the recognition surface.** Can biome's plugin syntax distinguish `as const` from `as Foo` cleanly and honour per-line `// biome-ignore` comments? *Working position:* yes. *Verification:* a small prototype before committing to the biome path. *Fallback:* ts-morph script.
- **OQ2 — Helper package name.** *Working position:* `cast-utils`. Alternatives: `typesafe-utils`, `type-escapes`, `unsafe-utils`. Implementer decides during implementation; not load-bearing.
- **OQ3 — Biome rule ID.** *Working position:* `lint/casts/no-bare-cast`. Biome's plugin convention may dictate the group name; implementer picks during implementation.
