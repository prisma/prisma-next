...
- [ ] **3.2 PSL interpreter call site.** At the variant-merge block in `mongo-contract-psl/src/interpreter.ts`, scope each variant's indexes via the helper before pushing into the base collection. Conflicts surface as `PSL_INVALID_INDEX` diagnostics with the index attribute's span. Strengthens existing `interpreter.polymorphism.test.ts` "merges variant indexes" tests to assert `partialFilterExpression`; adds tests for user-supplied filters (compatible, matching, conflicting).
- [ ] **3.2.1 PSL `collectIndexes` rejects unknown field references.** Closes a pre-existing escapee surfaced during Phase 2 R1: the Mongo PSL interpreter silently emitted indexes referencing fields not declared on the model. `collectIndexes` now emits `PSL_INDEX_FIELD_NOT_FOUND` for any `@@index`/`@@unique`/`@@textIndex` field-list reference that names a field not declared on the model, with the index attribute's span. Polymorphic regression `@@index([title])` on `Bug` (where `title` is on `Task`) routes through this new diagnostic — closes the PSL half of AC-M3-07.
...
- [ ] **3.3.1 TS contract-builder rejects unknown field references in `index({ keys })`.** Mirror of T3.2.1 on the TS authoring surface. `buildCollections()` throws an `Error` naming the model, the index signature, and the missing field when an `index({ keys })` declaration names a field not present on `modelBuilder.__fields`. Closes the TS half of AC-M3-07 and the same pre-existing escapee on the TS path.
...
| Mongo PSL: `@@index`/`@@unique`/`@@textIndex` rejects unknown fields | Unit | 3.2.1 | `PSL_INDEX_FIELD_NOT_FOUND` with attribute span |
| Mongo PSL: variant-on-base-field emits field-not-found | Unit | 3.2.1 | Closes PSL half of AC-M3-07 |
| Mongo TS DSL: variant index parity with PSL | Unit | 3.3 | Contract-builder call site |
| Mongo TS DSL: `index({ keys })` rejects unknown fields | Unit | 3.3.1 | Throws Error naming model + index + missing field |
| Mongo TS DSL: variant-on-base-field throws | Unit | 3.3.1 | Closes TS half of AC-M3-07 |
