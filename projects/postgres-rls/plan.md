...
3. **Slice `authoring-breadth`** — Linear: [TML-2870](https://linear.app/prisma-company/issue/TML-2870)
   ...
   - **Focus:** authoring + lowering breadth only. No planner/verifier. Settles the one still-open design decision — the **per-operation vs single-array** helper signature (and how model-level enable/disable rides) — and confirms whether the old two-body-form ADR is subsumed by the PSL-block substrate's ADR (drop if so). Both are listed under spec § Alternatives § Still open.

4. **Slice `migration-verifier-breadth`** — Linear: [TML-2871](https://linear.app/prisma-company/issue/TML-2871)
   - **Outcome:** the remaining ops — `DropPostgresRlsPolicyOp`, `AlterPostgresRlsPolicyOp`, `DisableRowLevelSecurityOp` — per ADR 195's `OpFactoryCall` pattern, plus the full diff algorithm (compare declared vs introspected by full wire name → Create/Drop/Alter; `ALTER POLICY … RENAME TO` for matching-hash-different-prefix; drop+create fallback for shapes Postgres can't `ALTER` in place — the `ALTER POLICY` matrix in spec § Alternatives § Settled defaults). And the full `PostgresSchemaVerifier.verifyTargetExtensions()` (the empty stub): introspect `pg_policies`/`pg_class.relrowsecurity`, emit missing/extra/`rls_policy_tampered`/`rls_policy_renamed`/`rls_not_enabled` with severity via the control-policy two-layer dispatch (`classifySqlVerifierIssueKind` → `dispositionForCategory`). AC8: a manual `ALTER POLICY … USING(reformatted)` is `tampered`, not false drift.
...
## Alternatives considered (slicing)

- **Foundation-first diamond (original, rejected 2026-06-08):** `foundation → authoring ∥ migration-ops → verifier+skeleton`. It optimized for parallel-dispatch throughput, but its first slice (and three of four) shipped no consumer — the only way to answer "does it work?" was "the synthetic unit tests pass." For a feature whose dominant risk is *integration* (PSL→IR→DDL→live-DB enforcement), that defers all integration risk to the final slice. Replaced by tracer-first: prove the thread end-to-end early, then widen. The foundation slice's IR work was not wasted — every approach needs it — but it should have ridden inside a thin vertical thread rather than shipping alone (see [`learnings.md`](learnings.md) L1).
- **Split the tracer's two ops into their own slice (rejected):** would reproduce the consumer-less-slice problem at smaller scale. `CreatePostgresRlsPolicyOp` + `EnableRowLevelSecurityOp` are bundled into the tracer precisely so the tracer can *demonstrate* filtering, not just emit DDL.
- **Fold cross-space roles into authoring breadth (rejected):** cross-space role resolution is the riskiest authoring work and needs the Supabase-pack roles to exist; isolating it with the walking skeleton (slice 5) keeps slice 3 to same-space resolution (which the tracer already exercises) and avoids blocking authoring breadth on the pack.
