...
5. **`plan(start, end)` for the structural path.** The planner's structural diff→op path takes two schema IRs and returns DDL ops: it diffs (via the one differ), maps issue → op reading only nodes (+ the derivation-stamped op-render payload), and reads no contract for op-rendering. Contract→expected derivation moves to the callers (aggregate orchestration; the offline `migration plan` CLI op derives both sides). `keepDiffIssue` and every issue-vocabulary input on `plan()` are deleted; sibling-space scoping is principled because issues carry their nodes (with namespaces). The planner still receives the contract solely to run the **codec-hook subsystem** — codec type-operations (`CREATE EXTENSION`/`CREATE TYPE`) and field-lifecycle hooks (ADR 213) — which contribute codec-owned ops alongside the structural DDL and are not structural diffing. Those keep their contract input in this slice; unifying them onto the one differ so `plan()` becomes literally `(start, end)` is a follow-on slice (see Non-goals). The grep-clean proof is therefore scoped to "no structural op-render reads of the contract," not "zero contract reference."
