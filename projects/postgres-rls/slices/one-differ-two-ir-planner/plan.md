...
**Completed when:** `fixtures:check` byte-identical (the hard gate); planner/issue-planner/strategy suites (both targets) ported and green; no issue types in `plan()`'s signature; aggregate planner reads as explicit operations (no "strategy" vocabulary); the empty-graph case never calls the family planner.
