...
- [runtime-target-layer project / ADR 230](../../docs/architecture%20docs/adrs/ADR%20230%20-%20Runtime%20target%20layer%20session-coupled%20connections.md) — the parallel project shipping runtime role binding (session-coupled connections — `set_config(role/claims)` + `RESET ALL`) that makes RLS predicates run with the right `auth.uid()` value.
