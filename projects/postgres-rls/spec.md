...
This project can run in parallel with [cross-contract-refs](../cross-contract-refs/spec.md) and [runtime-target-layer](../runtime-target-layer/spec.md) once TML-2459 + control-policy have landed. Composition with cross-contract-refs is purely consumer-side (the TS `ref()` helper accepts cross-contract handles transparently); composition with runtime-target-layer is "this project ships the static contract, that one ships the dynamic per-query session-var injection."
...
- [runtime-target-layer project spec](../runtime-target-layer/spec.md) — the parallel project shipping the runtime `SET LOCAL` injection that makes RLS predicates actually run with the right `auth.uid()` value.
