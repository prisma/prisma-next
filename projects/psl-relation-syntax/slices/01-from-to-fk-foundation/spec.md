...
| Redundant `Model.` qualifier on `to:` (`to: Post.id`) | **DEFERRED to S5 (I12, found in M1).** The PSL expression grammar has no member-access *value* form — `to: Post.id` silently drops `.id` (resolves to `"Post"`). Supporting it needs a psl-parser grammar change, shared with S5 arrow-path's `J.b` member-access. For S1 authors write the bare column (`to: id`); the resolver already strips a qualifier when one appears (forward-correct), and the present boundary is pinned by a regression test. |
