...
**Decision (operator, per Will's PR comment + architect rec):** migrations follow the PSL convention. An absent `schema` on a `Migration` method defaults to the target's default namespace (`public` on Postgres); binding to the unbound/`search_path` namespace requires **explicit opt-in** (pass the unbound namespace id). Recorded in the method JSDoc and pinned by a test. Restores symmetry with PSL/TS authoring.
