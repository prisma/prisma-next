...
## Typed verification queries (TML-2889, D1) — settled decisions

_Settled by the D1 dispatch (spike folded into first implementation, validated end-to-end on PG `tableExists` + SQLite column-exists); commits `512252a93`…`232ad58d4`._

- **Contract-free expression projection:** a dedicated `CfExprSelectQuery` (`exprSelect().from(source).project(alias, cfExpr).where(cfExpr).build()`) plus a `cfExpr` helper namespace (`countStar`, `lit`, `identifierRef`, `param`) and predicate combinators on `CfExpr` (`isNull`/`isNotNull`/`eqLit`/`gtLit`/`eqParam`). Lives beside the existing column-proxy builder in `relational-core/src/contract-free/table.ts`. Computed projections carry no codec (runner reads first-row/first-column truthiness).
- **FROM-less SELECT:** `SelectAst.from` is optional (`SelectAst.noFrom()` factory); renderers emit no FROM clause when absent. `exactOptionalPropertyTypes` forced a `toOptions()` spread helper for the `with*` methods.
- **Function FROM-source:** core `FunctionSource` (kind `'function-source'`; `fn` + `args` + optional alias) as a third `FromSource` subclass — target-agnostic (the function name is just a string; no target-named field on core, honoring the slice-2 retro lesson). Exhaustive `AnyFromSource` consumers swept workspace-wide (render arms, `collectParamRefs`, middleware lints/budgets, where-binding).
- **Catalog scalar functions ride `OperationExpr` unchanged** (spec OQ3 confirmed): `to_regclass` as `{strategy:'function', template:'to_regclass({{self}})'}` with the qualified name as a text-codec `ParamRef`. Accepted rendering consequence: non-atomic operands parenthesize — `(to_regclass($1)) IS NULL` — semantically identical, pinned in tests.
- **Check-builder placement:** per-target contract-free surfaces (`target-postgres/src/contract-free/checks.ts` `tableExistsAst`; `target-sqlite/src/contract-free/checks.ts` `columnExistsAst`). Names bind as `ParamRef`s with explicit target text codecs — never inlined. Lowered-SQL tests live in adapter packages (adapter→target dependency direction).
- **Transitional residue (D3 closes):** PG `dropTable`/`createIndex`/`dropIndex` and the SQLite authored-migration facade keep clearly-marked legacy raw-string checks; the SQLite render-TS round-trip pins the divergence per-path until the facade converts.

**R1-01 refinement (round 2, commit `50c5ba68a`):** the catalog-function carrier is split — core `cfExpr.fn(options)` (`relational-core/src/contract-free/table.ts`) owns all `OperationExpr` + `'function'`-template assembly; target packages contribute only vocabulary wrappers (`toRegclass` in `target-postgres/src/contract-free/checks.ts`: template string + codec-bound operand + return spec). No `new <Node>(...)` survives at any converted check site (reviewer-verified F21 grep, including the two-operand `format_type` case D2 needs).

**D2 settled decisions (commits `f2a75c81c`…`134d9ced7`, reviewer-verified freeze):**
- **OQ4 (EXISTS input):** `cfExpr.exists/notExists` accept the buildable `CfExprSelectQuery` and `.build()` internally — call sites stay fluent.
- **Join surface:** `.join(source, on)` / `.leftJoin(source, on)` (inner + left only) + `.limit(n)` on `CfExprSelectQuery`; `cfTable(name, alias?)` for aliased catalog sources; `cfExpr.columnRef`/`eqExpr`/`allOf` for alias-qualified ON/WHERE assembly.
- **`cfExpr.raw(sql, returns)`** carries zero-operand opaque DB expressions (`current_schema()`) as `RawExpr` — single opaque string + mandatory `returns`; reviewer-judged narrow enough not to be a string-glue escape hatch.
- **Schema-filter polymorphism:** `PostgresSchema.schemaFilterExpression()` — named schema binds `nspname` as text param; unbound override returns `current_schema()` (no sentinel branching).
- **Authored-surface convergence (breaking, per #751 precedent + no-backward-compat):** constraint ops are `PostgresMigration` methods; bare factories removed from `@prisma-next/postgres/migration`; `renderTypeScript` emits the method form. Examples regenerated. Upgrade instructions to be recorded at PR-stage.
- **The contract-free substrate is FROZEN as of D2** (reviewer-certified against the complete audited D3/D4 inventory); D3/D4 add target-level vocabulary wrappers + consumer conversion only.

## D3 — Postgres sweep (commits `7ee9352ab`, `6da8760e1`, `344cec2fe`)

All remaining Postgres verification checks build via the contract-free builder and lower through the adapter:
- `planner-sql-checks.ts` helpers (column existence/nullability/type via `format_type`/default-presence/primary-key/table-empty), enum + extension inline checks, and the `planner-strategies.ts` consumer (incl. the not-null temporary-default recipe's `columnHasNoDefaultCheck` postcheck). The temp-default *execute-step* SQL (`buildAddColumnSql`) stays slice-7-deferred. `*Call.toOp()` are async + lowerer-required across the converted ops.
- **`instanceof` → structural predicate (commit `344cec2fe`).** `qualifyTableFromNamespaceCoordinate` (adapter `sql-renderer.ts`) detected an explicit `schema` via `instanceof PostgresTableSource`. The render-TS round-trip executes the scaffold in a `tsx` subprocess where `PostgresTableSource` loads from source while the adapter holds the dist copy — two class identities, so `instanceof` returned false and the schema qualifier silently dropped (`information_schema.columns` → `columns`). D3's catalog checks were the first to exercise this. Fixed with a structural `hasExplicitSchema` predicate (`'schema' in table && typeof table.schema === 'string'`) — also the repo-preferred shape (type-predicates over `instanceof`). Lesson: dual-loaded (dist vs source) class identity makes `instanceof` unreliable across the subprocess boundary; prefer structural checks in the renderer.

## Migration schema-default convention (PR #825 review — operator-decided)

The architect review of #825 (and Will's PR comment) surfaced an inversion: PSL/TS authoring defaults an un-namespaced model to `public` (via `target.defaultNamespaceId`), but the slice's new `Migration` methods default an absent `schema` to `UNBOUND_NAMESPACE_ID` (→ `PostgresUnboundSchema` → `current_schema()`/`search_path`). Latent today (planner passes an explicit contract-resolved schema; the codemod rewrote examples with explicit `'public'`), but the new hand-authoring API would let an absent `schema` silently bind to `search_path`.

**Decision (operator, Will, 2026-06-15 — supersedes the earlier "default to `public`" reading):** Postgres `Migration` methods make `schema` **required** — no default, no "unspecified namespace" option on Postgres. A migration must name its schema explicitly; a `search_path`-relative (unbound) default is ambiguity, and ambiguity in a migration is an antipattern. The unbound/unspecified namespace concept remains only for **Mongo/SQLite** (no real namespace concept). Implementation: remove `options.schema ?? UNBOUND_NAMESPACE_ID` across the Postgres migration methods and make the `schema` field non-optional; typecheck then forces every call site to be explicit. (Required, NOT defaulted-to-`public`.)
