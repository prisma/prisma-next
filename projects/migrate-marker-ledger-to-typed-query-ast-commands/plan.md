...
- **Assemble control-plane instances once in the orchestrator (TML-2856).** PR #751 fixed the per-operation re-instantiation inside the SQL family — `controlAdapter = adapter.create(stack)` is now built **once** when the family instance is constructed and held, mirroring how `createExecutionStack` builds the runtime adapter once; every `getControlAdapter()` caller and the new `family.adapter` member share that single held instance, and the planner takes `family.adapter` directly. What remains for TML-2856 is the deeper structural move: the family should not be the thing that owns the control adapter at all. `PostgresMigration`'s constructor still calls `stack.adapter.create(stack)` a **second** time, so the migration class and the family hold two different adapter instances. The control orchestrator (the CLI commands that already call `family.create(stack)` once) should construct the adapter (and driver) instance once, hold them, and thread the *same* adapter to the planner, the migration runner's marker bootstrap, and the migration class — then `lowerAst` / `bootstrap*` / `family.adapter` come off `SqlControlFamilyInstance` and the family stops owning or re-creating the adapter entirely.
