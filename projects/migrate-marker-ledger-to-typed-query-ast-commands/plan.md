...
- **Remove `lowerAst()` from the SQL family interface (TML-2856).** `SqlControlFamilyInstance.lowerAst` / `bootstrap*` are pure passthroughs to the control adapter; the family should not re-expose the adapter's lowering. PR #751 exposed `family.adapter` and pointed the PG `createPlanner` at it; TML-2856 deletes `lowerAst` and migrates the PG + SQLite runner marker-bootstrap callers to `family.adapter.lower()`.
