...
- **Focus:** integration test under `test/integration/` (follow the existing infer integration precedents); full gates: `pnpm build`, `pnpm typecheck`, Lint job (`lint:casts`, `lint:deps`, `lint:framework-vocabulary`), `pnpm fixtures:check`, package + integration suites.
