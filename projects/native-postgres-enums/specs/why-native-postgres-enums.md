...
> Shareable explainer (audience: users, no Prisma Next internals). Migrate to `docs/` at
> project close-out.
...
- A future feature will let you create a native Postgres enum yourself. But if you later remove or reorder its values, you'll own that migration — dropping the type, creating the replacement, and `ALTER`ing the column by hand.
