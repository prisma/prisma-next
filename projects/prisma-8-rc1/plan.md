...
- **Critical path** — the `.include()` decode class ([TML-3054](https://linear.app/prisma-company/issue/TML-3054/include-decode-breaks-for-most-non-trivial-native-types-codec), in flight): codecs' `decodeJson` contracts don't match what Postgres renders through `json_agg`, so a plain `DateTime` column read through `.include()` throws today, and `numeric`/`BigInt` values silently lose precision in the envelope parse (unfixable per-codec; needs raw-text JSON handling or SQL-side casts). The flagship read path failing on default types is the hardest "everything we ship works" breaker on the list; the fix must land — with the every-codec-through-`.include()` sweep test the ticket specifies — before the July 24 matrix-verdict freeze, or the affected matrix cells read broken.
- Date columns work end-to-end once the type-constructor work binds the postgres `Date` type to `pg/date@1`. The codec shipped unbound in [TML-3037](https://linear.app/prisma-company/issue/TML-3037/contract-infer-output-round-trips-through-contract-emit); until the pin lands, a `date` column read through `.include()` fails at runtime — an "everything we ship works" breaker. One line in the contribution table (`{ codecId: 'pg/date@1', nativeType: 'date' }`, via `codecId`, not a second `targetTypes` registration — marker comment sits at `pgDateDescriptor`); `infer-roundtrip-runtime.integration.test.ts` pins the broken behavior and flips red on the fix, telling the implementer the exact assertion to flip. Rides the mixins/type-constructor slice. (The date instance also needs TML-3054's decode fix — the two meet at the same read path.)
