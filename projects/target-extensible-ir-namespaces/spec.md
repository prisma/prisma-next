...
Deliver target-extensible IR + first-class namespaces — the substrate the downstream Supabase integration depends on. Three units of work: one sub-project (contract IR reshape proven by Postgres enum migration), one pre-EA slice (runtime SQL qualification + default-namespace DSL/ORM fallback), one post-EA slice (explicit namespace-aware DSL/ORM surface).

# Purpose

Make first-class namespaces and target-extensible IR usable for the downstream Supabase integration. The IR gains a pack-contributed entity-kind mechanism (proven by Postgres enum migrating off the framework-shared `types` slot); runtime SQL and the DSL/ORM qualify identifiers through a default-namespace fallback so EA users on single-namespace contracts experience zero query-API breakage; the explicit namespace-aware surface (`db.sql.auth.user`) ships post-EA as purely additive work.

# At a glance

This is the umbrella for the work that started as PR #534 (TML-2520 namespace exemplar, merged) and continues with two structural follow-ons before EA and one ergonomic follow-on after EA. The composition:
...
EA users on a single-default-namespace contract write zero query-code changes across S1+S2. Multi-namespace contracts that want explicit per-namespace navigation opt in to S3 when it lands.
...
**S3 adds the explicit namespace-aware DSL/ORM surface.** `db.sql.auth.user`, `db.auth.User`. Purely additive on S2 — default-namespace lookups keep working — so it ships post-EA without breaking anyone. Multi-namespace consumers opt in to explicit navigation when they need it.

The composition's two load-bearing properties:

1. **Pre-EA scope (S1 + S2) ships zero user-facing query-API breakage.** EA users on default-namespace contracts write the same query code they wrote before this umbrella.
...
- [ ] **PDoD1.** All three units (S1, S2, S3) delivered, or post-EA units (S3) explicitly deferred to a sibling initiative with the deferral recorded in `projects/target-extensible-ir-namespaces/deferred.md`.
...
- **FR6.** Explicit namespace-aware DSL/ORM surface (`db.sql.<ns>.<table>`, `db.<ns>.<Model>`) ships as purely additive on FR5. Delivered by S3 (post-EA).
