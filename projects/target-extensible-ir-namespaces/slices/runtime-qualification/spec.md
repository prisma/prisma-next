...
At **runtime** the contract carries the target only as an id *string* (`contract.target`), not the descriptor object, so the flat-name resolvers do not read a per-target default. The shared resolver (`resolveStorageTable` / `resolveDomainModel`) takes an **optional** `defaultNamespaceId`; when none is supplied it resolves a bare name target-agnostically by scanning the contract's namespaces (sole namespace, else insertion order). For the single-default-namespace contracts in this slice's scope that scan is exact. Explicit cross-namespace selection and bare-name collision ergonomics — the only cases where a target-derived default would change the answer — are TML-2550.
...
- A shared table/model resolver (`resolveStorageTable` / `resolveDomainModel`) with an **optional** `defaultNamespaceId` — target-agnostic scan when absent.
