...
| C | **Exact → managed transition** (the "second step" after adoption): replace `map:` with `name:` (body text kept verbatim) | managed replaces exact | Planner pairs the missing managed node with the extra exact node **by content** and emits `ALTER INDEX/POLICY … RENAME TO` — never drop+create. Requires `widening`; under additive-only, degrades to create-only (old object survives until a widening plan runs), matching the existing policy-rename degradation. |
