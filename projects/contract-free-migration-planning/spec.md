...
The Postgres derivation projects an `extension` node and an `Embedding` table node; the column node carries a resolved `dependsOn` edge to the extension node. The differ produces two `not-found` issues (extension, column) and copies the edge onto the column's issue. The planner topo-sorts: `CREATE EXTENSION vector` before the column. No contract read, no integer rank, no codec consulted at plan time — the order falls out of the graph.
...
2. **Codec-contributed operations ride the differ.** Postgres extensions and custom types become diff nodes populated by both derivations, retiring `storageTypePlanCallStrategy`'s read of `contract.storage.types` via `planTypeOperations`. The field-lifecycle planner ([ADR 213](../../docs/architecture%20docs/adrs/ADR%20213%20-%20Codec%20lifecycle%20hooks.md)) consumes the differ's column issues instead of running its own per-field contract diff. `native_enum` already made this move (it is a diff node with real ordering); extensions and custom types follow its precedent.
3. **Extensions and custom types are authored entities.** An `extension` (and a codec-defined custom type) is declared in PSL/TS, emitted as a Postgres-pack contract entity, and derived to a diff node — the single source of truth for its lifecycle. A codec declares which entities it requires; the framework validates the requirement generically.
...
- **Explicit declaration is a breaking change.** Once a codec's required entity must be declared, existing pgvector/postgis contracts that relied on implicit extension creation break at load until they add the declaration. The load diagnostic must name the exact entity to declare, and `contract infer` plus the published upgrade instructions must generate the declarations for existing databases — the roles playbook.
...
- An app declares `extension vector` in **both** PSL and TS; it emits to a Postgres-pack contract entity, derives to a diff node, and a migration creates it (and drops it on removal). Using a `requires`-bearing codec without the declaration is a load error naming the codec and the missing entity. *(strand 3)*
