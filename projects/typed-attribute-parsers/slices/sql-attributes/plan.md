...
### D7 — Kit `funcCall`/`funcCallFrom` + scalar/array-literal leaf; migrate `@default`
...
### D8 (optional) — Cleanup sweep
- Delete any orphaned syntax helpers; confirm `@db.*` helpers (`parseOptional*Argument`, `getPositionalArguments`) retained; refresh fixtures + code-parity snapshots.

_(Final `hands to` ⊇ slice-DoD: every SQL attribute spec-driven (D2–D7), syntax helpers gone (D2–D5, D7), semantic checks + `@db.*` helpers retained. D2/D3/D5 are mechanical once D1 lands; D1/D4/D6/D7 carry the kit-growth risk.)_
