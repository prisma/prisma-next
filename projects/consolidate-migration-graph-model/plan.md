...
1. **Slice `migration-graph-model-core`** — Linear: [TML-2740](https://linear.app/prisma-company/issue/TML-2740/migration-graph-model-tolerant-multi-rootmulti-tipcycle-core)
   ...
   - **Builds on:** None (external dependency: the shipped tolerant classifier, already merged). **Blocked on a decision, not code:** the list-view / TML-2716 fork (spec § Open Questions) sets this slice's model input type — resolve before starting.
...
- [ ] **Decision dependency — [TML-2716](https://linear.app/prisma-company/issue/TML-2716/adopt-contractspaceaggregate-in-migration-list-graph-log-delete-hand) (adopt aggregate in list/graph/log).** The list views are not on the aggregate, and the classifier we generalise from lives on that path. The spec § Open Questions fork decides whether this project depends on / folds in TML-2716 (option 1) or founds the model on `MigrationGraph` now and converges later (option 2). Must be resolved before slice 1 — it sets slice 1's input type.
