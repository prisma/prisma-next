...
1. **Slice `migration-graph-model-core`** — Linear: [TML-2740](https://linear.app/prisma-company/issue/TML-2740/migration-graph-model-tolerant-multi-rootmulti-tipcycle-core)
   ...
   - **Focus:** Model + tests in `migration-tools` (`migration-graph.ts`, `migration-list-graph-topology.ts`, `graph.ts`, `graph-membership.ts`). No CLI consumer changes. Decide the `isGraphNode` `∅` special-case disposition here (it's part of the model surface).
...
2. **Slice `migrate-read-only-consumers`** — Linear: [TML-2741](https://linear.app/prisma-company/issue/TML-2741/migrate-read-only-migration-commands-onto-the-consolidated-graph-model)
   ...
   - **Focus:** Read-only CLI surfaces only (`migration-status.ts`, `migration-log.ts`, `graph-migration-mapper.ts`, `migration-new.ts`, `ref.ts`, `plan-resolution.ts`). The plan/apply origin paths and the helper deletion are slice 3.
...
3. **Slice `migrate-plan-apply-and-delete-helpers`** — Linear: [TML-2742](https://linear.app/prisma-company/issue/TML-2742/migrate-planapply-origin-delete-golden-path-graph-helpers)
   ...
   - **Focus:** Plan/apply consumers (`migrate.ts`, `aggregate/strategies/graph-walk.ts`, `compute-extension-space-apply-path.ts`), the new multi-tip error, and the deletion. Storage formats and apply mechanics are out of scope.
