...
### G11 — Pre-publish API: extensions must vendor types

**Symptom.** The integration ships `[reference/cipherstash/stack/packages/stack/src/prisma/internal-types/prisma-next.ts](../../../reference/cipherstash/stack/packages/stack/src/prisma/internal-types/prisma-next.ts)` — 376 lines of hand-mirrored Prisma Next public types — because there is no published `@prisma-next/`* package yet. The vendored shapes can drift before the integration migrates.

**Why the extension needs this.** The CipherStash extension lives in a *separate* repository (`cipherstash/stack`) from Prisma Next. To compile, type-check, build, and ship its own npm package, it needs to import PN's types. Without a published `@prisma-next/`* peer dependency, its only options are: (1) vendor a forked PN checkout in `node_modules` (heavy; tightly couples versions), (2) hand-mirror the types it imports (the chosen workaround; fragile but lightweight), or (3) use `unknown` / structural-subset types with no compile-time guarantees (gives up DX and safety). The integration picked (2) and ate the drift risk explicitly.

Applicability: **universal among external extension authors**. Every third-party extension pack will hit this until PN ships to npm. Vector ops, FTS, geographic, time-series, audit — all in the same boat.

**Today's workaround.** Single file with explicit per-section provenance comments, intentionally narrow (only what the integration produces or consumes), runtime arktype check on `typeParams` (`encryptedStorageParamsSchema`) as a runtime safety net, and a `ContractLike` interface that pins only the contract fields the integration actually consumes:

```1:22:reference/cipherstash/stack/packages/stack/src/prisma/internal-types/prisma-next.ts
/**
...
The most-loose seam is `ArkSchema<TParams>` — typed as an opaque token because the integration doesn't want to take a hard dep on arktype's full surface. That means the `paramsSchema` slot is essentially `unknown`-typed at the boundary.

**What the framework should provide.** Publish (a) the public type packages (`@prisma-next/framework-components/codec`, `@prisma-next/sql-relational-core/ast`, `@prisma-next/sql-runtime`, `@prisma-next/family-sql/control`, `@prisma-next/sql-operations`, `@prisma-next/contract-authoring`) at a stable pre-1.0 cadence with semver-meaningful patch bumps, and (b) a clear "extension author surface" entry point that re-exports just the symbols extensions need (`BaseCodec`, `SqlCodec`, `CodecRegistry`, `SqlOperationDescriptor`, `SqlRuntimeExtensionDescriptor`, `SqlControlExtensionDescriptor`, `ComponentDatabaseDependencies`, `PlanTypeOperationsInput`, `StorageTypePlanResult`).

**Payoff.** Delete `internal-types/prisma-next.ts`. Rename imports across the integration. One commit, mechanical.
...
│   └── internal-types/  (until G11 lands)
...
| **G11** | **Framework** | Only PN can publish PN's types. |
...
Everything else is real but smaller. **G11** (publish the types) is purely mechanical and unblocks downstream tooling regardless of priority.

The "either-side" gaps (**G5, G13, G14, G15**) are nice-to-have framework helpers — the integration ships fine without them, and other extensions can roll their own. They're framework leverage opportunities, not framework correctness obligations.
