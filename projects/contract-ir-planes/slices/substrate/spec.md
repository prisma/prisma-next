...
- `Storage` interface ([`packages/1-framework/1-core/framework-components/src/ir/storage.ts`](../../../../packages/1-framework/1-core/framework-components/src/ir/storage.ts)) gains `elementCoordinates(): Generator<EntityCoordinate>`. `EntityCoordinate` is a new exported type `{ namespaceId: string; entityKind: string; entityName: string }`. Concrete `SqlStorage` and `MongoStorage` implement the walk over their built-in slots (`tables` / `collections`) plus any pack-contributed slots present on their namespaces.
- New `EntityCoordinate` type co-located with `Storage` or in a sibling file (implementer choice; OQ4 working position: same file).
...
1. Add `EntityCoordinate` type and the `Storage.elementCoordinates()` signature; implement the polymorphic walk on `SqlStorage` and `MongoStorage` over their existing built-in slots (`tables` for SQL — yielding `(namespaceId, 'tables', tableName)`; `collections` for Mongo). Pack-contributed slots are walked too, but the walk reads them off the namespace via the descriptor registry — if no descriptors register, the walk just covers built-ins.
...
| `Storage.elementCoordinates()` iterator vs array | Handle | Working position per spec OQ4: `Generator<EntityCoordinate>` for laziness — consumers usually filter, large contracts shouldn't materialise. |
...
| `DEFAULT_NAMESPACES` singleton injection in `SqlStorage` and `MongoStorage` constructors | Explicitly out | Spec marks for S1.D deletion. This slice leaves the injection in place; the new `elementCoordinates()` walk handles the default-namespace case via the registry just like any other namespace. |
| `extractStorageElementNames` is still the migration-loader's walker | Explicitly out | Spec marks for S1.D replacement. `Storage.elementCoordinates()` introduced now; replacement happens later. The two coexist in this slice. |
...
4. **`SqlStorage.elementCoordinates()` over the `types?` slot.** Should the walk yield `(ns, 'types', name)` entries for the framework-shared `types` slot? Working position: **yes** — the slot exists and contains entities; the walk should expose them for consumers. S1.B will remove the slot, at which point this yield naturally disappears. Alternative: only yield built-in family-slot entries; pack-contributed and `types` entries are filtered out. Rejected because consumers that need filtering can filter; consumers that need the entries can't synthesize them.
