...
- **B1.** S1.D merged before this slice starts (per sequencing — operator decision: "after we've completed the work on the critical path"). Technically the bug pre-dates S1.B; this slice could in principle parallelise with S1.C / S1.D, but is sequenced after S1.D for focus.
...
2. **Cross-namespace enum column reference resolution.** The column→enum lookup at `enumRebuildCallRecipe`'s column-ref walk (`column.typeRef === typeName`) is bare-name today. Working position: **investigate at D2 brief assembly** — if the column already carries namespace-qualified typeRef (object-pair encoding from S1.C, if S1.C has landed) the resolution is free; if not, narrow the filter by namespace as part of D2's scope. If S1.E lands before S1.C in some hypothetical resequencing, column-ref encoding is bare-name and D2 must add the namespace narrowing explicitly.
