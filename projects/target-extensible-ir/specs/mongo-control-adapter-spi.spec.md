...
- `packages/3-mongo-target/1-mongo-target/src/core/schema-verify/verify-mongo-schema.ts` (verifier shell + per-element diff/canonicalize helpers `contractToMongoSchemaIR`, `canonicalizeSchemasForVerification`, `diffMongoSchemas`) → `packages/2-mongo-family/9-family/src/core/schema-verify/`. _Updated M2.5 R1: original spec scoped the per-element helpers to stay at target-mongo for a future project; reconnaissance found they only depend on foundation packages (no target-mongo internals) and lifting them with the verifier shell removes a target → family → target import-shape hop. Lifted in this milestone; the future project line in `plan.md § Open items` removed._
...
- **`verifyMongoSchema`** lives at `packages/2-mongo-family/9-family/src/core/schema-verify/`; the per-element helpers (`contractToMongoSchemaIR`, `canonicalizeSchemasForVerification`, `diffMongoSchemas`) lift alongside it (M2.5 R1 in-flight scope expansion — see `§ 4` note).
...
- **Public API surface aligns with SQL.** `@prisma-next/family-mongo/control` exposes the `MongoControlTargetDescriptor` **interface** (type-only, no runtime cycle) but does NOT re-export the runtime `mongoTargetDescriptor` value — that lives at `@prisma-next/target-mongo/control` (its canonical home), mirroring how `family-sql` does not re-export `postgresControlTargetDescriptor` or `sqliteControlTargetDescriptor` (verified). Consumers importing the runtime value update their import path one hop. _Resolution adopted M2.5 R1 — the original AC stated "still exposes `mongoTargetDescriptor` (re-exported from target-mongo)" which is internally contradictory with the dep-severance AC: the re-export would require `family-mongo → target-mongo` runtime imports — exactly the inversion the milestone severs._
