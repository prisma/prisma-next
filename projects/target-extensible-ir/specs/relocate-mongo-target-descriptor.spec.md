...
> **Status (M2.5).** This task spec was originally a standalone project at `projects/relocate-mongo-target-descriptor/spec.md` (filed as TML-2505). After scoping discussion in `target-extensible-ir`'s M2 R2 cycle, the work was promoted to in-scope for the parent project and is now executed as M2.5 (sequenced after the post-M2 R2 PR 494 rebase, before M3). TML-2505 is marked duplicate of TML-2459. See `projects/target-extensible-ir/plan.md § M2.5` for the milestone task list and acceptance gate.
...
- **`@prisma-next/family-mongo` no longer depends on `@prisma-next/target-mongo` (or `@prisma-next/adapter-mongo`, `@prisma-next/driver-mongo`).** The family→target dep edge is severed; verify with `cat packages/2-mongo-family/9-family/package.json | rg "target-mongo|adapter-mongo|driver-mongo"` returning zero hits in the `dependencies` block.
- **The three Mongo SPI bases relocate from `@prisma-next/mongo-contract` (foundation) up to `@prisma-next/family-mongo` (family layer), matching where SQL's SPI bases land in M3 of the `target-extensible-ir` project.** Files: `MongoContractSerializerBase`, `MongoSchemaVerifierBase`, abstract `MongoStorageBase`. They were placed at `mongo-contract` foundation in target-extensible-ir M2 R1 as a workaround for the inverted dep edge (`family-mongo → target-mongo` made it impossible for `target-mongo` to extend a base in `family-mongo`); once the dep edge is severed, the bases belong at the family layer per the architectural-principles ADR's Principle 2 (cross-target consistency). Verify with `ls packages/2-mongo-family/9-family/src/core/ir/` containing the three base files; `ls packages/2-mongo-family/1-foundation/mongo-contract/src/ir/` empty (or containing only contract-types-related files, not SPI bases). Update `target-mongo`'s implementer classes (`MongoTargetContractSerializer`, `MongoTargetSchemaVerifier`, `MongoTargetStorage`) to import the bases from `@prisma-next/family-mongo/control` (or wherever the family barrel surfaces them).
- `pnpm lint:deps` reports zero layering violations involving `mongo-target-descriptor` / `control-target` / Mongo SPI bases (and zero new violations elsewhere as a consequence).
