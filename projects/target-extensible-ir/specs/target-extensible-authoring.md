...
- **Where exactly does the lifted `composed-authoring-helpers.ts` live?** Likely `@prisma-next/contract-authoring` (a new family-agnostic package) but the precise package name and layering placement is decided in M3.5 implementation. The close-out promotion should pin the location and update this ADR's Decision section to name it.
- **Does the `output` discriminator stay `template | factory`, or generalise further?** M3.5 ships exactly two output kinds. Future contributions might want a third (a hybrid that takes the template's evaluation as input to a factory; or a "no-op" that just registers the contribution for type narrowing without producing a runtime value). Close-out should review whether the two-kind discriminator is the steady state or just the M3.5 floor.
- **`ctx` parameter on the factory.** The factory signature `(input, ctx) => SchemaNode` admits a `ctx` argument for the family/target context (codec lookup, namespace registry, etc.). M3.5 will discover what `ctx` actually needs to carry; this ADR's signature is a placeholder. Close-out should sharpen the `ctx` shape against what M4 and M5a actually call.
- **Backport timing for `field` and `type`.** TML-2513 tracks backporting the descriptor + factory output dispatch to the existing `field` and `type` namespaces so all three dispatch identically. The backport is mechanical (the dispatch code is one switch) but its ADR-level treatment is open: the close-out promotion can either include the backport as part of the same ADR (one unified contribution shape across three namespaces) or keep this ADR scoped to `entities` and let TML-2513 ship its own ADR appendix. The choice depends on TML-2513's landing time relative to the close-out PR.
- **Are there `entities` contributions whose output is naturally template-shaped?** M3.5's exemplars (enum, namespace) both want factory output (because their outputs are IR-class instances). If no in-tree `entities` contribution ever uses template output, the alternative-1-derived "templates are the lowest common denominator" framing is *aspirational* rather than load-bearing for this namespace. Close-out should either find a template-shaped exemplar (perhaps a "no-op" contribution that registers a kind for type narrowing) or document that `entities` contributions are factory-shaped by nature and the template branch exists for shape-uniformity with `field` / `type`.
