...
- `interface Namespace extends IRNode { readonly id: string }` — first-class building block. Sentinel id `'__unbound__'` reserved for the late-bound storage slot — the slot whose binding the target resolves at connection time (resolved by per-target singleton subclasses, not by call-site branches). The PSL AST identifier `'__unspecified__'` (the framework parser's name for the syntactic-absence bucket) is a different concept at a different layer; see FR14 / FR16a / FR16d.
- `abstract class NamespaceBase implements Namespace` — convenience base for target Namespace concretions.
- `interface Storage { readonly namespaces: Record<string, Namespace> }` — every IR carries namespaces. Type-level enforcement of "every storage object belongs to a namespace" (FR15). PR1 honours this on both families: Mongo's `MongoStorageBase` declares `abstract readonly namespaces`; SQL's `class SqlStorage` ships an `__unbound__` placeholder namespace (`SqlUnboundNamespace.instance`) so the framework promise is structurally honoured at the SQL family layer from PR1 forward. PR2's M5a swaps the placeholder for per-target namespace concretions (`PostgresSchema`, `SqliteUnboundDatabase`).
...
### `__unbound__` is a target-specific singleton subclass

Each target owning a `Namespace` concretion ships a singleton subclass (`PostgresUnboundSchema extends PostgresSchema`, `MongoTargetUnboundDatabase extends MongoTargetDatabase`) with `readonly id = '__unbound__' as const`. The subclass overrides the namespace's qualifier-emission methods to elide the prefix. Call sites stay polymorphic — there are **no `if (namespace.id === '__unbound__')` branches anywhere in the codebase**. The framework's promise is one stable static reference per target (`PostgresSchema.unbound`).
