...
>
> **Project:** [target-extensible IR (TML-2459)](../spec.md). Refined throughout M2–M5b as the principles are exercised against a second family (Mongo, M2), the SQL targets (M3), enums (M4), and the namespace exemplar (M5a/b).
>
> **Companion ADR:** [3-layer polymorphic IR convention](3-layer-polymorphic-ir-convention.md). This ADR captures the principles; the companion captures one concrete instance of them.
...
- **The affordance/specifics boundary inside the family.** The fuzzy-edge cost above. M2–M5b will exercise the boundary multiple times (Mongo Storage, enums, namespaces, FK refs). Close-out should either codify a sharper rule from the project's experience or document the judgement-call status explicitly.
- **Does principle 2 admit *partial* fluency?** A developer fluent in Postgres reading Mongo for the first time: are they "fluent in another" because the framework affordances carry, or are they re-learning the document model? The principle is honest about the *form* carrying; it does not promise that *all content* transports. Close-out should sharpen the wording — "fluent in another's architecture" rather than "fluent in another", perhaps.
- **Relationship to "Thin core, fat targets".** This ADR enriches the existing principle. Does the close-out replace "Thin core, fat targets" entirely, or list it alongside the two new principles? The M6 plan task ("update § Guiding Principles to surface" both new principles) leaves this open. Either reading is consistent with the spec; close-out should pick one.
- **Naming and ADR number.** This ADR is drafted as "architectural principles". The close-out promotion needs a permanent name and number. Candidates: "Architectural principles underwriting the 3-layer IR convention" (long but accurate), "Affordances and cross-target consistency" (shorter, less context-laden). The permanent name should be picked at close-out alongside the 3-layer convention ADR's permanent name so the two read as a pair.
