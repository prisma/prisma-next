...
4. **No call site in M2 should branch on `namespace.id === '__unbound__'`.** (At the time M2 landed this read as `'__unspecified__'`; PR2's M5a rename swapped the IR sentinel to `'__unbound__'`.) The ADR + namespace.ts comments commit to the singleton-subclass pattern; the reviewer should grep for the literal string in the M2 commits and flag any branch as a `must-fix` finding.
