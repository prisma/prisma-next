...
- **Mongo descriptor-seam asymmetry vs SQL → tracked as [TML-2505](https://linear.app/prisma-company/issue/TML-2505/relocate-mongo-target-descriptorts-into-target-mongo-fix-mongo-vs-sql) with project spec at `projects/relocate-mongo-target-descriptor/spec.md`.** Mongo's `mongoTargetDescriptor` lives in `@prisma-next/family-mongo`; SQL's targets each carry their own `control-target.ts` at the target layer (Postgres at `packages/3-targets/3-targets/postgres/src/exports/control.ts`; SQLite at `packages/3-targets/3-targets/sqlite/src/core/control-target.ts`). The asymmetry is a historical artefact of Mongo currently having a single target, and is the root cause of the M1 placement defect surfaced in M2 R1 (see orchestrator note in `code-review.md`). The placement-fix in M2 R1 unblocks this project's spec'd scope but leaves the descriptor-seam asymmetry in place. PR 494 (TML-2408 contract-spaces-mongo-port) deepened the asymmetry by adding multi-space runner wiring to the family-resident descriptor — the trigger for filing the relocation as a discrete follow-up. Out of scope for this project; tracked durably at TML-2505 + the sibling `projects/relocate-mongo-target-descriptor/`.
