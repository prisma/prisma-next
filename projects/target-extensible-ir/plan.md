...
- **M2 R2**: Full Contract IR class flip — `MongoIndex`, `MongoIndexOptions`, `MongoCollationOptions`, etc. from `type =` to AST classes extending `SchemaNodeBase`, with the cascade across foundation + authoring (TS + PSL) + query-ast + mongo-target + mongo-adapter.
...
- [ ] **(M2 R2)** Flip Mongo Contract IR (`MongoIndex`, `MongoIndexOptions`, `MongoCollationOptions`, etc.) from `type =` data shapes to AST classes that extend `SchemaNodeBase`, following the same recipe as Mongo's existing `MongoSchemaNode` AST.
