...
**Detailed task spec:** [`projects/target-extensible-ir/specs/mongo-control-adapter-spi.spec.md`](specs/mongo-control-adapter-spi.spec.md). The task list below summarises the milestone's nine orchestrated changes; the spec carries file-by-file detail and acceptance criteria.

**Tasks:**

- [ ] **Define `MongoControlAdapter` SPI** at `packages/2-mongo-family/9-family/src/core/control-adapter.ts` (mirrors `SqlControlAdapter`). Carries `readMarker`, `readAllMarkers`, `initMarker`, `updateMarker`, `writeLedgerEntry`, `introspectSchema`, plus `formatMongoOperations` (or that lives at family-mongo per implementer's call — see Open Question #1 in the spec). Re-export from `packages/2-mongo-family/9-family/src/exports/control-adapter.ts`.
...
- [ ] **Relocate `verifyMongoSchema`** (verifier shell) from `target-mongo/schema-verify` → `family-mongo/schema-verify` (mirrors `verifySqlSchema` placement). Per-element diff/canonicalize helpers (`contractToMongoSchemaIR`, `canonicalizeSchemasForVerification`, `diffMongoSchemas`) stay at target-mongo (lift recorded as future project in `plan.md § Open items`). Relocate `formatMongoOperations` to family-mongo (or onto the SPI).
- [ ] **Refactor `family-mongo/control-instance.ts`** to dispatch every target/adapter call via the `MongoControlAdapter` SPI surfaced by the control stack. Match SQL's `getControlAdapter()` helper pattern (structured error if adapter is missing or doesn't implement the SPI shape). Replace `MongoTargetContract` with the SPI's contract-shape generic.
