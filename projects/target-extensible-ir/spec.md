...
- [x] **AC12a (mechanism-level subclauses, M3.5 R1).** The `AuthoringContributions.entities` namespace exists; both family contract-builders (SQL + Mongo) consume the lifted family-agnostic scaffolding in [`@prisma-next/contract-authoring`](../../packages/1-framework/2-authoring/contract/src/composed-helpers-scaffolding.ts); pack-bag-driven type narrowing surfaces contributed entity helpers at `helpers.entities.<entityName>` with input typing inferred from the descriptor's factory signature. An in-tree synthetic pack exemplar at [`packages/2-sql/2-authoring/contract-ts/test/entities-namespace.exemplar.test.ts`](../../packages/2-sql/2-authoring/contract-ts/test/entities-namespace.exemplar.test.ts) verifies the type-narrowing, runtime construction, and JSON-cleanliness paths.
