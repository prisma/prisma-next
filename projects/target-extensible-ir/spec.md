...
- **FR16a.** The authoring DSL exposes namespace declarations in both PSL and the TS builder, with surfaces kept structurally parallel so moving between them is mechanical for users.
  ...
  **PSL surface.** A top-level `namespace <name> { … }` block can contain model, enum, and composite type declarations. Namespace blocks live only at the top level — they cannot recursively contain other namespace blocks (a `namespace a { namespace b { … } }` form is a parse error; this mirrors the flat-schema reality of every supported database). Blocks are **reopenable**: multiple `namespace foo { … }` blocks in the same file (or across files in a multi-file contract) merge into one AST entry. The framework parser is purely syntactic and reserves no identifiers — `unbound`, `public`, `auth` are all just strings. Bare-name resolution within a namespace: local namespace first, then `__unspecified__` (the syntactic-absence bucket), then error.

  **`types { … }` blocks stay document-scoped.** Named-type declarations live in a single document-level `types { … }` block, **outside** any `namespace { … }` block — they are not nested under namespaces. Rationale: named types are a contract-wide naming convenience (a way for users to alias scalar / composite shapes) rather than database objects bound to a particular schema; per-namespace `types { … }` would buy a symmetry users do not ask for at the cost of multiplying the parser refactor's surface. Today's parser already enforces document-scoped `types`; this preserves that behaviour. A `types { … }` block appearing inside a `namespace { … }` block is a parse error.
