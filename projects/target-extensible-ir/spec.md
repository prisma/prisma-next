...
**Asymmetric naming, deliberate.** The contribution surface (`AuthoringContributions.entityTypes`) names the *kinds of entity* a pack contributes — enum, namespace, RLS policy. The user-facing helpers surface (`helpers.entities.<name>(input)`) names the *entities* a contract author constructs — a `Status` enum, a `public` namespace, an `AdminPolicy`. The asymmetry reads honestly at each altitude: a pack ships entity types; a contract author constructs entities. Both surfaces use the word the consumer of *that* surface naturally reaches for. Surfaced as a typology hole during PR1 review (architect SDR § 3.1); resolved in M7.
...
- **FR8b.** Pack-bag-driven type narrowing is symmetric with the existing `field` / `type` mechanism. The `ComposedAuthoringHelpers<Family, Target, ExtensionPacks>` template gains an `entities` member that merges contributions from each pack's `entityTypes` namespace via the existing `MergeExtensionXxx<Packs>` + `UnionToIntersection` template. **The asymmetric naming is intentional** (see Authoring surface § "Pack-contributed entity construction" above): packs ship `entityTypes` (kinds); the user constructs `entities` (instances) at `helpers.entities.<entityName>(input)`. The composed-helpers template performs the one-step rename in the type system (`entityTypes` contribution slot → `entities` helper namespace); no runtime cost. Type narrowing on `input` is full per the contributing pack's descriptor schema.
