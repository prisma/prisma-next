...
- **5.11** Complete `@prisma-next/framework-components` extraction and introduce foundation layer. Task 3.10 creates the package with component types, assembly, and stack creation. This task finishes the extraction: move emission SPI types (`TypesImportSpec`, `RenderTypeContext`, `TargetFamilyHook`, etc.), authoring contribution types, and `checkContractComponentRequirements()` from `@prisma-next/contract` to `@prisma-next/framework-components`. Remove backward-compat re-exports left by task 3.10. Introduce a `foundation` layer below `core`; move `utils`, `plan`, and `contract` to `0-foundation/shared/` — making the contract a true leaf with zero framework-domain dependencies. See [detailed plan](plans/5.11-extract-framework-components-package-plan.md).
...
6. **Descriptor assembly consolidation (tasks 3.9 → 3.10), control flow inversion (task 5.8), and full framework-components extraction (task 5.11).** The descriptor-based assembly machinery (`extractCodecTypeImports`, `extractOperationTypeImports`, `DescriptorWithTypes`, etc.) currently lives in `packages/2-sql/3-tooling/family/src/core/assembly.ts`. This is family-agnostic logic that operates on `ComponentMetadata`. Task 3.9 extracts it into `@prisma-next/contract/assembly` and builds a Mongo control plane at layer 9 using the inverted pattern. Task 3.10 creates `@prisma-next/framework-components`, moves component types and assembly from `@prisma-next/contract`, renames to the established `createControlStack()` / `createExecutionStack()` convention, merges stack creation from the plane packages, and wires the Mongo demo through both planes (backward-compat re-exports left in place). Task 5.8 completes the inversion: the framework orchestration layer (`ControlClient`, CLI) drives assembly and calls `emit()` directly, family instances no longer own emission, and the SQL family package moves to layer 9. Task 5.11 finishes the extraction: moves emission SPI types, authoring contribution types, and `checkContractComponentRequirements()` to `@prisma-next/framework-components`, introduces the `foundation` layer below `core`, removes backward-compat re-exports, and makes `contract` a true leaf package.
