...
- **3.12** ✅ *(Follow-up, M3 addendum)* Absorb `@prisma-next/core-execution-plane` into `@prisma-next/framework-components` and migrate consumers off `@prisma-next/contract/framework-components`. The `core-execution-plane` package has drifted from its name — it contains only framework component types (`Runtime*Descriptor`, `Runtime*Instance`, `ExecutionStack`, `createExecutionStack()`, `assertRuntimeContractRequirementsSatisfied()`), not runtime pipeline logic. Moved all 3 source files into `@prisma-next/framework-components` as a new `./execution` subpath, splitting `types.ts` into `execution-instances.ts` and `execution-descriptors.ts`. Converted `@prisma-next/core-execution-plane` to a backward-compatible re-export shim. Migrated ~65 consumers of `@prisma-next/contract/framework-components` to import directly from `@prisma-next/framework-components/{components,authoring}`, deleted the `contract/framework-components` re-export module and removed the `@prisma-next/framework-components` dependency from `@prisma-next/contract` (eliminating the dependency inversion). Also split `framework-components.ts` into `type-renderers.ts` + component descriptors (F08), replaced `AssemblyInput` with `ComponentDescriptor` (F06), added `queryOperationTypes` to `ComponentMetadata`. **Deferred:** F12 (PSL interpretation assembly functions are framework-common) — address during task 3.11.
- **3.13** ✅ *(Follow-up, M3 addendum)* Consolidate control-plane types into `@prisma-next/framework-components`. Move `Control*Instance` (minus `toSchemaView`), `Control*Descriptor` (minus `migrations`), and result types from `@prisma-next/core-control-plane` and `@prisma-next/config` into `@prisma-next/framework-components/control`. Define capability interfaces (`MigratableTargetDescriptor` + `hasMigrations()`, `SchemaViewCapable` + `hasSchemaView()`) with type predicates in `core-control-plane` for dependency-inverted access to migration and schema-view capabilities. Extend `ControlStack<TFamilyId, TTargetId>` with descriptor fields so it subsumes `ControlPlaneStack` (now a deprecated type alias). Change `ControlFamilyDescriptor.create()` to single-arg `create(stack: ControlStack)`. Convert `core-control-plane` and `config` to re-export shims. Convert `createControlPlaneStack()` to a deprecated shim re-exporting `createControlStack()`. Shim removal deferred to task 5.11. See [detailed plan](plans/3.13-consolidate-control-plane-types-into-framework-components-plan.md).
...
- **5.8** Complete the control flow inversion for descriptor assembly. Tasks 3.9, 3.10, 3.12, and 3.13 extract assembly to the framework, rename to `createControlStack()` / `createExecutionStack()`, and build the Mongo family using the inverted pattern; this task migrates the SQL family to the same pattern and moves the operation registry to the framework. Key changes: (1) add `operationSignatures` to `ComponentMetadata` and assemble `OperationRegistry` in `createControlStack()` — operations are a cross-family concept; (2) change `emit()` to accept `ControlStack` instead of `EmitOptions` — eliminating redundant destructuring; (3) the framework orchestration layer (`ControlClient`, CLI `executeContractEmit`) calls `emit()` directly — `emitContract()` is removed from `ControlFamilyInstance`; (4) SQL family instance reads pre-assembled values from `ControlStack` instead of re-running `extract*` functions; (5) move SQL family package from `packages/2-sql/3-tooling/family/` to `packages/2-sql/9-family/` (layer 9, top of the import hierarchy). **Prerequisite:** Phase A (tasks 5.A1–5.A6) should land first — both branches modify `emit()`, `ControlClientImpl.emit()`, and family `emitContract()` methods. See [detailed plan](plans/5.8-control-flow-inversion-plan.md).
