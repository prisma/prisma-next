...
- **5.8** Complete the control flow inversion for descriptor assembly. Task 3.9 (M3) extracts assembly to the framework and builds the Mongo family using the inverted pattern; this task migrates the SQL family to the same pattern. The framework orchestration layer (`ControlClient`, CLI `executeContractEmit`) calls `assembleComponents()` and `emit()` directly — family instances no longer own emission. `ControlFamilyDescriptor.create()` receives pre-assembled state. Move the SQL family package from `packages/2-sql/3-tooling/family/` to `packages/2-sql/9-family/` (layer 9, top of the import hierarchy). Update runtime assembly if applicable. See [detailed plan](plans/5.8-control-flow-inversion-plan.md).
- **5.9** Update all tests, fixtures, and type tests for the unified contract type.
- **5.10** Run full test suite and typecheck.
- **5.11** Extract `@prisma-next/framework-components` package and introduce foundation layer. Move framework component descriptors, instances, pack refs, type renderers, authoring types, emission SPI types (`TypesImportSpec`, `RenderTypeContext`, `TargetFamilyHook`, etc.), assembly functions, and `checkContractComponentRequirements()` out of `@prisma-next/contract` into a new `@prisma-next/framework-components` package at `1-core/shared/framework-components/` (layer: core, plane: shared). Introduce a `foundation` layer below `core`; move `utils`, `plan`, and `contract` to `0-foundation/shared/` — making the contract a true leaf with zero framework-domain dependencies. Framework-components depends downward on contract (Clean Architecture DIP). Exposes plane-specific assembly subpaths: `@prisma-next/framework-components/control` (control-plane assembly from task 3.9) and `@prisma-next/framework-components/runtime` (placeholder). See [detailed plan](plans/5.11-extract-framework-components-package-plan.md).
...
6. **Descriptor assembly consolidation (task 3.9), control flow inversion (task 5.8), and framework-components extraction (task 5.11).** The descriptor-based assembly machinery (`extractCodecTypeImports`, `extractOperationTypeImports`, `DescriptorWithTypes`, etc.) currently lives in `packages/2-sql/3-tooling/family/src/core/assembly.ts`. This is family-agnostic logic that operates on `ComponentMetadata`. Task 3.9 extracts it into `@prisma-next/contract/assembly` and builds a Mongo control plane at layer 9 using the inverted pattern. Task 5.8 completes the inversion: the framework orchestration layer (`ControlClient`, CLI) drives assembly and calls `emit()` directly, family instances no longer own emission, and the SQL family package moves to layer 9. Task 5.11 extracts all component descriptors, assembly, authoring types, and emission SPI types out of `@prisma-next/contract` into `@prisma-next/framework-components`, introduces a `foundation` layer below `core`, and makes `contract` a true leaf package.
