...
- [ ] **T2.5 — PSL constructor + pack-meta.** Add `src/core/authoring.ts` + `src/core/descriptor-meta.ts` + `src/exports/pack.ts`. Re-author from PR #416 commit `c38863d37`. PSL `cipherstash.EncryptedString({ equality, freeTextSearch })` registered via the pack-meta surface (rides on M1's boolean `AuthoringArgumentDescriptor` kind). The cipherstash descriptor in `src/exports/control.ts` extends to wire pack-meta alongside the existing codec-hook block. Clears AC-CTOR1..4, AC-LOWER1..3, AC-ALIAS1..2.
  - **Bundled plan amendment (from M2 R1 review item 1):** add cipherstash entries to `architecture.config.json` mirroring pgvector's pattern (`packages/3-extensions/pgvector/**` with appropriate layer/plane). Once mapped, AC-UMB6's caveat dissolves and `pnpm lint:deps` enforces on cipherstash imports.
  - **Bundled plan amendment (from M2 R1 review item 2):** clean up the 5 pre-existing `projects/extension-contract-spaces/**` references in `packages/3-extensions/cipherstash/README.md` (4) and `test/descriptor.test.ts` (1). Replace per `.cursor/rules/doc-maintenance.mdc` (link to durable `docs/` or package READMEs instead). Pattern: pgvector's README + the F3 resolution from PR #416.
...
8. **Codec-SDK binding refactor — deferred out of Project 1.** [TML-2388](https://linear.app/prisma-company/issue/TML-2388) (Medium-priority framework-symmetry follow-up). The runtime codec captures `CipherstashSdk` in its `decode` closure, which collides with pack-meta consumers that read codec metadata at contract-emit time before any SDK binding exists. The clean fix is to thread SDK (or per-call context) through `CodecCallContext` rather than capturing it at codec construction; that refactor touches every codec in the repo and is outside Project 1 scope. *(Note from M2 R2: the SDK-free metadata-codec split landed in T2.5 — `src/core/codec-metadata.ts` — is a partial mitigation for the pack-meta side; it sidesteps the collision for emit-time consumers without solving the underlying framework-level binding question.)*
9. **Repo-wide `pnpm test:packages` flakiness — surfaced from M2 R2 review.** First uncached run intermittently fails `@prisma-next/cli` (4 tests in `control-api/contract-emit.test.ts > serializes overlapping emits` + `control-api/client.test.ts > ControlClient progress emission > no onProgress callback`) and `@prisma-next/adapter-postgres` (transient). All pass cleanly in isolation; cached re-runs are green. Verified independent of cipherstash (M2 R2 diff doesn't touch either package). Likely test-isolation or shared-fixture race under high-concurrency execution. Not blocking M2 R3 / M3 / M4 (cached gates remain green); recommend the user file a separate hygiene ticket.
