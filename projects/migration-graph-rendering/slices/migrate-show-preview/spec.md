...
1. The **Tier-3 graph tree** (shared renderer with `graph`/`status`), rendering the **whole** graph — nothing omitted. The chosen path renders in **bright green** across its **whole span** — the node markers (`○`/`∅`) at both ends, hashes, migration names, **and every lane/branch-line segment** of the path (not just the final `│↑`). Every **off-path** node, hash, migration, and lane line renders in **uniform dim grey** (fully drawn, just grey). **The green/grey path highlight overrides the `git log --graph`-style rotating lane colours** (the colored-lanes feature) — an off-path branch must be grey, never its rotation colour (e.g. red), and on-path must be green regardless of column. Marker placement: **`@contract` marks the app's working-contract node and is rendered only in the app space — never in an extension space**; `@db` marks the live-marker node; the `--to` ref marks its own node in parens. (`@contract` is the working contract, *not* the `--to` target.)
