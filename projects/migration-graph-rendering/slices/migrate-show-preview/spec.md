...
1. The **Tier-3 graph tree** (shared renderer with `graph`/`status`), rendering the **whole** graph — nothing omitted. **Colour model: on-path = ORDINARY colours; off-path = DIM grey. Dimming the off-path is the SOLE differentiator — there is NO bright-green highlight.** On-path migrations, nodes, hashes, names, and lane segments render exactly as a normal graph row does (normal text, normal lane colours) — untouched. Every **off-path** cell — node marker, node contract-hash, migration name, the edge `from → to` hashes **including the destination hash**, AND its lane segments — renders **dimmed grey**. The on/off-path classification is **per lane segment** following the actual path: a lane/trunk segment that carries an **on-path** edge stays ordinary; one carrying an **off-path** edge is dimmed. (Both observed bugs are the same lane-routing defect — an off-path trunk segment showing colour when it should be dim, and an on-path trunk segment dimmed when it should be ordinary.) Marker placement: **`@contract` marks the app's working-contract node and is rendered only in the app space — never in an extension space**; `@db` marks the live-marker node; the `--to` ref marks its own node in parens. (`@contract` is the working contract, *not* the `--to` target.)
2. A **linear, ordered list** of the migrations that will execute — rendered in the **same row format as the graph's migration rows** (minus the graph gutter), in **ordinary colours** (matching the on-path rows, not green), printed directly (no Clack `│` gutter). **Column alignment is shared across ALL sections: the `app:` graph section, every extension graph section (e.g. `pgvector:`), AND this run-list must align their data columns (migration name, `from → to` hashes, `N ops`) to the same offsets** — compute the dirName/hash column widths GLOBALLY across all spaces and the list, not per-space. (Today each space self-aligns and the list uses a separate width → mismatch.) **The order MUST be the runner's canonical cross-space schedule order — extension spaces (alphabetically by space id) FIRST, then the app space — sourced from the SAME ordering mechanism the runner uses (`concatenateSpaceApplyInputs` / the runner's `applyOrder`), NOT reconstructed from a per-space loop.** The preview's run order must be byte-for-byte the order `migrate` actually applies (extensions before app) — this is part of the faithfulness constraint (D-MS6), not just presentation. Script/loop-friendly.
