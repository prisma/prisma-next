...
- **FR4.** The disclosure-and-consent surface is the interactive `prisma-next init` prompt. After the existing init questions (target, authoring, schema path, optional `.env` write, optional facade-removal on re-init), `init` adds a final `clack.confirm` prompt asking the user whether to enable anonymous CLI usage telemetry. The prompt appears iff (i) the resolved value of `enableTelemetry` in the user-level config file is undefined (file missing or field absent), (ii) `init` is in interactive mode (`canPrompt === true`), and (iii) `--yes` is not auto-accepting prompts (consent must be a deliberate keystroke, not a side effect of automation). Final wording: _"Help us prioritize features by sharing anonymous CLI usage data? The telemetry implementation is open source at https://github.com/prisma/prisma-next (packages/1-framework/3-tooling/cli-telemetry and apps/telemetry-backend)."_ with a default of **Yes** (`initialValue: true`, matching ecosystem precedent in similar OSS tooling such as Next.js and Astro). The user's response is persisted to `config.json` as `enableTelemetry: true | false`; on an affirmative answer, the `installationId` UUID is generated and persisted in the same write. The prompt is never re-shown once `enableTelemetry` is set; changing the answer later requires editing the file (or, as a future improvement out of Phase 1 scope, a dedicated `prisma-next telemetry` command). No other CLI command shows a telemetry banner or prompt.
...
- **FR13.** The backend's network API has a stable required-set and is forward-compatible with newer client versions. **Required fields** — `installationId`, `version`, `command`, `runtimeName`, `runtimeVersion`, `os`, `arch` — must be present in every payload; the backend rejects with `400 Bad Request` any payload missing one of them. **Optional fields** — `flags`, `packageManager`, `databaseTarget`, `tsVersion`, `agent`, `extensions` — may be omitted by the client; the backend treats omitted nullable scalars as `null` and omitted arrays as `[]`. **Unknown fields** sent by newer clients are accepted and silently dropped before storage (forward compat). The required-set never widens for a given backend version; a backend that accepts a payload today will continue to accept the same payload after future deploys (so older clients keep working as long as they send the original required-set). For a well-formed event — i.e. one carrying all required fields with the correct shapes — the backend never returns a non-2xx response.
...
- **FR16.** The backend Postgres schema is authored as a Prisma Next contract (the backend dogfoods Prisma Next for its own data layer per FR12). One model per accepted submission, fields matching the `TelemetryEvent` shape. Until dashboard query patterns are known, the schema keeps only an `ingestedAt` index:
...
- [ ] **AC12** (FR13) — (a) The backend accepts events that carry the required-set plus optional fields plus unknown fields; unknown fields are dropped, the required + optional fields round-trip into storage, and the response is 2xx. (b) The backend accepts events that carry the required-set and omit some or all optional fields; omitted nullable scalars are stored as `null`, omitted arrays as `[]`, and the response is 2xx. (c) The backend rejects with `400 Bad Request` any payload missing one of the required fields (`installationId`, `version`, `command`, `runtimeName`, `runtimeVersion`, `os`, `arch`).
