...
The cli-telemetry integration suite exercises the actual Bun telemetry backend, not a mock HTTP server: it initializes the backend contract into an isolated dev Postgres database, starts `apps/telemetry-backend/src/server.ts` on an ephemeral port, forks the real sender, and verifies rows stored by the backend. Network-failure cases still point the sender at an unreachable local port because those tests assert silence when no backend is available.
...
- [ ] Integration test suite covering TC-1, TC-2, TC-3, TC-4, TC-5, TC-6, TC-7, TC-8, TC-9, TC-10, TC-11, TC-13, TC-14. Tests run against M1's backend implementation by starting the local Bun service with an isolated dev Postgres database. Use isolated tempdirs for `XDG_CONFIG_HOME` so test runs don't pollute the developer's real install. (satisfies: the listed TCs)
...
**Deliverable.** Close the residual m2 testing gaps (AC1, AC6, AC8 end-to-end assertions); user-facing documentation describing the telemetry surface; a manual code-review audit (TC-16) confirming no PII leakage; ADRs promoted from Proposed to Accepted; and the project close-out: transient project workspace deleted, all repo-wide references to `projects/cli-telemetry/**` replaced with canonical `docs/` links or removed.

**Demonstrable as.** Documentation merged and reviewable in `docs/`; audit checklist artefact present in the close-out PR; `projects/cli-telemetry/` no longer exists on `main`; `grep -r "projects/cli-telemetry" /Users/aqrln/prisma/prisma-next/docs /Users/aqrln/prisma/prisma-next/README.md /Users/aqrln/prisma/prisma-next/AGENTS.md` returns no hits.

**Tasks:**

- [ ] **M3.0 — Close the residual m2 testing gaps** (user-confirmed disposition at m3 entry; see `reviews/code-review.md § Orchestrator notes → m2 post-saga close`). Two sub-deliverables:
  - **CLI-process end-to-end tests** for TC-1 (AC1) and TC-9 (AC6). Add a new test file (suggested: `packages/1-framework/3-tooling/cli-telemetry/test/cli-e2e.test.ts`) that reuses the existing `startBackend(port)` + `createDevDatabase` harness from `integration.test.ts`, isolates `XDG_CONFIG_HOME` to a per-test tempdir, and spawns the real CLI binary via `pnpm exec prisma-next ...`. Cases: (a) `prisma-next --help` writes no `config.json`, backend row count = 0; (b) interactive `prisma-next init --target postgres --authoring psl --no-install --no-skill` with stubbed-stdin yes-answer creates `config.json` with valid v4 `installationId` and produces one backend row; (c) follow-up CLI invocation produces a second row sharing the `installationId` from step (b); (d) a CLI command that crashes mid-execution (use an existing command pointed at a deliberately broken input, or a hidden `--telemetry-crash-test` mode the implementer adds for this purpose) still produces a backend row. (satisfies: TC-1, TC-9)
  - **Stderr-emptiness assertions** for TC-11 (AC8). Change the `cli-telemetry end-to-end — failure modes are silent` block in `packages/1-framework/3-tooling/cli-telemetry/test/integration.test.ts` from `stdio: ['pipe', 'ignore', 'ignore', 'ipc']` to `stdio: ['pipe', 'pipe', 'pipe', 'ipc']`, capture stdout + stderr into in-memory buffers, and assert `stderr.length === 0` and `stdout.length === 0` after the child exits when `PRISMA_NEXT_DEBUG` is unset. Add one additional case that sets `PRISMA_NEXT_DEBUG=1` in the child env and asserts stderr contains the `[cli-telemetry]` prefix, so the debug-path inverse is covered too. (satisfies: TC-11)
- [ ] Write the user-facing telemetry documentation (per spec FR11): what fields are collected (link to the canonical event-shape doc); the user-level `config.json` file (location, structure, how to edit `enableTelemetry` to flip the choice, how to fully reset by deleting the file); the two env-var opt-out signals (`PRISMA_NEXT_DISABLE_TELEMETRY`, `DO_NOT_TRACK`); the `init` consent prompt (final wording, default-Yes, when it appears, why it isn't re-shown); the default-off behaviour when `enableTelemetry` is undefined; the rationale for telemetry being a per-user choice rather than per-project; and the best-effort nature of agent detection. Target location: `docs/Telemetry.md` (flat path; user explicit decision at m3 entry — neither `docs/oss/` nor `docs/onboarding/` fit cleanly). Link from `docs/README.md` and `AGENTS.md` (this repo's source-of-truth doc; the plan's earlier reference to `CLAUDE.md` predates the rename). (satisfies: spec FR11)
...
- `pnpm lint:deps`
- `pnpm build`
- `grep -r "projects/cli-telemetry" /Users/aqrln/prisma/prisma-next/docs /Users/aqrln/prisma/prisma-next/README.md /Users/aqrln/prisma/prisma-next/AGENTS.md` (must return no hits before merge)
...
- **OI5 (M1 → M3).** Root `package.json` `fixtures:emit` script's hardcoded `--filter` list did not include `@prisma-next/telemetry-backend`, so `pnpm fixtures:check` did not gate contract drift for the backend's contract artefacts (`apps/telemetry-backend/src/prisma/contract.json` / `contract.d.ts`). **Resolved in M1 R2**: `fixtures:emit` now includes `@prisma-next/telemetry-backend`, and `fixtures:check` diffs the backend's generated `contract.json` / `contract.d.ts` artefacts.
- **OI6 (M1 design → wire format).** The backend's arktype schema originally defaulted `runtimeName`, `runtimeVersion`, `os`, and `arch` to the literal string `'unknown'` when a client omitted them — a sentinel approach forced by the original FR13 backward-compat tolerance clause combined with FR16's NOT NULL pinning of those columns. The team flagged the consequence (downstream adoption-rate / version-distribution queries would have to filter `'unknown'` out of real values) before the trade-off hardened. **Resolved in M1 R2**: the spec was amended (FR13 + AC12), backend validation now requires those four fields, and AC12(c) covers `400 Bad Request` for any missing required field.
- **OI7 (M2 review blockers).** Review of `wuymqvwz` found the M2 client implementation was not acceptance-ready yet. **Resolved in follow-up**: first consenting `init` now fires a one-shot telemetry send after affirmative consent is persisted; the preAction hook resolves env/CI/user-consent gates before loading project config; the `init` consent prompt is suppressed under `PRISMA_NEXT_DISABLE_TELEMETRY`, `DO_NOT_TRACK=1`, and CI; flag-name collection reports only user-supplied user-facing long option names; the CLI version is sourced from package metadata; and Codex CLI detection has an explicit marker/limitation note.
- **OI8 (backend hardening/docs follow-ups).** Backend hardening/docs follow-up is complete: the backend now enforces a 32 KiB request-body cap, bounds telemetry scalar and array-item string sizes in the arktype schema (without a separate flag/extension count cap), refreshes `apps/telemetry-backend/README.md` for amended FR13, and avoids Biome suppression in `apps/telemetry-backend/prisma-next.config.ts`. Accepted decisions: `PRISMA_NEXT_TELEMETRY_ENDPOINT` may point at an explicit `http://` URL when set, and the current `bunx @prisma/cli@preview` deploy script is temporary until it is replaced by a custom `@prisma/compute-sdk` script.
...
- **M1 R2** — SATISFIED. Spec amendment to FR13 + AC12 landed: `runtimeName`, `runtimeVersion`, `os`, and `arch` are now required, backend rejects missing required fields with `400 Bad Request`, and OI5's workspace-wide fixture drift gate covers the backend contract artefacts. Re-deploy is still required so production matches the tightened wire format.
- **M2 R1 review (`wuymqvwz`)** — ANOTHER ROUND NEEDED. The client scaffold, gating/config store, sender, CLI hook, consent prompt, CI helper, and tests were present, but review found acceptance blockers captured in OI7 plus hardening/docs follow-ups in OI8.
- **M2 R1 follow-up** — OI7/OI8 fixed with targeted tests and validation. Full M2 SATISFIED status still requires the broader milestone validation/AC pass (TC-1 through TC-14), but the review blockers from `wuymqvwz` are addressed.
- **M2 R2 review** — ANOTHER ROUND NEEDED at the time. Three testing-gap findings (F1: no CLI-process e2e for AC1/AC6; F2: no perf comparison for AC5; F3: no stderr-emptiness for AC8). All should-fix; design soundness verified, assertion bar not met.
- **M2 close (post-saga)** — SATISFIED. The user landed the m2 surface to `main` after a multi-week PR-review saga (40 commits, branch `telemetry` deleted post-merge). On m3 entry the user confirmed dispositions for the three open findings: F1 → closed under m3.0 dispatch (add the e2e tests); F2 → resolved-by-design-change at saga commit `86f143164f9e` (`fix(cli): fire telemetry preAction hook as void, do not await it`) which eliminated the await-blocking risk by design; F3 → closed under m3.0 dispatch (capture + assert stderr emptiness). Full details at `reviews/code-review.md § Orchestrator notes → m2 post-saga close`.
- **M3 setup** — operating in unattended mode; decisions log at `wip/unattended-decisions.md`. Dispatch decomposition: M3.0 (m2 testing gaps), M3.1 (`docs/Telemetry.md`), M3.2 (AC13 audit), M3.3 (ADRs 216/217 → Accepted + cross-ref retarget), then `drive-close-project` for AC mapping + project-folder deletion + close-out PR (referencing TML-2557 directly).
