...
- **OI3 (M1 internal).** Choice of backend package location and workspace-layout strategy (top-level `apps/*` glob vs. a new domain inside `packages/`). Resolved during M1 task 1; called out here so reviewers can challenge the choice before the rest of M1 lands on top of it.
- **OI4 (cross-milestone).** The Prisma Compute deploy URL becomes a build-time constant in the M2 client. If the URL changes between M1 deploy and M2 release, the M2 client must be rebuilt — small operational concern, mention in the M1 close-out so M2 picks up the final URL.
