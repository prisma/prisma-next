...
- **FR7.** PSL invalid-usage diagnostics for `temporal.*` calls (and any field-preset call) derive from preset-instantiation rules and PSL surface rules only; there is no codec-applicability diagnostic on the authoring path. The complete set, with implemented diagnostic codes:
  ...
  - **Preset call combined with `@updatedAt` (during the transition before Phase C deletes the attribute)** → `PSL_PRESET_AND_UPDATED_AT_CONFLICT` (new code, transient — disappears with the attribute path in Phase C).
...
- [x] **AC5j.** `f temporal.updatedAt() @updatedAt` (transient: preset + the attribute that's being deleted in Phase C) produces `PSL_PRESET_AND_UPDATED_AT_CONFLICT`. Disappears in Phase C along with the attribute path.
