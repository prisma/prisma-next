...
- **FR4.** SQL TypeScript authoring exposes `field.temporal.createdAt()` and `field.temporal.updatedAt()` from the same registry entries the PSL surface consumes; equivalent PSL and TS models emit byte-equivalent contracts (verified via a shared `expectTimestampParity` harness used by both Postgres and SQLite parity tests). Object-key order is irrelevant to the comparison because `expect(actual).toEqual(expected)` performs a deep structural equality check.
...
- [x] **AC3.** The equivalent SQL TypeScript model using `field.temporal.createdAt()` and `field.temporal.updatedAt()` emits the same contract shape as the PSL model for Postgres and SQLite. Both targets' parity tests run through the shared `expectTimestampParity` harness in `ts-psl-parity.test.ts`, which builds the TS contract, interprets the PSL document, and asserts deep equality (`expect(actual).toEqual(expected)`). Covers FR4, NFR2.
