...
- **FR5.** PSL no longer recognizes `@updatedAt` as a known attribute. References to it produce a `PSL_UNKNOWN_ATTRIBUTE` diagnostic enhanced with a targeted migration hint ("`@updatedAt` is not a Prisma Next attribute. Use `temporal.updatedAt()` as a field-preset call instead."). The hint is implemented as a hardcoded `if (name === 'updatedAt')` branch in the diagnostic emitter — not a map. The hint is suppressed when the field already declares any `temporal.*` preset, to avoid telling users to do what they already did.
...
- **FR7.** PSL invalid-usage diagnostics for `temporal.*` calls (and any field-preset call) derive from preset-instantiation rules and PSL surface rules only; there is no codec-applicability diagnostic on the authoring path. The complete set:
  - **Wrong arity** (`temporal.updatedAt(123)`) → `PSL_INVALID_NAMESPACED_CALL_ARITY` (a renamed/generalized version of the existing `PSL_INVALID_TYPE_CONSTRUCTOR_ARITY`, applied uniformly to type-constructors and field-presets).
  - **Unknown preset name within a known namespace** (`temporal.foo()`) → `PSL_UNKNOWN_FIELD_PRESET` (new code).
  ...
  - **Preset call on an optional field** (`updatedAt temporal.updatedAt()?`) → hard error with a stable code (proposed: `PSL_PRESET_NOT_OPTIONAL`). The preset's whole point is that the system owns the value; optional contradicts that.
  - **Preset call on a list field** (`updatedAt temporal.updatedAt()[]`) → hard error (proposed: `PSL_PRESET_NOT_LIST`).
  - **Preset call inside `@default(...)` argument position** (`f DateTime @default(temporal.updatedAt())`) → existing `PSL_INVALID_DEFAULT_EXPRESSION` or equivalent.
  - **Preset call combined with `@id`** (`id temporal.updatedAt() @id`) → hard error; presets express their own id semantics via the descriptor's `id` flag.
  - **Same field declaring two preset calls** (`f temporal.updatedAt() temporal.createdAt()`) → likely already a parse error; covered by a regression test.
...
- [ ] **AC4.** PSL `@updatedAt` produces `PSL_UNKNOWN_ATTRIBUTE` enhanced with a targeted migration hint pointing at `temporal.updatedAt()`. The diagnostic message includes the literal preset spelling so users can copy-paste the fix. The diagnostic carries no codec-applicability messaging. The `BUILTIN_FIELD_ATTRIBUTE_NAMES` set no longer contains `'updatedAt'`. **The hint is suppressed when the field already declares any `temporal.*` preset.** Covers FR5.
- [ ] **AC5a.** `temporal.updatedAt(123)` produces `PSL_INVALID_NAMESPACED_CALL_ARITY` with a span on the offending argument. Covers FR7 (arity).
- [ ] **AC5b.** `temporal.foo()` produces `PSL_UNKNOWN_FIELD_PRESET` with a span on the preset name. Covers FR7 (unknown preset).
- [ ] **AC5c.** `weather.updatedAt()` (unknown namespace, not extension-composed) produces `PSL_EXTENSION_NAMESPACE_NOT_COMPOSED`. Covers FR7 (unknown namespace).
- [ ] **AC5d.** `updatedAt temporal.updatedAt() @default(now())` produces `PSL_PRESET_AND_DEFAULT_CONFLICT` (new code, distinct from `PSL_INVALID_DEFAULT_APPLICABILITY`). Covers FR7 (double default).
- [ ] **AC5e.** `updatedAt temporal.updatedAt()?` produces `PSL_PRESET_NOT_OPTIONAL`. Covers FR7 (optional).
- [ ] **AC5f.** `updatedAt temporal.updatedAt()[]` produces `PSL_PRESET_NOT_LIST`. Covers FR7 (list).
- [ ] **AC5g.** `f DateTime @default(temporal.updatedAt())` produces `PSL_INVALID_DEFAULT_EXPRESSION` (or the existing closest stable code). Covers FR7 (preset in default-arg position).
- [ ] **AC5h.** `id temporal.updatedAt() @id` produces a hard error with a stable code. Covers FR7 (preset + `@id`).
- [ ] **AC5i.** `f temporal.updatedAt() temporal.createdAt()` produces a parse-time or resolution-time error. Covers FR7 (double preset on one field).
...
- **RD11.** Diagnostic codes are renamed for honesty: `PSL_INVALID_TYPE_CONSTRUCTOR_ARITY` → `PSL_INVALID_NAMESPACED_CALL_ARITY` (uniformly applied to type-constructors and field-presets). The double-default conflict for preset + `@default` gets a new code `PSL_PRESET_AND_DEFAULT_CONFLICT` rather than reusing `PSL_INVALID_DEFAULT_APPLICABILITY` — the actual error is "duplicate default," not "not applicable here." Reusing the misleading code is exactly the accumulated-debt pattern the original `@updatedAt`-applicability check exemplified; we don't repeat it. New codes added in this project: `PSL_INVALID_NAMESPACED_CALL_ARITY` (rename), `PSL_UNKNOWN_FIELD_PRESET`, `PSL_PRESET_AND_DEFAULT_CONFLICT`, `PSL_PRESET_NOT_OPTIONAL`, `PSL_PRESET_NOT_LIST`.
