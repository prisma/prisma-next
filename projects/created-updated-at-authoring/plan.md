...
| AC5a | TC6a | PSL `temporal.updatedAt(123)` | Unit | `PSL_INVALID_NAMESPACED_CALL_ARITY` with span on the offending argument |
...
- [ ] **Diagnostic code work:**
  - Rename `PSL_INVALID_TYPE_CONSTRUCTOR_ARITY` to `PSL_INVALID_NAMESPACED_CALL_ARITY` (applies uniformly to type constructors and field presets).
  - Add new codes: `PSL_UNKNOWN_FIELD_PRESET`, `PSL_PRESET_AND_DEFAULT_CONFLICT`, `PSL_PRESET_NOT_OPTIONAL`, `PSL_PRESET_NOT_LIST`, `PSL_INVALID_NAMESPACED_CALL_ARGUMENT`.
  - **Do not** reuse `PSL_INVALID_DEFAULT_APPLICABILITY` for the preset+`@default` collision — the actual error is "duplicate default," not "not applicable here." Add `PSL_PRESET_AND_DEFAULT_CONFLICT` and use it. Satisfies TC6a–TC6f.
...
- Diagnostic codes are renamed for honesty: `PSL_INVALID_TYPE_CONSTRUCTOR_ARITY` → `PSL_INVALID_NAMESPACED_CALL_ARITY` (applies to both type-constructors and field-presets). New codes added in this project: `PSL_UNKNOWN_FIELD_PRESET`, `PSL_PRESET_AND_DEFAULT_CONFLICT`, `PSL_PRESET_NOT_OPTIONAL`, `PSL_PRESET_NOT_LIST`, `PSL_INVALID_NAMESPACED_CALL_ARGUMENT`. Reusing the misleading `PSL_INVALID_DEFAULT_APPLICABILITY` for the preset+`@default` collision is rejected — it's exactly the accumulated-debt pattern the original `@updatedAt`-applicability check exemplified.
