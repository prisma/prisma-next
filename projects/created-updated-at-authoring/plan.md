...
| AC3 | TC4 | TS `field.temporal.createdAt()` / `field.temporal.updatedAt()` produce contracts byte-equivalent to their PSL counterparts | Unit | The shared `expectTimestampParity` harness in `ts-psl-parity.test.ts` builds the TS contract, interprets the PSL document, and asserts deep structural equality (`toEqual`) for Postgres and SQLite |
...
- [ ] Update PSL test fixtures and tests:
  ...
  - `packages/2-sql/2-authoring/contract-psl/test/ts-psl-parity.test.ts` (lines 156–207, 246–251, 432–467) — port to `temporal.updatedAt()` ↔ `field.temporal.updatedAt()`. Drive both targets through the shared `expectTimestampParity` harness so the parity assertion is one call site per target. Satisfies TC3, TC4, TC5a, TC5b, TC8.
...
- The TS surface re-namespaces from `field.{createdAt,updatedAt}` to `field.temporal.{createdAt,updatedAt}`. PSL and TS share **leaf names**, not full paths — this is parallel-leaf-naming, not symmetric path-naming. The cost (one extra TS nesting level) is paid for registry-path parity (one shared `expectTimestampParity` harness in `ts-psl-parity.test.ts`, eliminated drift class).
