...
A working example app lives at `packages/3-extensions/supabase/examples/basic/`. It exercises every framework primitive end-to-end:
...
The example is a regression test surface: it runs in CI against a real Supabase project (or PGlite-with-supabase-schema for self-contained tests, depending on what's feasible). The example doubles as the demo content for the launch announcement.
...
- **FR20.** A working example app lives at `packages/3-extensions/supabase/examples/basic/` (or its analog) demonstrating:
...
This project lands the May 18th launch milestone. Slipping any upstream dependency cascades into a launch-date risk.
...
- [ ] **AC8.** End-to-end test against PGlite-with-Supabase-schema:
...
- [ ] **AC9.** The example app at `packages/3-extensions/supabase/examples/basic/` runs in CI green. Its README is reviewed for accuracy by a non-author teammate before the launch.
...
- **Tests** (unit tests for JWT validation, role binding, middleware invisibility; integration tests for RLS enforcement against PGlite-with-Supabase-schema; bundle-size CI check): ~500–800 LOC.
...
CI cost: the example app's integration tests against PGlite-with-Supabase-schema run on every PR. Estimated ~30s of CI time per run.
...
- **Where does the example app live in the workspace?** Two plausible homes: `packages/3-extensions/supabase/examples/basic/` (co-located with the package) or `examples/supabase-integration/` (in the workspace's top-level `examples/`). **Working assumption: co-located** (`packages/3-extensions/supabase/examples/basic/`) — keeps the example versioned with the package, matches the convention in `packages/3-extensions/cipherstash/examples/`. The implementer confirms by inspection of existing extension package layouts.
- **CI test database: real Supabase project vs PGlite-with-Supabase-schema?** Both have trade-offs. A real Supabase project tests against authentic Supabase behaviour (including any quirks of the actual `auth.*` schema and trigger setup) but requires CI to manage credentials. PGlite seeded with a minimal Supabase-mimicking schema is hermetic but might miss authentic Supabase behaviour. **Working assumption: PGlite-seeded for the package's own test suite (hermetic, fast, no credentials), real Supabase project for the launch-blocking acceptance test (run manually before launch, not in CI).**
