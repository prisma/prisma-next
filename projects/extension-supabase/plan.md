...
> **Scope note — the runtime PRs below are largely already shipped.** `SupabaseRuntime`, the `supabase()` factory, `asUser`/`asAnon`/`asServiceRole`, JWT validation, and `RoleBoundDb` landed under the runtime-target-layer project ([ADR 230](../../docs/architecture%20docs/adrs/ADR%20230%20-%20Runtime%20target%20layer%20session-coupled%20connections.md)), with role binding via session-coupled connections (not the `withRawConnection`/`SET LOCAL`-override design described below). The "M2 runtime facade" PR is therefore mostly done; this project's live work is the contract/handles/pack/example surface. Re-plan against the shipped runtime at drive-start.
