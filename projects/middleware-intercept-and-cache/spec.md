...
0. **`identityKey` on `RuntimeMiddlewareContext`.** Required method `identityKey(exec: ExecutionPlan): string` on `RuntimeMiddlewareContext` in `@prisma-next/framework-components/runtime`. Family runtimes populate the implementation: SQL composes `meta.storageHash + '|' + exec.sql + '|' + canonicalStringify(exec.params)`; Mongo composes `meta.storageHash + '|' + canonicalStringify(exec.command)`. Two semantically equivalent executions produce the same string. The return value is consumed directly as a `Map<string, …>` key by middleware — it is **not** hashed. Empirical data from prior Prisma query-plan caching work shows that no JS-side hash function (FNV, xxhash, SHA-256) outperforms using the canonical string as a `Map` key directly: V8 interns and hashes strings internally on insertion, and any user-space hash pays JIT/allocation overhead on top of that. The cache middleware therefore uses `ctx.identityKey(exec)` as the literal `Map` key, not as input to a further hash. **Note:** `computeSqlFingerprint` (which exists in `sql-runtime/src/fingerprint.ts` for telemetry) strips literals to group executions by statement shape — the *opposite* of what `identityKey` needs. Do not reuse it; `identityKey` wants the raw SQL plus canonicalized params so it discriminates between executions of the same statement with different parameter values.
...
5. **~~Cache key hashing cost.~~** Resolved: no hashing. The cache middleware uses the canonical `identityKey` string directly as a `Map<string, …>` key. Empirical data from prior Prisma query-plan caching work shows V8's internal string interning and hashing dominates any user-space hash function — adding SHA-256/FNV/xxhash on top makes the keying *slower*, not faster. Canonicalization (stable serialization regardless of object key order) is the only nontrivial cost in the key path; `canonicalStringify` lives in `@prisma-next/utils/canonical-stringify` so SQL and Mongo runtimes share a single source of truth. Handles BigInt, Date, Buffer/Uint8Array. If a future profile shows canonicalization itself dominating, optimize there — not via output hashing.
