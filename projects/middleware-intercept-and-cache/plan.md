...
- [ ] **1.0a' Add `hashIdentity` to `@prisma-next/utils`.** New module `packages/1-framework/0-foundation/utils/src/hash-identity.ts` exporting `hashIdentity(value: string): string`. Wraps Node's stdlib `createHash('blake2b512')` and returns `` `blake2b512:${hex}` `` — 137 chars total, fixed regardless of input size. The contract is "produce a bounded, opaque digest of a canonical-string identity." Two reasons for hashing rather than using the canonical string directly: (a) bounded memory — a 10 MB JSON parameter would otherwise produce a 10 MB cache key, scaling to gigabytes at `maxEntries=1000`; (b) sensitive-data isolation — parameter values flow into debug logs, Redis `KEYS` output, persistence dumps, and any user-supplied `CacheStore` implementation. BLAKE2b-512 chosen over SHA-256 for speed on 64-bit hardware; 512-bit output for collision headroom at negligible incremental cost. Unit tests: fixed-length output; deterministic across repeated calls; discriminates differing inputs (incl. trailing-character and separator-placement edge cases); opacity (output does not embed input); UTF-8 multi-byte handling; Unicode normalization is byte-significant (NFC vs NFD differ).

- [ ] **1.0b Populate `identityKey` in `SqlRuntimeImpl`.** In `packages/2-sql/5-runtime/src/sql-runtime.ts` (via a new `sql-runtime/src/identity-key.ts` helper), populate `sqlCtx.identityKey(exec)` as `` hashIdentity(`${exec.meta.storageHash}|${exec.sql}|${canonicalStringify(exec.params)}`) ``. Import `canonicalStringify` from `@prisma-next/utils/canonical-stringify` and `hashIdentity` from `@prisma-next/utils/hash-identity`. Do **not** reuse `computeSqlFingerprint` (it strips literals for telemetry grouping; `identityKey` needs the opposite — raw SQL plus per-param discrimination). Unit-test in `sql-runtime/test/`: same `(sql, params, storageHash)` → same key; differing params → different keys; differing `storageHash` → different keys; key order in object params does not affect key; output matches `^blake2b512:[0-9a-f]{128}$`; output does not embed raw SQL or params (opacity); fixed-size output regardless of payload size.

- [ ] **1.0c Populate `identityKey` in `MongoRuntimeImpl`.** In `packages/2-mongo-family/7-runtime/src/mongo-runtime.ts` (via a new `mongo-runtime/src/identity-key.ts` helper), populate `ctx.identityKey(exec)` as `` hashIdentity(`${exec.meta.storageHash}|${canonicalStringify(exec.command)}`) ``. Import the same `canonicalStringify` and `hashIdentity` from `@prisma-next/utils`. Unit-test in `mongo-runtime/test/`: distinct commands → distinct keys; equivalent commands with shuffled object key order → same key; output matches `^blake2b512:[0-9a-f]{128}$`; output does not embed command payload (opacity); fixed-size output regardless of payload size.
...
- [ ] **3.15 Unit tests — key composition.** In `middleware-cache/test/cache-key.test.ts`:
  - Default path: `ctx.identityKey(exec)` is invoked and its return value is used directly as the `Map` key (no further transformation in the cache middleware; the family runtime is responsible for hashing inside `identityKey`).
  ...
  - User-supplied `cacheAnnotation.apply({ key })` short-circuits `ctx.identityKey` (assert `identityKey` is not invoked when annotation `key` is supplied). User-supplied keys are stored as-is — no rehashing.
  - (Canonicalization stability across object-key order is covered by `canonicalStringify` tests in 1.0a; hash output format / opacity / bounded size are covered by `hashIdentity` tests in 1.0a'; not duplicated here.)
...
| `hashIdentity` exists in `@prisma-next/utils` | Unit | 1.0a' | `blake2b512:HEXDIGEST` format |
| `hashIdentity` produces fixed-length output regardless of input | Unit | 1.0a' | 137 chars |
| `hashIdentity` is deterministic and discriminating | Unit | 1.0a' | Trailing chars + separator placement |
| `hashIdentity` output does not embed raw input (opacity) | Unit | 1.0a' | Sensitive-data isolation |
...
| SQL `identityKey` returns `blake2b512:` digest (fixed-size, opaque) | Unit | 1.0b | Output format + opacity + size |
| Mongo `identityKey` returns `blake2b512:` digest (fixed-size, opaque) | Unit | 1.0c | Output format + opacity + size |
| Cache middleware uses `identityKey` output directly (no rehash) | Unit | 3.15 | `Map<string, …>` key is `identityKey` output as-is |
...
- **Cache key hashing perf (spec OQ 5)** — Resolved: hash via BLAKE2b-512. Earlier analysis (per prior Prisma query-plan caching work) suggested skipping hashing because V8's internal string-interning Map performance dominates any user-space hash. That analysis applies to **parametric** plan caching with small structural fingerprints — it does not generalize to **per-execution** identity keys that embed concrete parameter values. Two failure modes drove the reversal: memory (10 MB JSON params → 10 MB key, scaling to gigabytes at `maxEntries=1000`) and sensitive-data isolation (parameter values flowing into debug logs, Redis `KEYS`, persistence dumps, user-supplied `CacheStore` implementations). BLAKE2b-512 chosen over SHA-256 for speed on 64-bit hardware and 512-bit output for collision headroom; both are stdlib in Node. `canonicalStringify` (1.0a) and `hashIdentity` (1.0a') both live in `@prisma-next/utils` so SQL and Mongo runtimes share a single source of truth. Output format: `blake2b512:HEXDIGEST` (137 chars, fixed).
