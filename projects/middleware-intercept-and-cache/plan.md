...
- [ ] **1.0 Add `contentHash` to `RuntimeMiddlewareContext`.** Required method `contentHash(exec: ExecutionPlan): string` in `packages/1-framework/1-core/framework-components/src/runtime-middleware.ts`. Update the existing `RuntimeMiddlewareContext` type test in `framework-components/test/runtime-middleware.types.test-d.ts` to assert the new property. Update the three in-repo `RuntimeMiddlewareContext` fixtures (`framework-components/test/mock-family.test.ts`, `framework-components/test/run-with-middleware.test.ts`, `framework-components/test/runtime-core.test.ts`) with a stub implementation (e.g. `contentHash: () => 'mock-hash'`). `pnpm typecheck` must remain green; surface any other fixture sites the typechecker reveals.
...
| `hashContent` exists in `@prisma-next/utils` | Unit | 1.0a' | `blake2b512:HEXDIGEST` format |
| `hashContent` produces fixed-length output regardless of input | Unit | 1.0a' | 137 chars |
...
| `RuntimeMiddlewareContext.contentHash(exec)` exists, returns `string` | Type test | 1.0 | Required field; existing fixtures updated |
...
| SQL `contentHash` returns `blake2b512:` digest (fixed-size, opaque) | Unit | 1.0b | Output format + opacity + size |
| Mongo `contentHash` returns `blake2b512:` digest (fixed-size, opaque) | Unit | 1.0c | Output format + opacity + size |
...
- **Cache key hashing perf (spec OQ 5)** — Resolved: hash via BLAKE2b-512. Earlier analysis (per prior Prisma query-plan caching work) suggested skipping hashing because V8's internal string-interning Map performance dominates any user-space hash. That analysis applies to **parametric** plan caching with small structural fingerprints — it does not generalize to **per-execution** content hashes that embed concrete parameter values. Two failure modes drove the reversal: memory (10 MB JSON params → 10 MB key, scaling to gigabytes at `maxEntries=1000`) and sensitive-data isolation (parameter values flowing into debug logs, Redis `KEYS`, persistence dumps, user-supplied `CacheStore` implementations). BLAKE2b-512 chosen over SHA-256 for speed on 64-bit hardware and 512-bit output for collision headroom; both are stdlib in Node. `canonicalStringify` (1.0a) and `hashContent` (1.0a') both live in `@prisma-next/utils` so SQL and Mongo runtimes share a single source of truth. Output format: `blake2b512:HEXDIGEST` (137 chars, fixed).
...
- **`contentHash` design surface** — Returns a string (not a structured `{statement, params}` shape, not async, not batched). Future content-hash-keyed middleware (request coalescing, single-flight) consume the same string. If a structured shape becomes necessary, evolving the SPI is additive.
